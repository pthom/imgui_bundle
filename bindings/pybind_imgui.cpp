...
void py_init_module_imgui_main(py::module& m)
{
    ...
    auto pyClassImVec2 =
        py::class_<ImVec2>    // imgui.h:261
            (m, "ImVec2", "")
        .def_readwrite("x", &ImVec2::x, "")    // imgui.h:263
        .def_readwrite("y", &ImVec2::y, "")    // imgui.h:263
        .def(py::init<>())    // imgui.h:264
        .def(py::init<float, float>(),    // imgui.h:265
            py::arg("_x"), py::arg("_y"))
        ;


    auto pyClassImVec4 =
        py::class_<ImVec4>    // imgui.h:274
            (m, "ImVec4", "ImVec4: 4D vector used to store clipping rectangles, colors etc. [Compile-time configurable type]")
        .def_readwrite("x", &ImVec4::x, "")    // imgui.h:276
        .def_readwrite("y", &ImVec4::y, "")    // imgui.h:276
        .def_readwrite("z", &ImVec4::z, "")    // imgui.h:276
        .def_readwrite("w", &ImVec4::w, "")    // imgui.h:276
        .def(py::init<>())    // imgui.h:277
        .def(py::init<float, float, float, float>(),    // imgui.h:278
            py::arg("_x"), py::arg("_y"), py::arg("_z"), py::arg("_w"))
        ;


    m.def("create_context",    // imgui.h:296
        ImGui::CreateContext,
        py::arg("shared_font_atlas") = py::none(),
        pybind11::return_value_policy::reference);

    m.def("destroy_context",    // imgui.h:297
        ImGui::DestroyContext,
        py::arg("ctx") = py::none(),
        "None = destroy current context");

    m.def("get_current_context",    // imgui.h:298
        ImGui::GetCurrentContext, pybind11::return_value_policy::reference);

    m.def("set_current_context",    // imgui.h:299
        ImGui::SetCurrentContext, py::arg("ctx"));

    m.def("get_io",    // imgui.h:302
        ImGui::GetIO,
        "access the IO structure (mouse/keyboard/gamepad inputs, time, various configuration options/flags)",
        pybind11::return_value_policy::reference);

    m.def("get_style",    // imgui.h:303
        ImGui::GetStyle,
        "access the Style structure (colors, sizes). Always use PushStyleCol(), PushStyleVar() to modify style mid-frame!",
        pybind11::return_value_policy::reference);

    m.def("new_frame",    // imgui.h:304
        ImGui::NewFrame, "start a new Dear ImGui frame, you can submit any command from this point until Render()/EndFrame().");

    m.def("end_frame",    // imgui.h:305
    ...
    m.def("show_metrics_window",    // imgui.h:311
        [](std::optional<bool> p_open = std::nullopt) -> std::optional<bool>
        {
    ...
    m.def("show_debug_log_window",    // imgui.h:312
        [](std::optional<bool> p_open = std::nullopt) -> std::optional<bool>
        {
    ...
    m.def("show_stack_tool_window",    // imgui.h:313
        [](std::optional<bool> p_open = std::nullopt) -> std::optional<bool>
        {
    ...
    m.def("show_about_window",    // imgui.h:314
        [](std::optional<bool> p_open = std::nullopt) -> std::optional<bool>
        {
    ...
    m.def("show_style_editor",    // imgui.h:315
        ImGui::ShowStyleEditor,
        py::arg("ref") = py::none(),
        "add style editor block (not a window). you can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)");

    m.def("show_style_selector",    // imgui.h:316
        ImGui::ShowStyleSelector,
        py::arg("label"),
        "add style selector block (not a window), essentially a combo listing the default styles.");

    m.def("show_font_selector",    // imgui.h:317
        ImGui::ShowFontSelector,
        py::arg("label"),
        "add font selector block (not a window), essentially a combo listing the loaded fonts.");

    m.def("show_user_guide",    // imgui.h:318
    ...
    m.def("get_version",    // imgui.h:319
        ImGui::GetVersion,
        "get the compiled version string e.g. \"1.80 WIP\" (essentially the value for IMGUI_VERSION from the compiled version of imgui.cpp)",
        pybind11::return_value_policy::reference);

    m.def("style_colors_dark",    // imgui.h:322
        ImGui::StyleColorsDark,
        py::arg("dst") = py::none(),
        "new, recommended style (default)");

    m.def("style_colors_light",    // imgui.h:323
        ImGui::StyleColorsLight,
        py::arg("dst") = py::none(),
        "best used with borders and a custom, thicker font");

    m.def("style_colors_classic",    // imgui.h:324
        ImGui::StyleColorsClassic,
        py::arg("dst") = py::none(),
        "classic imgui style");

    m.def("begin",    // imgui.h:338
    ...
    m.def("end",    // imgui.h:339
        ImGui::End);

    m.def("begin_child",    // imgui.h:349
    ...
    m.def("end_child",    // imgui.h:351
        ImGui::EndChild);

    m.def("is_window_appearing",    // imgui.h:355
        ImGui::IsWindowAppearing);

    m.def("is_window_collapsed",    // imgui.h:356
        ImGui::IsWindowCollapsed);

    m.def("is_window_focused",    // imgui.h:357
        ImGui::IsWindowFocused,
        py::arg("flags") = 0,
        "is current window focused? or its root/child, depending on flags. see flags for options.");

    m.def("is_window_hovered",    // imgui.h:358
        ImGui::IsWindowHovered,
        py::arg("flags") = 0,
        "is current window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you are trying to check whether your mouse should be dispatched to imgui or to your app, you should use the 'io.WantCaptureMouse' boolean for that! Please read the FAQ!");

    m.def("get_window_draw_list",    // imgui.h:359
        py::overload_cast<>(ImGui::GetWindowDrawList),
        "get draw list associated to the current window, to append your own drawing primitives",
        pybind11::return_value_policy::reference);

    m.def("get_window_dpi_scale",    // imgui.h:360
        ImGui::GetWindowDpiScale, "get DPI scale currently associated to the current window's viewport.");

    m.def("get_window_pos",    // imgui.h:361
    ...
    m.def("set_next_window_content_size",    // imgui.h:372
    ...
    m.def("pop_item_width",    // imgui.h:426
        ImGui::PopItemWidth);

    m.def("set_next_item_width",    // imgui.h:427
    ...
    m.def("calc_item_width",    // imgui.h:428
    ...
    m.def("push_text_wrap_pos",    // imgui.h:429
    ...
    m.def("pop_text_wrap_pos",    // imgui.h:430
        ImGui::PopTextWrapPos);

    m.def("get_font",    // imgui.h:434
        ImGui::GetFont,
        "get current font",
        pybind11::return_value_policy::reference);

    m.def("get_font_size",    // imgui.h:435
        ImGui::GetFontSize, "get current font size (= height in pixels) of current font with current scale applied");

    m.def("get_font_tex_uv_white_pixel",    // imgui.h:436
    ...
    m.def("get_color_u32",    // imgui.h:438
    ...
    m.def("get_style_color_vec4",    // imgui.h:440
        ImGui::GetStyleColorVec4,
        py::arg("idx"),
        "retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwise use GetColorU32() to get style color with style alpha baked in.",
        pybind11::return_value_policy::reference);

    m.def("separator",    // imgui.h:449
    ...
    m.def("get_cursor_pos_y",    // imgui.h:460
        ImGui::GetCursorPosY, "other functions such as GetCursorScreenPos or everything in ImDrawList::");

    m.def("set_cursor_pos",    // imgui.h:461
        ImGui::SetCursorPos,
        py::arg("local_pos"),
        "are using the main, absolute coordinate system.");

    m.def("set_cursor_pos_x",    // imgui.h:462
        ImGui::SetCursorPosX,
        py::arg("local_x"),
        "GetWindowPos() + GetCursorPos() == GetCursorScreenPos() etc.)");

    m.def("set_cursor_pos_y",    // imgui.h:463
        ImGui::SetCursorPosY, py::arg("local_y"));

    m.def("get_cursor_start_pos",    // imgui.h:464
        ImGui::GetCursorStartPos, "initial cursor position in window coordinates");

    m.def("get_cursor_screen_pos",    // imgui.h:465
    ...
    m.def("set_cursor_screen_pos",    // imgui.h:466
        ImGui::SetCursorScreenPos,
        py::arg("pos"),
        "cursor position in absolute coordinates");

    m.def("align_text_to_frame_padding",    // imgui.h:467
    ...
    m.def("text_colored",    // imgui.h:497
    ...
    m.def("text_disabled",    // imgui.h:499
        [](const char * fmt)
        {
    ...
    m.def("text_wrapped",    // imgui.h:501
        [](const char * fmt)
        {
    ...
    m.def("checkbox_flags",    // imgui.h:516
    ...
    m.def("checkbox_flags",    // imgui.h:517
    ...
    m.def("image_button",    // imgui.h:526
    ...
    m.def("drag_float_range2",    // imgui.h:553
    ...
    m.def("drag_int_range2",    // imgui.h:558
    ...
    m.def("drag_scalar_n",    // imgui.h:560
    ...
    m.def("slider_float",    // imgui.h:568
    ...
    m.def("slider_float2",    // imgui.h:569
    ...
    m.def("slider_float3",    // imgui.h:570
    ...
    m.def("slider_float4",    // imgui.h:571
    ...
    m.def("slider_angle",    // imgui.h:572
    ...
    m.def("slider_scalar",    // imgui.h:577
    ...
    m.def("slider_scalar_n",    // imgui.h:578
    ...
    m.def("v_slider_float",    // imgui.h:579
    ...
    m.def("v_slider_int",    // imgui.h:580
    ...
    m.def("v_slider_scalar",    // imgui.h:581
    ...
    m.def("input_float2",    // imgui.h:590
    ...
    m.def("input_float3",    // imgui.h:591
    ...
    m.def("input_float4",    // imgui.h:592
    ...
    m.def("input_double",    // imgui.h:597
    ...
    m.def("input_scalar",    // imgui.h:598
    ...
    m.def("input_scalar_n",    // imgui.h:599
    ...
    m.def("color_picker3",    // imgui.h:606
        [](const char * label, std::array<float, 3> col, ImGuiColorEditFlags flags = 0) -> std::tuple<bool, std::array<float, 3>>
        {
    ...
    m.def("color_picker4",    // imgui.h:607
    ...
    m.def("tree_node_ex",    // imgui.h:619
    ...
    m.def("plot_histogram",    // imgui.h:657
        [](const char * label, const py::array & values, int values_offset = 0, const char * overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0), int stride = -1)
        {
    ...
    m.def("begin_popup_context_window",    // imgui.h:723
        py::overload_cast<const char *, ImGuiPopupFlags>(ImGui::BeginPopupContextWindow),
        py::arg("str_id") = py::none(), py::arg("popup_flags") = 1,
        "open+begin popup when clicked on current window.");

    m.def("begin_popup_context_void",    // imgui.h:724
    ...
    m.def("table_set_column_index",    // imgui.h:759
        py::overload_cast<int>(ImGui::TableSetColumnIndex),
        py::arg("column_n"),
        "append into the specified column. Return True when column is visible.");

    m.def("table_setup_column",    // imgui.h:769
    ...
    m.def("table_header",    // imgui.h:772
        py::overload_cast<const char *>(ImGui::TableHeader),
        py::arg("label"),
        "submit one header cell manually (rarely used)");

    m.def("table_get_sort_specs",    // imgui.h:780
    ...
    m.def("table_set_bg_color",    // imgui.h:787
    ...
    m.def("begin_disabled",    // imgui.h:856
        ImGui::BeginDisabled, py::arg("disabled") = true);

    m.def("end_disabled",    // imgui.h:857
        ImGui::EndDisabled);

    m.def("push_clip_rect",    // imgui.h:861
        ImGui::PushClipRect, py::arg("clip_rect_min"), py::arg("clip_rect_max"), py::arg("intersect_with_current_clip_rect"));

    m.def("pop_clip_rect",    // imgui.h:862
        ImGui::PopClipRect);

    m.def("set_item_default_focus",    // imgui.h:866
        ImGui::SetItemDefaultFocus, "make last item the default focused item of a window.");

    m.def("set_keyboard_focus_here",    // imgui.h:867
        ImGui::SetKeyboardFocusHere,
        py::arg("offset") = 0,
        "focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.");

    m.def("is_item_hovered",    // imgui.h:872
        ImGui::IsItemHovered,
        py::arg("flags") = 0,
        "is the last item hovered? (and usable, aka not blocked by a popup, etc.). See ImGuiHoveredFlags for more options.");

    m.def("is_item_active",    // imgui.h:873
    ...
    m.def("is_item_deactivated_after_edit",    // imgui.h:880
    ...
    m.def("get_main_viewport",    // imgui.h:894
        ImGui::GetMainViewport,
        "return primary/default viewport. This can never be None.",
        pybind11::return_value_policy::reference);

    m.def("get_background_draw_list",    // imgui.h:897
    ...
    m.def("get_foreground_draw_list",    // imgui.h:898
    ...
    m.def("color_convert_hsv_to_rgb",    // imgui.h:921
    ...
    m.def("is_mouse_dragging",    // imgui.h:950
    ...
    m.def("get_mouse_drag_delta",    // imgui.h:951
    ...
    m.def("reset_mouse_drag_delta",    // imgui.h:952
        ImGui::ResetMouseDragDelta, py::arg("button") = 0);

    m.def("get_mouse_cursor",    // imgui.h:953
    ...
    m.def("debug_text_encoding",    // imgui.h:972
        ImGui::DebugTextEncoding, py::arg("text"));

    m.def("debug_check_version_and_data_layout",    // imgui.h:973
    ...
    m.def("get_platform_io",    // imgui.h:987
        ImGui::GetPlatformIO,
        "platform/renderer functions, for backend to setup + viewports list.",
        pybind11::return_value_policy::reference);

    m.def("update_platform_windows",    // imgui.h:988
    ...
    m.def("find_viewport_by_id",    // imgui.h:991
        ImGui::FindViewportByID,
        py::arg("id_"),
        "this is a helper for backends.",
        pybind11::return_value_policy::reference);

    m.def("find_viewport_by_platform_handle",    // imgui.h:992
    ...
    py::enum_<ImGuiWindowFlags_>(m, "WindowFlags_", py::arithmetic(), " Flags for ImGui::Begin()\n (Those are per-window flags. There are shared flags in ImGuiIO: io.ConfigWindowsResizeFromEdges and io.ConfigWindowsMoveFromTitleBarOnly)")    // imgui.h:1002
    ...
    py::enum_<ImGuiInputTextFlags_>(m, "InputTextFlags_", py::arithmetic(), " Flags for ImGui::InputText()\n (Those are per-item flags. There are shared flags in ImGuiIO: io.ConfigInputTextCursorBlink and io.ConfigInputTextEnterKeepActive)")    // imgui.h:1043
    ...
    py::enum_<ImGuiTreeNodeFlags_>(m, "TreeNodeFlags_", py::arithmetic(), "Flags for ImGui::TreeNodeEx(), ImGui::CollapsingHeader*()")    // imgui.h:1075
    ...
    py::enum_<ImGuiSelectableFlags_>(m, "SelectableFlags_", py::arithmetic(), "Flags for ImGui::Selectable()")    // imgui.h:1120
    ...
    py::enum_<ImGuiComboFlags_>(m, "ComboFlags_", py::arithmetic(), "Flags for ImGui::BeginCombo()")    // imgui.h:1131
    ...
    py::enum_<ImGuiTabBarFlags_>(m, "TabBarFlags_", py::arithmetic(), "Flags for ImGui::BeginTabBar()")    // imgui.h:1145
    ...
    py::enum_<ImGuiTabItemFlags_>(m, "TabItemFlags_", py::arithmetic(), "Flags for ImGui::BeginTabItem()")    // imgui.h:1161
    ...
    py::enum_<ImGuiTableColumnFlags_>(m, "TableColumnFlags_", py::arithmetic(), "Flags for ImGui::TableSetupColumn()")    // imgui.h:1247
    ...
    py::enum_<ImGuiTableRowFlags_>(m, "TableRowFlags_", py::arithmetic(), "Flags for ImGui::TableNextRow()")    // imgui.h:1284
    ...
    py::enum_<ImGuiFocusedFlags_>(m, "FocusedFlags_", py::arithmetic(), "Flags for ImGui::IsWindowFocused()")    // imgui.h:1308
    ...
    py::enum_<ImGuiDragDropFlags_>(m, "DragDropFlags_", py::arithmetic(), "Flags for ImGui::BeginDragDropSource(), ImGui::AcceptDragDropPayload()")    // imgui.h:1361
    ...
    py::enum_<ImGuiDataType_>(m, "DataType_", py::arithmetic(), "A primary data type")    // imgui.h:1383
    ...
    py::enum_<ImGuiDir_>(m, "Dir_", py::arithmetic(), "A cardinal direction")    // imgui.h:1399
    ...
    py::enum_<ImGuiSortDirection_>(m, "SortDirection_", py::arithmetic(), "A sorting direction")    // imgui.h:1410
    ...
    py::enum_<ImGuiKey>(m, "Key", py::arithmetic(), " A key identifier (ImGuiKey_XXX or ImGuiMod_XXX value)\n All our named keys are >= 512. Keys value 0 to 511 are left unused as legacy native/opaque key values (< 1.87)\n Since >= 1.89 we increased typing (went from int to enum), some legacy code may need a cast to ImGuiKey.")    // imgui.h:1420
    ...
    py::enum_<ImGuiConfigFlags_>(m, "ConfigFlags_", py::arithmetic(), "Configuration flags stored in io.ConfigFlags. Set by user/application.")    // imgui.h:1561
    ...
    py::enum_<ImGuiBackendFlags_>(m, "BackendFlags_", py::arithmetic(), "Backend capabilities flags stored in io.BackendFlags. Set by imgui_impl_xxx or custom backend.")    // imgui.h:1586
    ...
    py::enum_<ImGuiCol_>(m, "Col_", py::arithmetic(), "Enumeration for PushStyleColor() / PopStyleColor()")    // imgui.h:1601
    ...
    py::enum_<ImGuiButtonFlags_>(m, "ButtonFlags_", py::arithmetic(), "Flags for InvisibleButton() [extended in imgui_internal.h]")    // imgui.h:1700
    ...
    py::enum_<ImGuiColorEditFlags_>(m, "ColorEditFlags_", py::arithmetic(), "Flags for ColorEdit3() / ColorEdit4() / ColorPicker3() / ColorPicker4() / ColorButton()")    // imgui.h:1713
    ...
    py::enum_<ImGuiMouseButton_>(m, "MouseButton_", py::arithmetic(), " Identify a mouse button.\n Those values are guaranteed to be stable and we frequently use 0/1 directly. Named enums provided for convenience.")    // imgui.h:1776
    ...
    py::enum_<ImGuiMouseCursor_>(m, "MouseCursor_", py::arithmetic(), " Enumeration for GetMouseCursor()\n User code may request backend to display given cursor by calling SetMouseCursor(), which is why we have some cursors that are marked unused here")    // imgui.h:1786
    ...
    py::enum_<ImGuiCond_>(m, "Cond_", py::arithmetic(), " Enumeration for ImGui::SetWindow***(), SetNextWindow***(), SetNextItem***() functions\n Represent a condition.\n Important: Treat as a regular enum! Do NOT combine multiple values using binary operators! All the functions above treat 0 as a shortcut to ImGuiCond_Always.")    // imgui.h:1804
    ...
    auto pyClassImNewWrapper =
        py::class_<ImNewWrapper>    // imgui.h:1823
            (m, "ImNewWrapper", "")
    ...
    auto pyClassImGuiStyle =
        py::class_<ImGuiStyle>    // imgui.h:1916
            (m, "Style", "")
        .def_readwrite("alpha", &ImGuiStyle::Alpha, "Global alpha applies to everything in Dear ImGui.")    // imgui.h:1918
        .def_readwrite("disabled_alpha", &ImGuiStyle::DisabledAlpha, "Additional alpha multiplier applied by BeginDisabled(). Multiply over current value of Alpha.")    // imgui.h:1919
        .def_readwrite("window_padding", &ImGuiStyle::WindowPadding, "Padding within a window.")    // imgui.h:1920
        .def_readwrite("window_rounding", &ImGuiStyle::WindowRounding, "Radius of window corners rounding. Set to 0.0 to have rectangular windows. Large values tend to lead to variety of artifacts and are not recommended.")    // imgui.h:1921
        .def_readwrite("window_border_size", &ImGuiStyle::WindowBorderSize, "Thickness of border around windows. Generally set to 0.0 or 1.0. (Other values are not well tested and more CPU/GPU costly).")    // imgui.h:1922
        .def_readwrite("window_min_size", &ImGuiStyle::WindowMinSize, "Minimum window size. This is a global setting. If you want to constrain individual windows, use SetNextWindowSizeConstraints().")    // imgui.h:1923
        .def_readwrite("window_title_align", &ImGuiStyle::WindowTitleAlign, "Alignment for title bar text. Defaults to (0.0,0.5) for left-aligned,vertically centered.")    // imgui.h:1924
        .def_readwrite("window_menu_button_position", &ImGuiStyle::WindowMenuButtonPosition, "Side of the collapsing/docking button in the title bar (None/Left/Right). Defaults to ImGuiDir_Left.")    // imgui.h:1925
        .def_readwrite("child_rounding", &ImGuiStyle::ChildRounding, "Radius of child window corners rounding. Set to 0.0 to have rectangular windows.")    // imgui.h:1926
        .def_readwrite("child_border_size", &ImGuiStyle::ChildBorderSize, "Thickness of border around child windows. Generally set to 0.0 or 1.0. (Other values are not well tested and more CPU/GPU costly).")    // imgui.h:1927
        .def_readwrite("popup_rounding", &ImGuiStyle::PopupRounding, "Radius of popup window corners rounding. (Note that tooltip windows use WindowRounding)")    // imgui.h:1928
        .def_readwrite("popup_border_size", &ImGuiStyle::PopupBorderSize, "Thickness of border around popup/tooltip windows. Generally set to 0.0 or 1.0. (Other values are not well tested and more CPU/GPU costly).")    // imgui.h:1929
        .def_readwrite("frame_padding", &ImGuiStyle::FramePadding, "Padding within a framed rectangle (used by most widgets).")    // imgui.h:1930
        .def_readwrite("frame_rounding", &ImGuiStyle::FrameRounding, "Radius of frame corners rounding. Set to 0.0 to have rectangular frame (used by most widgets).")    // imgui.h:1931
        .def_readwrite("frame_border_size", &ImGuiStyle::FrameBorderSize, "Thickness of border around frames. Generally set to 0.0 or 1.0. (Other values are not well tested and more CPU/GPU costly).")    // imgui.h:1932
        .def_readwrite("item_spacing", &ImGuiStyle::ItemSpacing, "Horizontal and vertical spacing between widgets/lines.")    // imgui.h:1933
        .def_readwrite("item_inner_spacing", &ImGuiStyle::ItemInnerSpacing, "Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label).")    // imgui.h:1934
        .def_readwrite("cell_padding", &ImGuiStyle::CellPadding, "Padding within a table cell")    // imgui.h:1935
        .def_readwrite("touch_extra_padding", &ImGuiStyle::TouchExtraPadding, "Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!")    // imgui.h:1936
        .def_readwrite("indent_spacing", &ImGuiStyle::IndentSpacing, "Horizontal indentation when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).")    // imgui.h:1937
        .def_readwrite("columns_min_spacing", &ImGuiStyle::ColumnsMinSpacing, "Minimum horizontal spacing between two columns. Preferably > (FramePadding.x + 1).")    // imgui.h:1938
        .def_readwrite("scrollbar_size", &ImGuiStyle::ScrollbarSize, "Width of the vertical scrollbar, Height of the horizontal scrollbar.")    // imgui.h:1939
        .def_readwrite("scrollbar_rounding", &ImGuiStyle::ScrollbarRounding, "Radius of grab corners for scrollbar.")    // imgui.h:1940
        .def_readwrite("grab_min_size", &ImGuiStyle::GrabMinSize, "Minimum width/height of a grab box for slider/scrollbar.")    // imgui.h:1941
        .def_readwrite("grab_rounding", &ImGuiStyle::GrabRounding, "Radius of grabs corners rounding. Set to 0.0 to have rectangular slider grabs.")    // imgui.h:1942
        .def_readwrite("log_slider_deadzone", &ImGuiStyle::LogSliderDeadzone, "The size in pixels of the dead-zone around zero on logarithmic sliders that cross zero.")    // imgui.h:1943
        .def_readwrite("tab_rounding", &ImGuiStyle::TabRounding, "Radius of upper corners of a tab. Set to 0.0 to have rectangular tabs.")    // imgui.h:1944
        .def_readwrite("tab_border_size", &ImGuiStyle::TabBorderSize, "Thickness of border around tabs.")    // imgui.h:1945
        .def_readwrite("tab_min_width_for_close_button", &ImGuiStyle::TabMinWidthForCloseButton, "Minimum width for close button to appear on an unselected tab when hovered. Set to 0.0 to always show when hovering, set to FLT_MAX to never show close button unless selected.")    // imgui.h:1946
        .def_readwrite("color_button_position", &ImGuiStyle::ColorButtonPosition, "Side of the color button in the ColorEdit4 widget (left/right). Defaults to ImGuiDir_Right.")    // imgui.h:1947
        .def_readwrite("button_text_align", &ImGuiStyle::ButtonTextAlign, "Alignment of button text when button is larger than text. Defaults to (0.5, 0.5) (centered).")    // imgui.h:1948
        .def_readwrite("selectable_text_align", &ImGuiStyle::SelectableTextAlign, "Alignment of selectable text. Defaults to (0.0, 0.0) (top-left aligned). It's generally important to keep this left-aligned if you want to lay multiple items on a same line.")    // imgui.h:1949
        .def_readwrite("display_window_padding", &ImGuiStyle::DisplayWindowPadding, "Window position are clamped to be visible within the display area or monitors by at least this amount. Only applies to regular windows.")    // imgui.h:1950
        .def_readwrite("display_safe_area_padding", &ImGuiStyle::DisplaySafeAreaPadding, "If you cannot see the edges of your screen (e.g. on a TV) increase the safe area padding. Apply to popups/tooltips as well regular windows. NB: Prefer configuring your TV sets correctly!")    // imgui.h:1951
        .def_readwrite("mouse_cursor_scale", &ImGuiStyle::MouseCursorScale, "Scale software rendered mouse cursor (when io.MouseDrawCursor is enabled). We apply per-monitor DPI scaling over this scale. May be removed later.")    // imgui.h:1952
        .def_readwrite("anti_aliased_lines", &ImGuiStyle::AntiAliasedLines, "Enable anti-aliased lines/borders. Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).")    // imgui.h:1953
        .def_readwrite("anti_aliased_lines_use_tex", &ImGuiStyle::AntiAliasedLinesUseTex, "Enable anti-aliased lines/borders using textures where possible. Require backend to render with bilinear filtering (NOT point/nearest filtering). Latched at the beginning of the frame (copied to ImDrawList).")    // imgui.h:1954
        .def_readwrite("anti_aliased_fill", &ImGuiStyle::AntiAliasedFill, "Enable anti-aliased edges around filled shapes (rounded rectangles, circles, etc.). Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).")    // imgui.h:1955
        .def_readwrite("curve_tessellation_tol", &ImGuiStyle::CurveTessellationTol, "Tessellation tolerance when using PathBezierCurveTo() without a specific number of segments. Decrease for highly tessellated curves (higher quality, more polygons), increase to reduce quality.")    // imgui.h:1956
        .def_readwrite("circle_tessellation_max_error", &ImGuiStyle::CircleTessellationMaxError, "Maximum error (in pixels) allowed when using AddCircle()/AddCircleFilled() or drawing rounded corner rectangles with no explicit segment count specified. Decrease for higher quality but more geometry.")    // imgui.h:1957
        .def(py::init<>())    // imgui.h:1960
        .def("scale_all_sizes",    // imgui.h:1961
            &ImGuiStyle::ScaleAllSizes, py::arg("scale_factor"))
        ;


    auto pyClassImGuiKeyData =
        py::class_<ImGuiKeyData>    // imgui.h:1973
        ...
        .def_readwrite("down", &ImGuiKeyData::Down, "True for if key is down")    // imgui.h:1975
        .def_readwrite("down_duration", &ImGuiKeyData::DownDuration, "Duration the key has been down (<0.0: not pressed, 0.0: just pressed, >0.0: time held)")    // imgui.h:1976
        .def_readwrite("down_duration_prev", &ImGuiKeyData::DownDurationPrev, "Last frame duration the key has been down")    // imgui.h:1977
        .def_readwrite("analog_value", &ImGuiKeyData::AnalogValue, "0.0..1.0 for gamepad values")    // imgui.h:1978
        ;


    auto pyClassImGuiIO =
        py::class_<ImGuiIO>    // imgui.h:1981
            (m, "IO", "")
        .def_readwrite("config_flags", &ImGuiIO::ConfigFlags, "= 0              // See ImGuiConfigFlags_ enum. Set by user/application. Gamepad/keyboard navigation options, etc.")    // imgui.h:1987
        .def_readwrite("backend_flags", &ImGuiIO::BackendFlags, "= 0              // See ImGuiBackendFlags_ enum. Set by backend (imgui_impl_xxx files or custom backend) to communicate features supported by the backend.")    // imgui.h:1988
        .def_readwrite("display_size", &ImGuiIO::DisplaySize, "<unset>          // Main display size, in pixels (generally == GetMainViewport()->Size). May change every frame.")    // imgui.h:1989
        .def_readwrite("delta_time", &ImGuiIO::DeltaTime, "= 1.0/60.0     // Time elapsed since last frame, in seconds. May change every frame.")    // imgui.h:1990
        .def_readwrite("ini_saving_rate", &ImGuiIO::IniSavingRate, "= 5.0           // Minimum time between saving positions/sizes to .ini file, in seconds.")    // imgui.h:1991
        .def_readonly("ini_filename", &ImGuiIO::IniFilename, "= \"imgui.ini\"    // Path to .ini file (important: default \"imgui.ini\" is relative to current working dir!). Set None to disable automatic .ini loading/saving or if you want to manually call LoadIniSettingsXXX() / SaveIniSettingsXXX() functions.")    // imgui.h:1992
        .def_readonly("log_filename", &ImGuiIO::LogFilename, "= \"imgui_log.txt\"// Path to .log file (default parameter to ImGui::LogToFile when no file is specified).")    // imgui.h:1993
        .def_readwrite("mouse_double_click_time", &ImGuiIO::MouseDoubleClickTime, "= 0.30          // Time for a double-click, in seconds.")    // imgui.h:1994
        .def_readwrite("mouse_double_click_max_dist", &ImGuiIO::MouseDoubleClickMaxDist, "= 6.0           // Distance threshold to stay in to validate a double-click, in pixels.")    // imgui.h:1995
        .def_readwrite("mouse_drag_threshold", &ImGuiIO::MouseDragThreshold, "= 6.0           // Distance threshold before considering we are dragging.")    // imgui.h:1996
        .def_readwrite("key_repeat_delay", &ImGuiIO::KeyRepeatDelay, "= 0.275         // When holding a key/button, time before it starts repeating, in seconds (for buttons in Repeat mode, etc.).")    // imgui.h:1997
        .def_readwrite("key_repeat_rate", &ImGuiIO::KeyRepeatRate, "= 0.050         // When holding a key/button, rate at which it repeats, in seconds.")    // imgui.h:1998
        .def_readwrite("hover_delay_normal", &ImGuiIO::HoverDelayNormal, "= 0.30 sec       // Delay on hovering before IsItemHovered(ImGuiHoveredFlags_DelayNormal) returns True.")    // imgui.h:1999
        .def_readwrite("hover_delay_short", &ImGuiIO::HoverDelayShort, "= 0.10 sec       // Delay on hovering before IsItemHovered(ImGuiHoveredFlags_DelayShort) returns True.")    // imgui.h:2000
        ...
        .def_readwrite("config_input_text_enter_keep_active", &ImGuiIO::ConfigInputTextEnterKeepActive, "= False          // [BETA] Pressing Enter will keep item active and select contents (single-line only).")    // imgui.h:2026
        .def_readwrite("config_drag_click_to_input_text", &ImGuiIO::ConfigDragClickToInputText, "= False          // [BETA] Enable turning DragXXX widgets into text input with a simple mouse click-release (without moving). Not desirable on devices without a keyboard.")    // imgui.h:2027
        ...
        .def_readwrite("config_windows_move_from_title_bar_only", &ImGuiIO::ConfigWindowsMoveFromTitleBarOnly, "= False       // Enable allowing to move windows only when clicking on their title bar. Does not apply to windows without a title bar.")    // imgui.h:2029
        .def_readwrite("config_memory_compact_timer", &ImGuiIO::ConfigMemoryCompactTimer, "= 60.0          // Timer (in seconds) to free transient windows/tables memory buffers when unused. Set to -1.0 to disable.")    // imgui.h:2030
        .def_readonly("backend_platform_name", &ImGuiIO::BackendPlatformName, "= None")    // imgui.h:2038
        .def_readonly("backend_renderer_name", &ImGuiIO::BackendRendererName, "= None")    // imgui.h:2039
        .def_readwrite("backend_platform_user_data", &ImGuiIO::BackendPlatformUserData, "= None           // User data for platform backend")    // imgui.h:2040
        .def_readwrite("backend_renderer_user_data", &ImGuiIO::BackendRendererUserData, "= None           // User data for renderer backend")    // imgui.h:2041
        .def_readwrite("backend_language_user_data", &ImGuiIO::BackendLanguageUserData, "= None           // User data for non C++ programming language backend")    // imgui.h:2042
        .def_readwrite("clipboard_user_data", &ImGuiIO::ClipboardUserData, "")    // imgui.h:2048
        .def("add_key_event",    // imgui.h:2064
            &ImGuiIO::AddKeyEvent,
            py::arg("key"), py::arg("down"),
            "Queue a new key down/up event. Key should be \"translated\" (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)")
        .def("add_key_analog_event",    // imgui.h:2065
            &ImGuiIO::AddKeyAnalogEvent,
            py::arg("key"), py::arg("down"), py::arg("v"),
            "Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.")
        .def("add_mouse_pos_event",    // imgui.h:2066
            &ImGuiIO::AddMousePosEvent,
            py::arg("x"), py::arg("y"),
            "Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. app not focused and not hovered)")
        .def("add_mouse_button_event",    // imgui.h:2067
            &ImGuiIO::AddMouseButtonEvent,
            py::arg("button"), py::arg("down"),
            "Queue a mouse button change")
        .def("add_mouse_wheel_event",    // imgui.h:2068
            &ImGuiIO::AddMouseWheelEvent,
            py::arg("wh_x"), py::arg("wh_y"),
            "Queue a mouse wheel update")
        .def("add_mouse_viewport_event",    // imgui.h:2069
            &ImGuiIO::AddMouseViewportEvent,
            py::arg("id_"),
            "Queue a mouse hovered viewport. Requires backend to set ImGuiBackendFlags_HasMouseHoveredViewport to call this (for multi-viewport support).")
        .def("add_focus_event",    // imgui.h:2070
            &ImGuiIO::AddFocusEvent,
            py::arg("focused"),
            "Queue a gain/loss of focus for the application (generally based on OS/platform focus of your window)")
        .def("add_input_character",    // imgui.h:2071
            &ImGuiIO::AddInputCharacter,
            py::arg("c"),
            "Queue a new character input")
        .def("add_input_character_utf16",    // imgui.h:2072
            &ImGuiIO::AddInputCharacterUTF16,
            py::arg("c"),
            "Queue a new character input from a UTF-16 character, it can be a surrogate")
        .def("add_input_characters_utf8",    // imgui.h:2073
            &ImGuiIO::AddInputCharactersUTF8,
            py::arg("str"),
            "Queue a new characters input from a UTF-8 string")
        .def("set_key_event_native_data",    // imgui.h:2075
        ...
        .def_readwrite("want_capture_keyboard", &ImGuiIO::WantCaptureKeyboard, "Set when Dear ImGui will use keyboard inputs, in this case do not dispatch them to your main game/application (either way, always pass keyboard inputs to imgui). (e.g. InputText active, or an imgui window is focused and navigation is enabled, etc.).")    // imgui.h:2087
        .def_readwrite("want_text_input", &ImGuiIO::WantTextInput, "Mobile/console: when set, you may display an on-screen keyboard. This is set by Dear ImGui when it wants textual keyboard input to happen (e.g. when a InputText widget is active).")    // imgui.h:2088
        .def_readwrite("want_set_mouse_pos", &ImGuiIO::WantSetMousePos, "MousePos has been altered, backend should reposition mouse on next frame. Rarely used! Set only when ImGuiConfigFlags_NavEnableSetMousePos flag is enabled.")    // imgui.h:2089
        .def_readwrite("want_save_ini_settings", &ImGuiIO::WantSaveIniSettings, "When manual .ini load/save is active (io.IniFilename == None), this will be set to notify your application that you can call SaveIniSettingsToMemory() and save yourself. Important: clear io.WantSaveIniSettings yourself after saving!")    // imgui.h:2090
        .def_readwrite("nav_active", &ImGuiIO::NavActive, "Keyboard/Gamepad navigation is currently allowed (will handle ImGuiKey_NavXXX events) = a window is focused and it doesn't use the ImGuiWindowFlags_NoNavInputs flag.")    // imgui.h:2091
        .def_readwrite("nav_visible", &ImGuiIO::NavVisible, "Keyboard/Gamepad navigation is visible and allowed (will handle ImGuiKey_NavXXX events).")    // imgui.h:2092
        .def_readwrite("framerate", &ImGuiIO::Framerate, "Estimate of application framerate (rolling average over 60 frames, based on io.DeltaTime), in frame per second. Solely for convenience. Slow applications may not want to use a moving average or may want to reset underlying buffers occasionally.")    // imgui.h:2093
        .def_readwrite("metrics_render_vertices", &ImGuiIO::MetricsRenderVertices, "Vertices output during last call to Render()")    // imgui.h:2094
        .def_readwrite("metrics_render_indices", &ImGuiIO::MetricsRenderIndices, "Indices output during last call to Render() = number of triangles * 3")    // imgui.h:2095
        .def_readwrite("metrics_render_windows", &ImGuiIO::MetricsRenderWindows, "Number of visible windows")    // imgui.h:2096
        .def_readwrite("metrics_active_windows", &ImGuiIO::MetricsActiveWindows, "Number of active windows")    // imgui.h:2097
        .def_readwrite("metrics_active_allocations", &ImGuiIO::MetricsActiveAllocations, "Number of active allocations, updated by MemAlloc/MemFree based on current context. May be off if you have multiple imgui contexts.")    // imgui.h:2098
        .def_readwrite("mouse_delta", &ImGuiIO::MouseDelta, "")    // imgui.h:2099
        .def_readwrite("mouse_pos", &ImGuiIO::MousePos, "Mouse position, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX) if mouse is unavailable (on another screen, etc.)")    // imgui.h:2117
        .def_property("mouse_down",    // imgui.h:2118
        ...
        .def_readwrite("mouse_wheel", &ImGuiIO::MouseWheel, "Mouse wheel Vertical: 1 unit scrolls about 5 lines text.")    // imgui.h:2119
        .def_readwrite("mouse_wheel_h", &ImGuiIO::MouseWheelH, "Mouse wheel Horizontal. Most users don't have a mouse with a horizontal wheel, may not be filled by all backends.")    // imgui.h:2120
        ...
        .def_readwrite("key_ctrl", &ImGuiIO::KeyCtrl, "Keyboard modifier down: Control")    // imgui.h:2122
        .def_readwrite("key_shift", &ImGuiIO::KeyShift, "Keyboard modifier down: Shift")    // imgui.h:2123
        .def_readwrite("key_alt", &ImGuiIO::KeyAlt, "Keyboard modifier down: Alt")    // imgui.h:2124
        .def_readwrite("key_super", &ImGuiIO::KeySuper, "Keyboard modifier down: Cmd/Super/Windows")    // imgui.h:2125
        .def_readwrite("key_mods", &ImGuiIO::KeyMods, "Key mods flags (any of ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super flags, same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags). Read-only, updated by NewFrame()")    // imgui.h:2128
        .def_readwrite("want_capture_mouse_unless_popup_close", &ImGuiIO::WantCaptureMouseUnlessPopupClose, "Alternative to WantCaptureMouse: (WantCaptureMouse == True && WantCaptureMouseUnlessPopupClose == False) when a click over None is expected to close a popup.")    // imgui.h:2130
        .def_readwrite("mouse_pos_prev", &ImGuiIO::MousePosPrev, "Previous mouse position (note that MouseDelta is not necessary == MousePos-MousePosPrev, in case either position is invalid)")    // imgui.h:2131
        .def_property("mouse_clicked_time",    // imgui.h:2133
            [](ImGuiIO &self) -> pybind11::array
            {
        ...
        .def_property("mouse_clicked",    // imgui.h:2134
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_double_clicked",    // imgui.h:2135
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_clicked_count",    // imgui.h:2136
            [](ImGuiIO &self) -> pybind11::array
            {
        ...
        .def_property("mouse_clicked_last_count",    // imgui.h:2137
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<ImU16>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(ImU16)});
        ...
        .def_property("mouse_released",    // imgui.h:2138
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_down_owned",    // imgui.h:2139
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_down_owned_unless_popup_close",    // imgui.h:2140
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_down_duration",    // imgui.h:2141
            [](ImGuiIO &self) -> pybind11::array
            {
        ...
        .def_property("mouse_down_duration_prev",    // imgui.h:2142
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<float>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(float)});
        ...
        .def_property("mouse_drag_max_distance_sqr",    // imgui.h:2144
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<float>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(float)});
    ...
    auto pyClassImGuiSizeCallbackData =
        ...
        .def_readwrite("user_data", &ImGuiSizeCallbackData::UserData, "Read-only.   What user passed to SetNextWindowSizeConstraints(). Generally store an integer or float in here (need reinterpret_cast<>).")    // imgui.h:2202
    ...
    auto pyClassImDrawList =
        py::class_<ImDrawList>    // imgui.h:2619
        ...
        .def_readwrite("_data", &ImDrawList::_Data, "Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui context)")    // imgui.h:2629
        ...
        .def(py::init<ImDrawListSharedData *>(),    // imgui.h:2641
            py::arg("shared_data"),
    ...
    auto pyClassImFontAtlas =
        ...
        .def("get_glyph_ranges_default",    // imgui.h:2897
            ...
            pybind11::return_value_policy::reference)
        .def("get_glyph_ranges_greek",    // imgui.h:2898
            &ImFontAtlas::GetGlyphRangesGreek,
            "Default + Greek and Coptic",
            pybind11::return_value_policy::reference)
        .def("get_glyph_ranges_korean",    // imgui.h:2899
            &ImFontAtlas::GetGlyphRangesKorean,
            "Default + Korean characters",
            pybind11::return_value_policy::reference)
        .def("get_glyph_ranges_japanese",    // imgui.h:2900
            &ImFontAtlas::GetGlyphRangesJapanese,
            "Default + Hiragana, Katakana, Half-Width, Selection of 2999 Ideographs",
            pybind11::return_value_policy::reference)
        .def("get_glyph_ranges_chinese_full",    // imgui.h:2901
            &ImFontAtlas::GetGlyphRangesChineseFull,
            "Default + Half-Width + Japanese Hiragana/Katakana + full set of about 21000 CJK Unified Ideographs",
            pybind11::return_value_policy::reference)
        .def("get_glyph_ranges_chinese_simplified_common",    // imgui.h:2902
            &ImFontAtlas::GetGlyphRangesChineseSimplifiedCommon,
            "Default + Half-Width + Japanese Hiragana/Katakana + set of 2500 CJK Unified Ideographs for common simplified Chinese",
            pybind11::return_value_policy::reference)
        .def("get_glyph_ranges_cyrillic",    // imgui.h:2903
            &ImFontAtlas::GetGlyphRangesCyrillic,
            "Default + about 400 Cyrillic characters",
            pybind11::return_value_policy::reference)
        .def("get_glyph_ranges_thai",    // imgui.h:2904
            &ImFontAtlas::GetGlyphRangesThai,
            "Default + Thai characters",
            pybind11::return_value_policy::reference)
        .def("get_glyph_ranges_vietnamese",    // imgui.h:2905
            &ImFontAtlas::GetGlyphRangesVietnamese,
            "Default + Vietnamese characters",
            pybind11::return_value_policy::reference)
        .def("add_custom_rect_regular",    // imgui.h:2918
            &ImFontAtlas::AddCustomRectRegular, py::arg("width"), py::arg("height"))
        .def("add_custom_rect_font_glyph",    // imgui.h:2919
        ...
        .def("calc_custom_rect_uv",    // imgui.h:2923
            &ImFontAtlas::CalcCustomRectUV, py::arg("rect"), py::arg("out_uv_min"), py::arg("out_uv_max"))
        .def("get_mouse_cursor_tex_data",    // imgui.h:2924
        ...
        .def_readwrite("flags", &ImFontAtlas::Flags, "Build flags (see ImFontAtlasFlags_)")    // imgui.h:2930
        .def_readwrite("tex_id", &ImFontAtlas::TexID, "User data to refer to the texture once it has been uploaded to user's graphic systems. It is passed back to you during rendering via the ImDrawCmd structure.")    // imgui.h:2931
        .def_readwrite("tex_desired_width", &ImFontAtlas::TexDesiredWidth, "Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics API have texture size restrictions you may want to increase texture width to decrease height.")    // imgui.h:2932
        .def_readwrite("tex_glyph_padding", &ImFontAtlas::TexGlyphPadding, "Padding between glyphs within texture in pixels. Defaults to 1. If your rendering method doesn't rely on bilinear filtering you may set this to 0 (will also need to set AntiAliasedLinesUseTex = False).")    // imgui.h:2933
        .def_readwrite("locked", &ImFontAtlas::Locked, "Marked as Locked by ImGui::NewFrame() so attempt to modify the atlas will assert.")    // imgui.h:2934
        .def_readwrite("tex_ready", &ImFontAtlas::TexReady, "Set when texture was built matching current font input")    // imgui.h:2938
        .def_readwrite("tex_pixels_use_colors", &ImFontAtlas::TexPixelsUseColors, "Tell whether our texture data is known to use colors (rather than just alpha channel), in order to help backend select a format.")    // imgui.h:2939
        .def_readwrite("tex_width", &ImFontAtlas::TexWidth, "Texture width calculated during Build().")    // imgui.h:2942
        .def_readwrite("tex_height", &ImFontAtlas::TexHeight, "Texture height calculated during Build().")    // imgui.h:2943
        .def_readwrite("tex_uv_scale", &ImFontAtlas::TexUvScale, "= (1.0/TexWidth, 1.0/TexHeight)")    // imgui.h:2944
        .def_readwrite("tex_uv_white_pixel", &ImFontAtlas::TexUvWhitePixel, "Texture coordinates to a white pixel")    // imgui.h:2945
        .def_readonly("font_builder_io", &ImFontAtlas::FontBuilderIO, "Opaque interface to a font builder (default to stb_truetype, can be changed to use FreeType by defining IMGUI_ENABLE_FREETYPE).")    // imgui.h:2952
        .def_readwrite("font_builder_flags", &ImFontAtlas::FontBuilderFlags, "Shared flags (for all fonts) for custom font builder. THIS IS BUILD IMPLEMENTATION DEPENDENT. Per-font override is also available in ImFontConfig.")    // imgui.h:2953
        .def_readwrite("pack_id_mouse_cursors", &ImFontAtlas::PackIdMouseCursors, "Custom texture rectangle ID for white pixel and mouse cursors")    // imgui.h:2956
        .def_readwrite("pack_id_lines", &ImFontAtlas::PackIdLines, "Custom texture rectangle ID for baked anti-aliased lines")    // imgui.h:2957
        ;


    auto pyClassImFont =
        py::class_<ImFont>    // imgui.h:2966
            (m, "ImFont", " Font runtime data and rendering\n ImFontAtlas automatically loads a default embedded font for you when you call GetTexDataAsAlpha8() or GetTexDataAsRGBA32().")
        .def_readwrite("fallback_advance_x", &ImFont::FallbackAdvanceX, "4     // out // = FallbackGlyph->AdvanceX")    // imgui.h:2970
        .def_readwrite("font_size", &ImFont::FontSize, "4     // in  //            // Height of characters/line, set during loading (don't change after loading)")    // imgui.h:2971
        .def_readonly("fallback_glyph", &ImFont::FallbackGlyph, "4-8   // out // = FindGlyph(FontFallbackChar)")    // imgui.h:2976
        .def_readwrite("container_atlas", &ImFont::ContainerAtlas, "4-8   // out //            // What we has been loaded into")    // imgui.h:2979
        .def_readonly("config_data", &ImFont::ConfigData, "4-8   // in  //            // Pointer within ContainerAtlas->ConfigData")    // imgui.h:2980
        .def_readwrite("config_data_count", &ImFont::ConfigDataCount, "2     // in  // ~ 1        // Number of ImFontConfig involved in creating this font. Bigger than 1 when merging multiple font sources into one ImFont.")    // imgui.h:2981
        .def_readwrite("fallback_char", &ImFont::FallbackChar, "2     // out // = FFFD/'?' // Character used if a glyph isn't found.")    // imgui.h:2982
        .def_readwrite("ellipsis_char", &ImFont::EllipsisChar, "2     // out // = '...'    // Character used for ellipsis rendering.")    // imgui.h:2983
        .def_readwrite("dot_char", &ImFont::DotChar, "2     // out // = '.'      // Character used for ellipsis rendering (if a single '...' character isn't found)")    // imgui.h:2984
        .def_readwrite("dirty_lookup_tables", &ImFont::DirtyLookupTables, "1     // out //")    // imgui.h:2985
        .def_readwrite("scale", &ImFont::Scale, "4     // in  // = 1.      // Base font scale, multiplied by the per-window font scale which you can adjust with SetWindowFontScale()")    // imgui.h:2986
        .def_readwrite("ascent", &ImFont::Ascent, "4+4   // out //            // Ascent: distance from top to bottom of e.g. 'A' [0..FontSize]")    // imgui.h:2987
        .def_readwrite("descent", &ImFont::Descent, "4+4   // out //            // Ascent: distance from top to bottom of e.g. 'A' [0..FontSize]")    // imgui.h:2987
        .def_readwrite("metrics_total_surface", &ImFont::MetricsTotalSurface, "4     // out //            // Total surface in pixels to get an idea of the font rasterization/texture cost (not exact, we approximate the cost of padding between glyphs)")    // imgui.h:2988
        .def(py::init<>(),    // imgui.h:2992
            "Methods")
        .def("find_glyph",    // imgui.h:2994
            &ImFont::FindGlyph,
            py::arg("c"),
            pybind11::return_value_policy::reference)
        .def("find_glyph_no_fallback",    // imgui.h:2995
            &ImFont::FindGlyphNoFallback,
            py::arg("c"),
            pybind11::return_value_policy::reference)
        .def("calc_word_wrap_position_a",    // imgui.h:3003
            &ImFont::CalcWordWrapPositionA,
            py::arg("scale"), py::arg("text"), py::arg("text_end"), py::arg("wrap_width"),
            pybind11::return_value_policy::reference)
        .def("render_char",    // imgui.h:3004
            &ImFont::RenderChar, py::arg("draw_list"), py::arg("size"), py::arg("pos"), py::arg("col"), py::arg("c"))
        .def("render_text",    // imgui.h:3005
        ...
        .def("build_lookup_table",    // imgui.h:3008
            &ImFont::BuildLookupTable)
        .def("clear_output_data",    // imgui.h:3009
            &ImFont::ClearOutputData)
        .def("grow_index",    // imgui.h:3010
            &ImFont::GrowIndex, py::arg("new_size"))
        .def("add_glyph",    // imgui.h:3011
        ...
        .def("add_remap_char",    // imgui.h:3012
            &ImFont::AddRemapChar,
            py::arg("dst"), py::arg("src"), py::arg("overwrite_dst") = true,
            "Makes 'dst' character/glyph points to 'src' character/glyph. Currently needs to be called AFTER fonts have been built.")
        .def("set_glyph_visible",    // imgui.h:3013
            &ImFont::SetGlyphVisible, py::arg("c"), py::arg("visible"))
        .def("is_glyph_range_unused",    // imgui.h:3014
            &ImFont::IsGlyphRangeUnused, py::arg("c_begin"), py::arg("c_last"))
        ;


    py::enum_<ImGuiViewportFlags_>(m, "ViewportFlags_", py::arithmetic(), "Flags stored in ImGuiViewport::Flags, giving indications to the platform backends.")    // imgui.h:3022
    ...
    auto pyClassImGuiViewport =
        py::class_<ImGuiViewport>    // imgui.h:3047
        ...
        .def_readwrite("id_", &ImGuiViewport::ID, "Unique identifier for the viewport")    // imgui.h:3049
        .def_readwrite("flags", &ImGuiViewport::Flags, "See ImGuiViewportFlags_")    // imgui.h:3050
        .def_readwrite("pos", &ImGuiViewport::Pos, "Main Area: Position of the viewport (Dear ImGui coordinates are the same as OS desktop/native coordinates)")    // imgui.h:3051
        .def_readwrite("size", &ImGuiViewport::Size, "Main Area: Size of the viewport.")    // imgui.h:3052
        .def_readwrite("work_pos", &ImGuiViewport::WorkPos, "Work Area: Position of the viewport minus task bars, menus bars, status bars (>= Pos)")    // imgui.h:3053
        .def_readwrite("work_size", &ImGuiViewport::WorkSize, "Work Area: Size of the viewport minus task bars, menu bars, status bars (<= Size)")    // imgui.h:3054
        .def_readwrite("dpi_scale", &ImGuiViewport::DpiScale, "1.0 = 96 DPI = No extra scale.")    // imgui.h:3055
        .def_readwrite("parent_viewport_id", &ImGuiViewport::ParentViewportId, "(Advanced) 0: no parent. Instruct the platform backend to setup a parent/child relationship between platform windows.")    // imgui.h:3056
        .def_readwrite("draw_data", &ImGuiViewport::DrawData, "The ImDrawData corresponding to this viewport. Valid after Render() and until the next call to NewFrame().")    // imgui.h:3057
        .def_readwrite("renderer_user_data", &ImGuiViewport::RendererUserData, "None* to hold custom data structure for the renderer (e.g. swap chain, framebuffers etc.). generally set by your Renderer_CreateWindow function.")    // imgui.h:3064
        .def_readwrite("platform_user_data", &ImGuiViewport::PlatformUserData, "None* to hold custom data structure for the OS / platform (e.g. windowing info, render context). generally set by your Platform_CreateWindow function.")    // imgui.h:3065
        .def_readwrite("platform_handle", &ImGuiViewport::PlatformHandle, "None* for FindViewportByPlatformHandle(). (e.g. suggested to use natural platform handle such as HWND, GLFWWindow*, SDL_Window*)")    // imgui.h:3066
        .def_readwrite("platform_handle_raw", &ImGuiViewport::PlatformHandleRaw, "None* to hold lower-level, platform-native window handle (under Win32 this is expected to be a HWND, unused for other platforms), when using an abstraction layer like GLFW or SDL (where PlatformHandle would be a SDL_Window*)")    // imgui.h:3067
        .def_readwrite("platform_window_created", &ImGuiViewport::PlatformWindowCreated, "Platform window has been created (Platform_CreateWindow() has been called). This is False during the first frame where a viewport is being created.")    // imgui.h:3068
        .def_readwrite("platform_request_move", &ImGuiViewport::PlatformRequestMove, "Platform window requested move (e.g. window was moved by the OS / host window manager, authoritative position will be OS window position)")    // imgui.h:3069
        .def_readwrite("platform_request_resize", &ImGuiViewport::PlatformRequestResize, "Platform window requested resize (e.g. window was resized by the OS / host window manager, authoritative size will be OS window size)")    // imgui.h:3070
        .def_readwrite("platform_request_close", &ImGuiViewport::PlatformRequestClose, "Platform window requested closure (e.g. window was moved by the OS / host window manager, e.g. pressing ALT-F4)")    // imgui.h:3071
        .def(py::init<>())    // imgui.h:3073
        ;


    auto pyClassImGuiPlatformIO =
        py::class_<ImGuiPlatformIO>    // imgui.h:3131
            (m, "PlatformIO", "(Optional) Access via ImGui::GetPlatformIO()")
        .def(py::init<>(),    // imgui.h:3189
            "Zero clear")
        ;


    auto pyClassImGuiPlatformMonitor =
        py::class_<ImGuiPlatformMonitor>    // imgui.h:3194
        ...
        .def_readwrite("main_pos", &ImGuiPlatformMonitor::MainPos, "Coordinates of the area displayed on this monitor (Min = upper left, Max = bottom right)")    // imgui.h:3196
        .def_readwrite("main_size", &ImGuiPlatformMonitor::MainSize, "Coordinates of the area displayed on this monitor (Min = upper left, Max = bottom right)")    // imgui.h:3196
        .def_readwrite("work_pos", &ImGuiPlatformMonitor::WorkPos, "Coordinates without task bars / side bars / menu bars. Used to avoid positioning popups/tooltips inside this region. If you don't have this info, please copy the value for MainPos/MainSize.")    // imgui.h:3197
        .def_readwrite("work_size", &ImGuiPlatformMonitor::WorkSize, "Coordinates without task bars / side bars / menu bars. Used to avoid positioning popups/tooltips inside this region. If you don't have this info, please copy the value for MainPos/MainSize.")    // imgui.h:3197
        .def_readwrite("dpi_scale", &ImGuiPlatformMonitor::DpiScale, "1.0 = 96 DPI")    // imgui.h:3198
        .def(py::init<>())    // imgui.h:3199
        ;


    auto pyClassImGuiPlatformImeData =
        py::class_<ImGuiPlatformImeData>    // imgui.h:3203
            (m, "PlatformImeData", "(Optional) Support for IME (Input Method Editor) via the io.SetPlatformImeDataFn() function.")
        .def_readwrite("want_visible", &ImGuiPlatformImeData::WantVisible, "A widget wants the IME to be visible")    // imgui.h:3205
        .def_readwrite("input_pos", &ImGuiPlatformImeData::InputPos, "Position of the input cursor")    // imgui.h:3206
        .def_readwrite("input_line_height", &ImGuiPlatformImeData::InputLineHeight, "Line height")    // imgui.h:3207
