...
void py_init_module_imgui_main(py::module& m)
{
    ...
    auto pyClassImVec2 =
        py::class_<ImVec2>    // imgui.h:263
            (m, "ImVec2", "")
        .def_readwrite("x", &ImVec2::x, "")    // imgui.h:265
        .def_readwrite("y", &ImVec2::y, "")    // imgui.h:265
        .def(py::init<>())    // imgui.h:266
        .def(py::init<float, float>(),    // imgui.h:267
            py::arg("_x"), py::arg("_y"))
        ;


    auto pyClassImVec4 =
        py::class_<ImVec4>    // imgui.h:276
            (m, "ImVec4", "ImVec4: 4D vector used to store clipping rectangles, colors etc. [Compile-time configurable type]")
        .def_readwrite("x", &ImVec4::x, "")    // imgui.h:278
        .def_readwrite("y", &ImVec4::y, "")    // imgui.h:278
        .def_readwrite("z", &ImVec4::z, "")    // imgui.h:278
        .def_readwrite("w", &ImVec4::w, "")    // imgui.h:278
        .def(py::init<>())    // imgui.h:279
        .def(py::init<float, float, float, float>(),    // imgui.h:280
            py::arg("_x"), py::arg("_y"), py::arg("_z"), py::arg("_w"))
        ;


    m.def("create_context",    // imgui.h:298
        ImGui::CreateContext,
        py::arg("shared_font_atlas") = py::none(),
        pybind11::return_value_policy::reference);

    m.def("destroy_context",    // imgui.h:299
        ImGui::DestroyContext,
        py::arg("ctx") = py::none(),
        "None = destroy current context");

    m.def("get_current_context",    // imgui.h:300
        ImGui::GetCurrentContext, pybind11::return_value_policy::reference);

    m.def("set_current_context",    // imgui.h:301
        ImGui::SetCurrentContext, py::arg("ctx"));

    m.def("get_io",    // imgui.h:304
        ImGui::GetIO,
        "access the IO structure (mouse/keyboard/gamepad inputs, time, various configuration options/flags)",
        pybind11::return_value_policy::reference);

    m.def("get_style",    // imgui.h:305
        ImGui::GetStyle,
        "access the Style structure (colors, sizes). Always use PushStyleCol(), PushStyleVar() to modify style mid-frame!",
        pybind11::return_value_policy::reference);

    m.def("new_frame",    // imgui.h:306
        ImGui::NewFrame, "start a new Dear ImGui frame, you can submit any command from this point until Render()/EndFrame().");

    m.def("end_frame",    // imgui.h:307
    ...
    m.def("show_metrics_window",    // imgui.h:313
        [](std::optional<bool> p_open = std::nullopt) -> std::optional<bool>
        {
    ...
    m.def("show_debug_log_window",    // imgui.h:314
        [](std::optional<bool> p_open = std::nullopt) -> std::optional<bool>
        {
    ...
    m.def("show_stack_tool_window",    // imgui.h:315
        [](std::optional<bool> p_open = std::nullopt) -> std::optional<bool>
        {
    ...
    m.def("show_about_window",    // imgui.h:316
        [](std::optional<bool> p_open = std::nullopt) -> std::optional<bool>
        {
    ...
    m.def("show_style_editor",    // imgui.h:317
        ImGui::ShowStyleEditor,
        py::arg("ref") = py::none(),
        "add style editor block (not a window). you can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)");

    m.def("show_style_selector",    // imgui.h:318
        ImGui::ShowStyleSelector,
        py::arg("label"),
        "add style selector block (not a window), essentially a combo listing the default styles.");

    m.def("show_font_selector",    // imgui.h:319
        ImGui::ShowFontSelector,
        py::arg("label"),
        "add font selector block (not a window), essentially a combo listing the loaded fonts.");

    m.def("show_user_guide",    // imgui.h:320
    ...
    m.def("get_version",    // imgui.h:321
        ImGui::GetVersion,
        "get the compiled version string e.g. \"1.80 WIP\" (essentially the value for IMGUI_VERSION from the compiled version of imgui.cpp)",
        pybind11::return_value_policy::reference);

    m.def("style_colors_dark",    // imgui.h:324
        ImGui::StyleColorsDark,
        py::arg("dst") = py::none(),
        "new, recommended style (default)");

    m.def("style_colors_light",    // imgui.h:325
        ImGui::StyleColorsLight,
        py::arg("dst") = py::none(),
        "best used with borders and a custom, thicker font");

    m.def("style_colors_classic",    // imgui.h:326
        ImGui::StyleColorsClassic,
        py::arg("dst") = py::none(),
        "classic imgui style");

    m.def("begin",    // imgui.h:340
    ...
    m.def("end",    // imgui.h:341
        ImGui::End);

    m.def("begin_child",    // imgui.h:351
    ...
    m.def("end_child",    // imgui.h:353
        ImGui::EndChild);

    m.def("is_window_appearing",    // imgui.h:357
        ImGui::IsWindowAppearing);

    m.def("is_window_collapsed",    // imgui.h:358
        ImGui::IsWindowCollapsed);

    m.def("is_window_focused",    // imgui.h:359
        ImGui::IsWindowFocused,
        py::arg("flags") = 0,
        "is current window focused? or its root/child, depending on flags. see flags for options.");

    m.def("is_window_hovered",    // imgui.h:360
        ImGui::IsWindowHovered,
        py::arg("flags") = 0,
        "is current window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you are trying to check whether your mouse should be dispatched to imgui or to your app, you should use the 'io.WantCaptureMouse' boolean for that! Please read the FAQ!");

    m.def("get_window_draw_list",    // imgui.h:361
        py::overload_cast<>(ImGui::GetWindowDrawList),
        "get draw list associated to the current window, to append your own drawing primitives",
        pybind11::return_value_policy::reference);

    m.def("get_window_dpi_scale",    // imgui.h:362
        ImGui::GetWindowDpiScale, "get DPI scale currently associated to the current window's viewport.");

    m.def("get_window_pos",    // imgui.h:363
    ...
    m.def("set_next_window_content_size",    // imgui.h:374
    ...
    m.def("pop_item_width",    // imgui.h:428
        ImGui::PopItemWidth);

    m.def("set_next_item_width",    // imgui.h:429
    ...
    m.def("calc_item_width",    // imgui.h:430
    ...
    m.def("push_text_wrap_pos",    // imgui.h:431
    ...
    m.def("get_cursor_pos_y",    // imgui.h:462
        ImGui::GetCursorPosY, "other functions such as GetCursorScreenPos or everything in ImDrawList::");

    m.def("set_cursor_pos",    // imgui.h:463
        ImGui::SetCursorPos,
        py::arg("local_pos"),
        "are using the main, absolute coordinate system.");

    m.def("set_cursor_pos_x",    // imgui.h:464
        ImGui::SetCursorPosX,
        py::arg("local_x"),
        "GetWindowPos() + GetCursorPos() == GetCursorScreenPos() etc.)");

    m.def("set_cursor_pos_y",    // imgui.h:465
        ImGui::SetCursorPosY, py::arg("local_y"));

    m.def("get_cursor_start_pos",    // imgui.h:466
        ImGui::GetCursorStartPos, "initial cursor position in window coordinates");

    m.def("get_cursor_screen_pos",    // imgui.h:467
    ...
    m.def("set_cursor_screen_pos",    // imgui.h:468
        ImGui::SetCursorScreenPos,
        py::arg("pos"),
        "cursor position in absolute coordinates");

    m.def("align_text_to_frame_padding",    // imgui.h:469
    ...
    m.def("text_colored",    // imgui.h:499
    ...
    m.def("text_disabled",    // imgui.h:501
        [](const char * fmt)
        {
    ...
    m.def("text_wrapped",    // imgui.h:503
        [](const char * fmt)
        {
    ...
    m.def("checkbox_flags",    // imgui.h:518
    ...
    m.def("checkbox_flags",    // imgui.h:519
    ...
    m.def("image_button",    // imgui.h:528
    ...
    m.def("drag_float_range2",    // imgui.h:555
    ...
    m.def("drag_int_range2",    // imgui.h:560
    ...
    m.def("drag_scalar_n",    // imgui.h:562
    ...
    m.def("slider_float",    // imgui.h:570
    ...
    m.def("slider_float2",    // imgui.h:571
    ...
    m.def("slider_float3",    // imgui.h:572
    ...
    m.def("slider_float4",    // imgui.h:573
    ...
    m.def("slider_angle",    // imgui.h:574
    ...
    m.def("slider_scalar",    // imgui.h:579
    ...
    m.def("slider_scalar_n",    // imgui.h:580
    ...
    m.def("v_slider_float",    // imgui.h:581
    ...
    m.def("v_slider_int",    // imgui.h:582
    ...
    m.def("v_slider_scalar",    // imgui.h:583
    ...
    m.def("input_float2",    // imgui.h:592
    ...
    m.def("input_float3",    // imgui.h:593
    ...
    m.def("input_float4",    // imgui.h:594
    ...
    m.def("input_double",    // imgui.h:599
    ...
    m.def("input_scalar",    // imgui.h:600
    ...
    m.def("input_scalar_n",    // imgui.h:601
    ...
    m.def("color_picker3",    // imgui.h:608
        [](const char * label, std::array<float, 3> col, ImGuiColorEditFlags flags = 0) -> std::tuple<bool, std::array<float, 3>>
        {
    ...
    m.def("color_picker4",    // imgui.h:609
    ...
    m.def("tree_node_ex",    // imgui.h:622
    ...
    m.def("plot_histogram",    // imgui.h:659
        [](const char * label, const py::array & values, int values_offset = 0, const char * overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0), int stride = -1)
        {
    ...
    m.def("begin_popup_context_window",    // imgui.h:725
        py::overload_cast<const char *, ImGuiPopupFlags>(ImGui::BeginPopupContextWindow),
        py::arg("str_id") = py::none(), py::arg("popup_flags") = 1,
        "open+begin popup when clicked on current window.");

    m.def("begin_popup_context_void",    // imgui.h:726
    ...
    m.def("table_set_column_index",    // imgui.h:761
        py::overload_cast<int>(ImGui::TableSetColumnIndex),
        py::arg("column_n"),
        "append into the specified column. Return True when column is visible.");

    m.def("table_setup_column",    // imgui.h:771
    ...
    m.def("table_header",    // imgui.h:774
        py::overload_cast<const char *>(ImGui::TableHeader),
        py::arg("label"),
        "submit one header cell manually (rarely used)");

    m.def("table_get_sort_specs",    // imgui.h:782
    ...
    m.def("table_set_bg_color",    // imgui.h:789
    ...
    m.def("begin_disabled",    // imgui.h:858
        ImGui::BeginDisabled, py::arg("disabled") = true);

    m.def("end_disabled",    // imgui.h:859
        ImGui::EndDisabled);

    m.def("push_clip_rect",    // imgui.h:863
        ImGui::PushClipRect, py::arg("clip_rect_min"), py::arg("clip_rect_max"), py::arg("intersect_with_current_clip_rect"));

    m.def("pop_clip_rect",    // imgui.h:864
        ImGui::PopClipRect);

    m.def("set_item_default_focus",    // imgui.h:868
        ImGui::SetItemDefaultFocus, "make last item the default focused item of a window.");

    m.def("set_keyboard_focus_here",    // imgui.h:869
        ImGui::SetKeyboardFocusHere,
        py::arg("offset") = 0,
        "focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.");

    m.def("is_item_hovered",    // imgui.h:874
        ImGui::IsItemHovered,
        py::arg("flags") = 0,
        "is the last item hovered? (and usable, aka not blocked by a popup, etc.). See ImGuiHoveredFlags for more options.");

    m.def("is_item_active",    // imgui.h:875
    ...
    m.def("is_item_deactivated_after_edit",    // imgui.h:882
    ...
    m.def("get_main_viewport",    // imgui.h:896
        ImGui::GetMainViewport,
        "return primary/default viewport. This can never be None.",
        pybind11::return_value_policy::reference);

    m.def("get_background_draw_list",    // imgui.h:899
    ...
    m.def("get_foreground_draw_list",    // imgui.h:900
    ...
    m.def("get_background_draw_list",    // imgui.h:901
    ...
    m.def("get_foreground_draw_list",    // imgui.h:902
    ...
    m.def("color_convert_hsv_to_rgb",    // imgui.h:923
    ...
    m.def("is_mouse_dragging",    // imgui.h:952
    ...
    m.def("get_mouse_drag_delta",    // imgui.h:953
    ...
    m.def("reset_mouse_drag_delta",    // imgui.h:954
        ImGui::ResetMouseDragDelta, py::arg("button") = 0);

    m.def("get_mouse_cursor",    // imgui.h:955
    ...
    m.def("debug_text_encoding",    // imgui.h:974
        ImGui::DebugTextEncoding, py::arg("text"));

    m.def("debug_check_version_and_data_layout",    // imgui.h:975
    ...
    m.def("get_platform_io",    // imgui.h:989
        ImGui::GetPlatformIO,
        "platform/renderer functions, for backend to setup + viewports list.",
        pybind11::return_value_policy::reference);

    m.def("update_platform_windows",    // imgui.h:990
    ...
    m.def("find_viewport_by_id",    // imgui.h:993
        ImGui::FindViewportByID,
        py::arg("id_"),
        "this is a helper for backends.",
        pybind11::return_value_policy::reference);

    m.def("find_viewport_by_platform_handle",    // imgui.h:994
    ...
    py::enum_<ImGuiWindowFlags_>(m, "WindowFlags_", py::arithmetic(), " Flags for ImGui::Begin()\n (Those are per-window flags. There are shared flags in ImGuiIO: io.ConfigWindowsResizeFromEdges and io.ConfigWindowsMoveFromTitleBarOnly)")    // imgui.h:1004
    ...
    py::enum_<ImGuiInputTextFlags_>(m, "InputTextFlags_", py::arithmetic(), " Flags for ImGui::InputText()\n (Those are per-item flags. There are shared flags in ImGuiIO: io.ConfigInputTextCursorBlink and io.ConfigInputTextEnterKeepActive)")    // imgui.h:1045
    ...
    py::enum_<ImGuiTreeNodeFlags_>(m, "TreeNodeFlags_", py::arithmetic(), "Flags for ImGui::TreeNodeEx(), ImGui::CollapsingHeader*()")    // imgui.h:1077
    ...
    py::enum_<ImGuiSelectableFlags_>(m, "SelectableFlags_", py::arithmetic(), "Flags for ImGui::Selectable()")    // imgui.h:1122
    ...
    py::enum_<ImGuiComboFlags_>(m, "ComboFlags_", py::arithmetic(), "Flags for ImGui::BeginCombo()")    // imgui.h:1133
    ...
    py::enum_<ImGuiTabBarFlags_>(m, "TabBarFlags_", py::arithmetic(), "Flags for ImGui::BeginTabBar()")    // imgui.h:1147
    ...
    py::enum_<ImGuiTabItemFlags_>(m, "TabItemFlags_", py::arithmetic(), "Flags for ImGui::BeginTabItem()")    // imgui.h:1163
    ...
    py::enum_<ImGuiTableColumnFlags_>(m, "TableColumnFlags_", py::arithmetic(), "Flags for ImGui::TableSetupColumn()")    // imgui.h:1249
    ...
    py::enum_<ImGuiTableRowFlags_>(m, "TableRowFlags_", py::arithmetic(), "Flags for ImGui::TableNextRow()")    // imgui.h:1286
    ...
    py::enum_<ImGuiFocusedFlags_>(m, "FocusedFlags_", py::arithmetic(), "Flags for ImGui::IsWindowFocused()")    // imgui.h:1310
    ...
    py::enum_<ImGuiDragDropFlags_>(m, "DragDropFlags_", py::arithmetic(), "Flags for ImGui::BeginDragDropSource(), ImGui::AcceptDragDropPayload()")    // imgui.h:1363
    ...
    py::enum_<ImGuiDataType_>(m, "DataType_", py::arithmetic(), "A primary data type")    // imgui.h:1385
    ...
    py::enum_<ImGuiDir_>(m, "Dir_", py::arithmetic(), "A cardinal direction")    // imgui.h:1401
    ...
    py::enum_<ImGuiSortDirection_>(m, "SortDirection_", py::arithmetic(), "A sorting direction")    // imgui.h:1412
    ...
    py::enum_<ImGuiKey>(m, "Key", py::arithmetic(), " A key identifier (ImGuiKey_XXX or ImGuiMod_XXX value)\n All our named keys are >= 512. Keys value 0 to 511 are left unused as legacy native/opaque key values (< 1.87)\n Since >= 1.89 we increased typing (went from int to enum), some legacy code may need a cast to ImGuiKey.")    // imgui.h:1422
    ...
    py::enum_<ImGuiConfigFlags_>(m, "ConfigFlags_", py::arithmetic(), "Configuration flags stored in io.ConfigFlags. Set by user/application.")    // imgui.h:1559
    ...
    py::enum_<ImGuiBackendFlags_>(m, "BackendFlags_", py::arithmetic(), "Backend capabilities flags stored in io.BackendFlags. Set by imgui_impl_xxx or custom backend.")    // imgui.h:1584
    ...
    py::enum_<ImGuiCol_>(m, "Col_", py::arithmetic(), "Enumeration for PushStyleColor() / PopStyleColor()")    // imgui.h:1599
    ...
    py::enum_<ImGuiButtonFlags_>(m, "ButtonFlags_", py::arithmetic(), "Flags for InvisibleButton() [extended in imgui_internal.h]")    // imgui.h:1698
    ...
    py::enum_<ImGuiColorEditFlags_>(m, "ColorEditFlags_", py::arithmetic(), "Flags for ColorEdit3() / ColorEdit4() / ColorPicker3() / ColorPicker4() / ColorButton()")    // imgui.h:1711
    ...
    py::enum_<ImGuiMouseButton_>(m, "MouseButton_", py::arithmetic(), " Identify a mouse button.\n Those values are guaranteed to be stable and we frequently use 0/1 directly. Named enums provided for convenience.")    // imgui.h:1774
    ...
    py::enum_<ImGuiMouseCursor_>(m, "MouseCursor_", py::arithmetic(), " Enumeration for GetMouseCursor()\n User code may request backend to display given cursor by calling SetMouseCursor(), which is why we have some cursors that are marked unused here")    // imgui.h:1784
    ...
    py::enum_<ImGuiCond_>(m, "Cond_", py::arithmetic(), " Enumeration for ImGui::SetWindow***(), SetNextWindow***(), SetNextItem***() functions\n Represent a condition.\n Important: Treat as a regular enum! Do NOT combine multiple values using binary operators! All the functions above treat 0 as a shortcut to ImGuiCond_Always.")    // imgui.h:1802
    ...
    auto pyClassImNewWrapper =
        py::class_<ImNewWrapper>    // imgui.h:1821
            (m, "ImNewWrapper", "")
    ...
    auto pyClassImGuiStyle =
        py::class_<ImGuiStyle>    // imgui.h:1914
            (m, "Style", "")
        .def_readwrite("alpha", &ImGuiStyle::Alpha, "Global alpha applies to everything in Dear ImGui.")    // imgui.h:1916
        .def_readwrite("disabled_alpha", &ImGuiStyle::DisabledAlpha, "Additional alpha multiplier applied by BeginDisabled(). Multiply over current value of Alpha.")    // imgui.h:1917
        .def_readwrite("window_padding", &ImGuiStyle::WindowPadding, "Padding within a window.")    // imgui.h:1918
        .def_readwrite("window_rounding", &ImGuiStyle::WindowRounding, "Radius of window corners rounding. Set to 0.0 to have rectangular windows. Large values tend to lead to variety of artifacts and are not recommended.")    // imgui.h:1919
        .def_readwrite("window_border_size", &ImGuiStyle::WindowBorderSize, "Thickness of border around windows. Generally set to 0.0 or 1.0. (Other values are not well tested and more CPU/GPU costly).")    // imgui.h:1920
        .def_readwrite("window_min_size", &ImGuiStyle::WindowMinSize, "Minimum window size. This is a global setting. If you want to constrain individual windows, use SetNextWindowSizeConstraints().")    // imgui.h:1921
        .def_readwrite("window_title_align", &ImGuiStyle::WindowTitleAlign, "Alignment for title bar text. Defaults to (0.0,0.5) for left-aligned,vertically centered.")    // imgui.h:1922
        .def_readwrite("window_menu_button_position", &ImGuiStyle::WindowMenuButtonPosition, "Side of the collapsing/docking button in the title bar (None/Left/Right). Defaults to ImGuiDir_Left.")    // imgui.h:1923
        .def_readwrite("child_rounding", &ImGuiStyle::ChildRounding, "Radius of child window corners rounding. Set to 0.0 to have rectangular windows.")    // imgui.h:1924
        .def_readwrite("child_border_size", &ImGuiStyle::ChildBorderSize, "Thickness of border around child windows. Generally set to 0.0 or 1.0. (Other values are not well tested and more CPU/GPU costly).")    // imgui.h:1925
        .def_readwrite("popup_rounding", &ImGuiStyle::PopupRounding, "Radius of popup window corners rounding. (Note that tooltip windows use WindowRounding)")    // imgui.h:1926
        .def_readwrite("popup_border_size", &ImGuiStyle::PopupBorderSize, "Thickness of border around popup/tooltip windows. Generally set to 0.0 or 1.0. (Other values are not well tested and more CPU/GPU costly).")    // imgui.h:1927
        .def_readwrite("frame_padding", &ImGuiStyle::FramePadding, "Padding within a framed rectangle (used by most widgets).")    // imgui.h:1928
        .def_readwrite("frame_rounding", &ImGuiStyle::FrameRounding, "Radius of frame corners rounding. Set to 0.0 to have rectangular frame (used by most widgets).")    // imgui.h:1929
        .def_readwrite("frame_border_size", &ImGuiStyle::FrameBorderSize, "Thickness of border around frames. Generally set to 0.0 or 1.0. (Other values are not well tested and more CPU/GPU costly).")    // imgui.h:1930
        .def_readwrite("item_spacing", &ImGuiStyle::ItemSpacing, "Horizontal and vertical spacing between widgets/lines.")    // imgui.h:1931
        .def_readwrite("item_inner_spacing", &ImGuiStyle::ItemInnerSpacing, "Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label).")    // imgui.h:1932
        .def_readwrite("cell_padding", &ImGuiStyle::CellPadding, "Padding within a table cell")    // imgui.h:1933
        .def_readwrite("touch_extra_padding", &ImGuiStyle::TouchExtraPadding, "Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!")    // imgui.h:1934
        .def_readwrite("indent_spacing", &ImGuiStyle::IndentSpacing, "Horizontal indentation when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).")    // imgui.h:1935
        .def_readwrite("columns_min_spacing", &ImGuiStyle::ColumnsMinSpacing, "Minimum horizontal spacing between two columns. Preferably > (FramePadding.x + 1).")    // imgui.h:1936
        .def_readwrite("scrollbar_size", &ImGuiStyle::ScrollbarSize, "Width of the vertical scrollbar, Height of the horizontal scrollbar.")    // imgui.h:1937
        .def_readwrite("scrollbar_rounding", &ImGuiStyle::ScrollbarRounding, "Radius of grab corners for scrollbar.")    // imgui.h:1938
        .def_readwrite("grab_min_size", &ImGuiStyle::GrabMinSize, "Minimum width/height of a grab box for slider/scrollbar.")    // imgui.h:1939
        .def_readwrite("grab_rounding", &ImGuiStyle::GrabRounding, "Radius of grabs corners rounding. Set to 0.0 to have rectangular slider grabs.")    // imgui.h:1940
        .def_readwrite("log_slider_deadzone", &ImGuiStyle::LogSliderDeadzone, "The size in pixels of the dead-zone around zero on logarithmic sliders that cross zero.")    // imgui.h:1941
        .def_readwrite("tab_rounding", &ImGuiStyle::TabRounding, "Radius of upper corners of a tab. Set to 0.0 to have rectangular tabs.")    // imgui.h:1942
        .def_readwrite("tab_border_size", &ImGuiStyle::TabBorderSize, "Thickness of border around tabs.")    // imgui.h:1943
        .def_readwrite("tab_min_width_for_close_button", &ImGuiStyle::TabMinWidthForCloseButton, "Minimum width for close button to appear on an unselected tab when hovered. Set to 0.0 to always show when hovering, set to FLT_MAX to never show close button unless selected.")    // imgui.h:1944
        .def_readwrite("color_button_position", &ImGuiStyle::ColorButtonPosition, "Side of the color button in the ColorEdit4 widget (left/right). Defaults to ImGuiDir_Right.")    // imgui.h:1945
        .def_readwrite("button_text_align", &ImGuiStyle::ButtonTextAlign, "Alignment of button text when button is larger than text. Defaults to (0.5, 0.5) (centered).")    // imgui.h:1946
        .def_readwrite("selectable_text_align", &ImGuiStyle::SelectableTextAlign, "Alignment of selectable text. Defaults to (0.0, 0.0) (top-left aligned). It's generally important to keep this left-aligned if you want to lay multiple items on a same line.")    // imgui.h:1947
        .def_readwrite("display_window_padding", &ImGuiStyle::DisplayWindowPadding, "Window position are clamped to be visible within the display area or monitors by at least this amount. Only applies to regular windows.")    // imgui.h:1948
        .def_readwrite("display_safe_area_padding", &ImGuiStyle::DisplaySafeAreaPadding, "If you cannot see the edges of your screen (e.g. on a TV) increase the safe area padding. Apply to popups/tooltips as well regular windows. NB: Prefer configuring your TV sets correctly!")    // imgui.h:1949
        .def_readwrite("mouse_cursor_scale", &ImGuiStyle::MouseCursorScale, "Scale software rendered mouse cursor (when io.MouseDrawCursor is enabled). We apply per-monitor DPI scaling over this scale. May be removed later.")    // imgui.h:1950
        .def_readwrite("anti_aliased_lines", &ImGuiStyle::AntiAliasedLines, "Enable anti-aliased lines/borders. Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).")    // imgui.h:1951
        .def_readwrite("anti_aliased_lines_use_tex", &ImGuiStyle::AntiAliasedLinesUseTex, "Enable anti-aliased lines/borders using textures where possible. Require backend to render with bilinear filtering (NOT point/nearest filtering). Latched at the beginning of the frame (copied to ImDrawList).")    // imgui.h:1952
        ...
        .def("get_color",    // imgui.h:1962
            &ImGuiStyle::GetColor,
            py::arg("idx_color"),
            pybind11::return_value_policy::reference)
        .def("set_color",    // imgui.h:1963
            &ImGuiStyle::SetColor, py::arg("idx_color"), py::arg("color"))
        .def(py::init<>())    // imgui.h:1968
        .def("scale_all_sizes",    // imgui.h:1969
            &ImGuiStyle::ScaleAllSizes, py::arg("scale_factor"))
        ;


    auto pyClassImGuiKeyData =
        py::class_<ImGuiKeyData>    // imgui.h:1981
        ...
        .def_readwrite("down", &ImGuiKeyData::Down, "True for if key is down")    // imgui.h:1983
        .def_readwrite("down_duration", &ImGuiKeyData::DownDuration, "Duration the key has been down (<0.0: not pressed, 0.0: just pressed, >0.0: time held)")    // imgui.h:1984
        .def_readwrite("down_duration_prev", &ImGuiKeyData::DownDurationPrev, "Last frame duration the key has been down")    // imgui.h:1985
        .def_readwrite("analog_value", &ImGuiKeyData::AnalogValue, "0.0..1.0 for gamepad values")    // imgui.h:1986
        ;


    auto pyClassImGuiIO =
        py::class_<ImGuiIO>    // imgui.h:1989
            (m, "IO", "")
        .def_readwrite("config_flags", &ImGuiIO::ConfigFlags, "= 0              // See ImGuiConfigFlags_ enum. Set by user/application. Gamepad/keyboard navigation options, etc.")    // imgui.h:1995
        .def_readwrite("backend_flags", &ImGuiIO::BackendFlags, "= 0              // See ImGuiBackendFlags_ enum. Set by backend (imgui_impl_xxx files or custom backend) to communicate features supported by the backend.")    // imgui.h:1996
        .def_readwrite("display_size", &ImGuiIO::DisplaySize, "<unset>          // Main display size, in pixels (generally == GetMainViewport()->Size). May change every frame.")    // imgui.h:1997
        .def_readwrite("delta_time", &ImGuiIO::DeltaTime, "= 1.0/60.0     // Time elapsed since last frame, in seconds. May change every frame.")    // imgui.h:1998
        .def_readwrite("ini_saving_rate", &ImGuiIO::IniSavingRate, "= 5.0           // Minimum time between saving positions/sizes to .ini file, in seconds.")    // imgui.h:1999
        .def_readonly("ini_filename", &ImGuiIO::IniFilename, "= \"imgui.ini\"    // Path to .ini file (important: default \"imgui.ini\" is relative to current working dir!). Set None to disable automatic .ini loading/saving or if you want to manually call LoadIniSettingsXXX() / SaveIniSettingsXXX() functions.")    // imgui.h:2000
        .def_readonly("log_filename", &ImGuiIO::LogFilename, "= \"imgui_log.txt\"// Path to .log file (default parameter to ImGui::LogToFile when no file is specified).")    // imgui.h:2001
        .def_readwrite("mouse_double_click_time", &ImGuiIO::MouseDoubleClickTime, "= 0.30          // Time for a double-click, in seconds.")    // imgui.h:2002
        .def_readwrite("mouse_double_click_max_dist", &ImGuiIO::MouseDoubleClickMaxDist, "= 6.0           // Distance threshold to stay in to validate a double-click, in pixels.")    // imgui.h:2003
        .def_readwrite("mouse_drag_threshold", &ImGuiIO::MouseDragThreshold, "= 6.0           // Distance threshold before considering we are dragging.")    // imgui.h:2004
        .def_readwrite("key_repeat_delay", &ImGuiIO::KeyRepeatDelay, "= 0.275         // When holding a key/button, time before it starts repeating, in seconds (for buttons in Repeat mode, etc.).")    // imgui.h:2005
        .def_readwrite("key_repeat_rate", &ImGuiIO::KeyRepeatRate, "= 0.050         // When holding a key/button, rate at which it repeats, in seconds.")    // imgui.h:2006
        .def_readwrite("hover_delay_normal", &ImGuiIO::HoverDelayNormal, "= 0.30 sec       // Delay on hovering before IsItemHovered(ImGuiHoveredFlags_DelayNormal) returns True.")    // imgui.h:2007
        .def_readwrite("hover_delay_short", &ImGuiIO::HoverDelayShort, "= 0.10 sec       // Delay on hovering before IsItemHovered(ImGuiHoveredFlags_DelayShort) returns True.")    // imgui.h:2008
        .def_readwrite("user_data", &ImGuiIO::UserData, "= None           // Store your own data.")    // imgui.h:2009
        .def_readwrite("fonts", &ImGuiIO::Fonts, "<auto>           // Font atlas: load, rasterize and pack one or more fonts into a single texture.")    // imgui.h:2011
        .def_readwrite("font_global_scale", &ImGuiIO::FontGlobalScale, "= 1.0           // Global scale all fonts")    // imgui.h:2012
        .def_readwrite("font_allow_user_scaling", &ImGuiIO::FontAllowUserScaling, "= False          // Allow user scaling text of individual window with CTRL+Wheel.")    // imgui.h:2013
        .def_readwrite("font_default", &ImGuiIO::FontDefault, "= None           // Font to use on NewFrame(). Use None to uses Fonts->Fonts[0].")    // imgui.h:2014
        .def_readwrite("display_framebuffer_scale", &ImGuiIO::DisplayFramebufferScale, "= (1, 1)         // For retina display or other situations where window coordinates are different from framebuffer coordinates. This generally ends up in ImDrawData::FramebufferScale.")    // imgui.h:2015
        .def_readwrite("config_docking_no_split", &ImGuiIO::ConfigDockingNoSplit, "= False          // Simplified docking mode: disable window splitting, so docking is limited to merging multiple windows together into tab-bars.")    // imgui.h:2018
        .def_readwrite("config_docking_with_shift", &ImGuiIO::ConfigDockingWithShift, "= False          // Enable docking with holding Shift key (reduce visual noise, allows dropping in wider space)")    // imgui.h:2019
        .def_readwrite("config_docking_always_tab_bar", &ImGuiIO::ConfigDockingAlwaysTabBar, "= False          // [BETA] [FIXME: This currently creates regression with auto-sizing and general overhead] Make every single floating window display within a docking node.")    // imgui.h:2020
        ...
        .def_readwrite("config_viewports_no_auto_merge", &ImGuiIO::ConfigViewportsNoAutoMerge, "= False;         // Set to make all floating imgui windows always create their own viewport. Otherwise, they are merged into the main host viewports when overlapping it. May also set ImGuiViewportFlags_NoAutoMerge on individual viewport.")    // imgui.h:2024
        .def_readwrite("config_viewports_no_task_bar_icon", &ImGuiIO::ConfigViewportsNoTaskBarIcon, "= False          // Disable default OS task bar icon flag for secondary viewports. When a viewport doesn't want a task bar icon, ImGuiViewportFlags_NoTaskBarIcon will be set on it.")    // imgui.h:2025
        ...
        .def_readwrite("mouse_draw_cursor", &ImGuiIO::MouseDrawCursor, "= False          // Request ImGui to draw a mouse cursor for you (if you are on a platform without a mouse cursor). Cannot be easily renamed to 'io.ConfigXXX' because this is frequently used by backend implementations.")    // imgui.h:2030
        ...
        .def_readwrite("config_input_trickle_event_queue", &ImGuiIO::ConfigInputTrickleEventQueue, "= True           // Enable input queue trickling: some types of events submitted during the same frame (e.g. button down + up) will be spread over multiple frames, improving interactions with low framerates.")    // imgui.h:2032
        .def_readwrite("config_input_text_cursor_blink", &ImGuiIO::ConfigInputTextCursorBlink, "= True           // Enable blinking cursor (optional as some users consider it to be distracting).")    // imgui.h:2033
        .def_readwrite("config_input_text_enter_keep_active", &ImGuiIO::ConfigInputTextEnterKeepActive, "= False          // [BETA] Pressing Enter will keep item active and select contents (single-line only).")    // imgui.h:2034
        .def_readwrite("config_drag_click_to_input_text", &ImGuiIO::ConfigDragClickToInputText, "= False          // [BETA] Enable turning DragXXX widgets into text input with a simple mouse click-release (without moving). Not desirable on devices without a keyboard.")    // imgui.h:2035
        ...
        .def_readwrite("config_windows_move_from_title_bar_only", &ImGuiIO::ConfigWindowsMoveFromTitleBarOnly, "= False       // Enable allowing to move windows only when clicking on their title bar. Does not apply to windows without a title bar.")    // imgui.h:2037
        .def_readwrite("config_memory_compact_timer", &ImGuiIO::ConfigMemoryCompactTimer, "= 60.0          // Timer (in seconds) to free transient windows/tables memory buffers when unused. Set to -1.0 to disable.")    // imgui.h:2038
        .def_readonly("backend_platform_name", &ImGuiIO::BackendPlatformName, "= None")    // imgui.h:2046
        .def_readonly("backend_renderer_name", &ImGuiIO::BackendRendererName, "= None")    // imgui.h:2047
        .def_readwrite("backend_platform_user_data", &ImGuiIO::BackendPlatformUserData, "= None           // User data for platform backend")    // imgui.h:2048
        .def_readwrite("backend_renderer_user_data", &ImGuiIO::BackendRendererUserData, "= None           // User data for renderer backend")    // imgui.h:2049
        .def_readwrite("backend_language_user_data", &ImGuiIO::BackendLanguageUserData, "= None           // User data for non C++ programming language backend")    // imgui.h:2050
        .def_readwrite("clipboard_user_data", &ImGuiIO::ClipboardUserData, "")    // imgui.h:2056
        .def("add_key_event",    // imgui.h:2072
            &ImGuiIO::AddKeyEvent,
            py::arg("key"), py::arg("down"),
            "Queue a new key down/up event. Key should be \"translated\" (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)")
        .def("add_key_analog_event",    // imgui.h:2073
            &ImGuiIO::AddKeyAnalogEvent,
            py::arg("key"), py::arg("down"), py::arg("v"),
            "Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.")
        .def("add_mouse_pos_event",    // imgui.h:2074
            &ImGuiIO::AddMousePosEvent,
            py::arg("x"), py::arg("y"),
            "Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. app not focused and not hovered)")
        .def("add_mouse_button_event",    // imgui.h:2075
            &ImGuiIO::AddMouseButtonEvent,
            py::arg("button"), py::arg("down"),
            "Queue a mouse button change")
        .def("add_mouse_wheel_event",    // imgui.h:2076
            &ImGuiIO::AddMouseWheelEvent,
            py::arg("wh_x"), py::arg("wh_y"),
            "Queue a mouse wheel update")
        .def("add_mouse_viewport_event",    // imgui.h:2077
            &ImGuiIO::AddMouseViewportEvent,
            py::arg("id_"),
            "Queue a mouse hovered viewport. Requires backend to set ImGuiBackendFlags_HasMouseHoveredViewport to call this (for multi-viewport support).")
        .def("add_focus_event",    // imgui.h:2078
            &ImGuiIO::AddFocusEvent,
            py::arg("focused"),
            "Queue a gain/loss of focus for the application (generally based on OS/platform focus of your window)")
        .def("add_input_character",    // imgui.h:2079
            &ImGuiIO::AddInputCharacter,
            py::arg("c"),
            "Queue a new character input")
        .def("add_input_character_utf16",    // imgui.h:2080
            &ImGuiIO::AddInputCharacterUTF16,
            py::arg("c"),
            "Queue a new character input from a UTF-16 character, it can be a surrogate")
        .def("add_input_characters_utf8",    // imgui.h:2081
            &ImGuiIO::AddInputCharactersUTF8,
            py::arg("str"),
            "Queue a new characters input from a UTF-8 string")
        .def("set_key_event_native_data",    // imgui.h:2083
        ...
        .def_readwrite("want_capture_keyboard", &ImGuiIO::WantCaptureKeyboard, "Set when Dear ImGui will use keyboard inputs, in this case do not dispatch them to your main game/application (either way, always pass keyboard inputs to imgui). (e.g. InputText active, or an imgui window is focused and navigation is enabled, etc.).")    // imgui.h:2095
        .def_readwrite("want_text_input", &ImGuiIO::WantTextInput, "Mobile/console: when set, you may display an on-screen keyboard. This is set by Dear ImGui when it wants textual keyboard input to happen (e.g. when a InputText widget is active).")    // imgui.h:2096
        .def_readwrite("want_set_mouse_pos", &ImGuiIO::WantSetMousePos, "MousePos has been altered, backend should reposition mouse on next frame. Rarely used! Set only when ImGuiConfigFlags_NavEnableSetMousePos flag is enabled.")    // imgui.h:2097
        .def_readwrite("want_save_ini_settings", &ImGuiIO::WantSaveIniSettings, "When manual .ini load/save is active (io.IniFilename == None), this will be set to notify your application that you can call SaveIniSettingsToMemory() and save yourself. Important: clear io.WantSaveIniSettings yourself after saving!")    // imgui.h:2098
        .def_readwrite("nav_active", &ImGuiIO::NavActive, "Keyboard/Gamepad navigation is currently allowed (will handle ImGuiKey_NavXXX events) = a window is focused and it doesn't use the ImGuiWindowFlags_NoNavInputs flag.")    // imgui.h:2099
        .def_readwrite("nav_visible", &ImGuiIO::NavVisible, "Keyboard/Gamepad navigation is visible and allowed (will handle ImGuiKey_NavXXX events).")    // imgui.h:2100
        .def_readwrite("framerate", &ImGuiIO::Framerate, "Estimate of application framerate (rolling average over 60 frames, based on io.DeltaTime), in frame per second. Solely for convenience. Slow applications may not want to use a moving average or may want to reset underlying buffers occasionally.")    // imgui.h:2101
        .def_readwrite("metrics_render_vertices", &ImGuiIO::MetricsRenderVertices, "Vertices output during last call to Render()")    // imgui.h:2102
        .def_readwrite("metrics_render_indices", &ImGuiIO::MetricsRenderIndices, "Indices output during last call to Render() = number of triangles * 3")    // imgui.h:2103
        .def_readwrite("metrics_render_windows", &ImGuiIO::MetricsRenderWindows, "Number of visible windows")    // imgui.h:2104
        .def_readwrite("metrics_active_windows", &ImGuiIO::MetricsActiveWindows, "Number of active windows")    // imgui.h:2105
        .def_readwrite("metrics_active_allocations", &ImGuiIO::MetricsActiveAllocations, "Number of active allocations, updated by MemAlloc/MemFree based on current context. May be off if you have multiple imgui contexts.")    // imgui.h:2106
        .def_readwrite("mouse_delta", &ImGuiIO::MouseDelta, "")    // imgui.h:2107
        .def_readwrite("mouse_pos", &ImGuiIO::MousePos, "Mouse position, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX) if mouse is unavailable (on another screen, etc.)")    // imgui.h:2125
        .def_property("mouse_down",    // imgui.h:2126
        ...
        .def_readwrite("mouse_wheel", &ImGuiIO::MouseWheel, "Mouse wheel Vertical: 1 unit scrolls about 5 lines text.")    // imgui.h:2127
        .def_readwrite("mouse_wheel_h", &ImGuiIO::MouseWheelH, "Mouse wheel Horizontal. Most users don't have a mouse with a horizontal wheel, may not be filled by all backends.")    // imgui.h:2128
        ...
        .def_readwrite("key_ctrl", &ImGuiIO::KeyCtrl, "Keyboard modifier down: Control")    // imgui.h:2130
        .def_readwrite("key_shift", &ImGuiIO::KeyShift, "Keyboard modifier down: Shift")    // imgui.h:2131
        .def_readwrite("key_alt", &ImGuiIO::KeyAlt, "Keyboard modifier down: Alt")    // imgui.h:2132
        .def_readwrite("key_super", &ImGuiIO::KeySuper, "Keyboard modifier down: Cmd/Super/Windows")    // imgui.h:2133
        .def_readwrite("key_mods", &ImGuiIO::KeyMods, "Key mods flags (any of ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super flags, same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags. DOES NOT CONTAINS ImGuiMod_Shortcut which is pretranslated). Read-only, updated by NewFrame()")    // imgui.h:2136
        .def_readwrite("want_capture_mouse_unless_popup_close", &ImGuiIO::WantCaptureMouseUnlessPopupClose, "Alternative to WantCaptureMouse: (WantCaptureMouse == True && WantCaptureMouseUnlessPopupClose == False) when a click over None is expected to close a popup.")    // imgui.h:2138
        .def_readwrite("mouse_pos_prev", &ImGuiIO::MousePosPrev, "Previous mouse position (note that MouseDelta is not necessary == MousePos-MousePosPrev, in case either position is invalid)")    // imgui.h:2139
        .def_property("mouse_clicked_time",    // imgui.h:2141
            [](ImGuiIO &self) -> pybind11::array
            {
        ...
        .def_property("mouse_clicked",    // imgui.h:2142
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_double_clicked",    // imgui.h:2143
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_clicked_count",    // imgui.h:2144
            [](ImGuiIO &self) -> pybind11::array
            {
        ...
        .def_property("mouse_clicked_last_count",    // imgui.h:2145
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<ImU16>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(ImU16)});
        ...
        .def_property("mouse_released",    // imgui.h:2146
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_down_owned",    // imgui.h:2147
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_down_owned_unless_popup_close",    // imgui.h:2148
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<bool>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(bool)});
        ...
        .def_property("mouse_down_duration",    // imgui.h:2149
            [](ImGuiIO &self) -> pybind11::array
            {
        ...
        .def_property("mouse_down_duration_prev",    // imgui.h:2150
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<float>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(float)});
        ...
        .def_property("mouse_drag_max_distance_sqr",    // imgui.h:2152
            [](ImGuiIO &self) -> pybind11::array
            {
                auto dtype = pybind11::dtype(pybind11::format_descriptor<float>::format());
                auto base = pybind11::array(dtype, {5}, {sizeof(float)});
        ...
        .def_readwrite("pen_pressure", &ImGuiIO::PenPressure, "Touch/Pen pressure (0.0 to 1.0, should be >0.0 only when MouseDown[0] == True). Helper storage currently unused by Dear ImGui.")    // imgui.h:2153
        .def_readwrite("app_focus_lost", &ImGuiIO::AppFocusLost, "Only modify via AddFocusEvent()")    // imgui.h:2154
        .def_readwrite("app_accepting_events", &ImGuiIO::AppAcceptingEvents, "Only modify via SetAppAcceptingEvents()")    // imgui.h:2155
        .def_readwrite("backend_using_legacy_key_arrays", &ImGuiIO::BackendUsingLegacyKeyArrays, "-1: unknown, 0: using AddKeyEvent(), 1: using legacy io.KeysDown[]")    // imgui.h:2156
        .def_readwrite("backend_using_legacy_nav_input_array", &ImGuiIO::BackendUsingLegacyNavInputArray, "0: using AddKeyAnalogEvent(), 1: writing to legacy io.NavInputs[] directly")    // imgui.h:2157
        .def_readwrite("input_queue_surrogate", &ImGuiIO::InputQueueSurrogate, "For AddInputCharacterUTF16()")    // imgui.h:2158
        .def(py::init<>())    // imgui.h:2161
        ;


    auto pyClassImGuiInputTextCallbackData =
        py::class_<ImGuiInputTextCallbackData>    // imgui.h:2177
        ...
        .def_readwrite("event_flag", &ImGuiInputTextCallbackData::EventFlag, "One ImGuiInputTextFlags_Callback*    // Read-only")    // imgui.h:2179
        .def_readwrite("flags", &ImGuiInputTextCallbackData::Flags, "What user passed to InputText()      // Read-only")    // imgui.h:2180
        .def_readwrite("user_data", &ImGuiInputTextCallbackData::UserData, "What user passed to InputText()      // Read-only")    // imgui.h:2181
        .def_readwrite("event_char", &ImGuiInputTextCallbackData::EventChar, "Character input                      // Read-write   // [CharFilter] Replace character with another one, or set to zero to drop. return 1 is equivalent to setting EventChar=0;")    // imgui.h:2186
        .def_readwrite("event_key", &ImGuiInputTextCallbackData::EventKey, "Key pressed (Up/Down/TAB)            // Read-only    // [Completion,History]")    // imgui.h:2187
        .def_readwrite("buf_text_len", &ImGuiInputTextCallbackData::BufTextLen, "Text length (in bytes)               // Read-write   // [Resize,Completion,History,Always] Exclude zero-terminator storage. In C land: == strlen(some_text), in C++ land: string.length()")    // imgui.h:2189
        .def_readwrite("buf_size", &ImGuiInputTextCallbackData::BufSize, "Buffer size (in bytes) = capacity+1  // Read-only    // [Resize,Completion,History,Always] Include zero-terminator storage. In C land == ARRAYSIZE(my_char_array), in C++ land: string.capacity()+1")    // imgui.h:2190
        .def_readwrite("buf_dirty", &ImGuiInputTextCallbackData::BufDirty, "Set if you modify Buf/BufTextLen!    // Write        // [Completion,History,Always]")    // imgui.h:2191
        .def_readwrite("cursor_pos", &ImGuiInputTextCallbackData::CursorPos, "// Read-write   // [Completion,History,Always]")    // imgui.h:2192
        .def_readwrite("selection_start", &ImGuiInputTextCallbackData::SelectionStart, "// Read-write   // [Completion,History,Always] == to SelectionEnd when no selection)")    // imgui.h:2193
        .def_readwrite("selection_end", &ImGuiInputTextCallbackData::SelectionEnd, "// Read-write   // [Completion,History,Always]")    // imgui.h:2194
        .def(py::init<>())    // imgui.h:2198
        .def("delete_chars",    // imgui.h:2199
            &ImGuiInputTextCallbackData::DeleteChars, py::arg("pos"), py::arg("bytes_count"))
        .def("insert_chars",    // imgui.h:2200
            &ImGuiInputTextCallbackData::InsertChars, py::arg("pos"), py::arg("text"), py::arg("text_end") = py::none())
        ;


    auto pyClassImGuiSizeCallbackData =
        py::class_<ImGuiSizeCallbackData>    // imgui.h:2208
        ...
        .def_readwrite("user_data", &ImGuiSizeCallbackData::UserData, "Read-only.   What user passed to SetNextWindowSizeConstraints(). Generally store an integer or float in here (need reinterpret_cast<>).")    // imgui.h:2210
        .def_readwrite("pos", &ImGuiSizeCallbackData::Pos, "Read-only.   Window position, for reference.")    // imgui.h:2211
        .def_readwrite("current_size", &ImGuiSizeCallbackData::CurrentSize, "Read-only.   Current window size.")    // imgui.h:2212
        .def_readwrite("desired_size", &ImGuiSizeCallbackData::DesiredSize, "Read-write.  Desired size, based on user's mouse position. Write to this field to restrain resizing.")    // imgui.h:2213
        ;


    auto pyClassImGuiWindowClass =
        py::class_<ImGuiWindowClass>    // imgui.h:2223
        ...
        .def_readwrite("class_id", &ImGuiWindowClass::ClassId, "User data. 0 = Default class (unclassed). Windows of different classes cannot be docked with each others.")    // imgui.h:2225
        ...
        .def_readwrite("viewport_flags_override_set", &ImGuiWindowClass::ViewportFlagsOverrideSet, "Viewport flags to set when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis.")    // imgui.h:2227
        .def_readwrite("viewport_flags_override_clear", &ImGuiWindowClass::ViewportFlagsOverrideClear, "Viewport flags to clear when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis.")    // imgui.h:2228
        .def_readwrite("tab_item_flags_override_set", &ImGuiWindowClass::TabItemFlagsOverrideSet, "[EXPERIMENTAL] TabItem flags to set when a window of this class gets submitted into a dock node tab bar. May use with ImGuiTabItemFlags_Leading or ImGuiTabItemFlags_Trailing.")    // imgui.h:2229
        .def_readwrite("dock_node_flags_override_set", &ImGuiWindowClass::DockNodeFlagsOverrideSet, "[EXPERIMENTAL] Dock node flags to set when a window of this class is hosted by a dock node (it doesn't have to be selected!)")    // imgui.h:2230
        .def_readwrite("docking_always_tab_bar", &ImGuiWindowClass::DockingAlwaysTabBar, "Set to True to enforce single floating windows of this class always having their own docking node (equivalent of setting the global io.ConfigDockingAlwaysTabBar)")    // imgui.h:2231
        .def_readwrite("docking_allow_unclassed", &ImGuiWindowClass::DockingAllowUnclassed, "Set to True to allow windows of this class to be docked/merged with an unclassed window. // FIXME-DOCK: Move to DockNodeFlags override?")    // imgui.h:2232
        .def(py::init<>())    // imgui.h:2234
        ;


    auto pyClassImGuiPayload =
        py::class_<ImGuiPayload>    // imgui.h:2238
            (m, "Payload", "Data payload for Drag and Drop operations: AcceptDragDropPayload(), GetDragDropPayload()")
        .def_readwrite("data", &ImGuiPayload::Data, "Data (copied and owned by dear imgui)")    // imgui.h:2241
        .def_readwrite("data_size", &ImGuiPayload::DataSize, "Data size")    // imgui.h:2242
        .def_readwrite("source_id", &ImGuiPayload::SourceId, "Source item id")    // imgui.h:2245
        .def_readwrite("source_parent_id", &ImGuiPayload::SourceParentId, "Source parent id (if available)")    // imgui.h:2246
        .def_readwrite("data_frame_count", &ImGuiPayload::DataFrameCount, "Data timestamp")    // imgui.h:2247
        .def_readwrite("preview", &ImGuiPayload::Preview, "Set when AcceptDragDropPayload() was called and mouse has been hovering the target item (nb: handle overlapping drag targets)")    // imgui.h:2249
        .def_readwrite("delivery", &ImGuiPayload::Delivery, "Set when AcceptDragDropPayload() was called and mouse button is released over the target item.")    // imgui.h:2250
        .def(py::init<>())    // imgui.h:2252
        ;


    auto pyClassImGuiTableColumnSortSpecs =
        py::class_<ImGuiTableColumnSortSpecs>    // imgui.h:2260
            (m, "TableColumnSortSpecs", "Sorting specification for one column of a table (sizeof == 12 bytes)")
        .def_readwrite("column_user_id", &ImGuiTableColumnSortSpecs::ColumnUserID, "User id of the column (if specified by a TableSetupColumn() call)")    // imgui.h:2262
        .def_readwrite("column_index", &ImGuiTableColumnSortSpecs::ColumnIndex, "Index of the column")    // imgui.h:2263
        .def_readwrite("sort_order", &ImGuiTableColumnSortSpecs::SortOrder, "Index within parent ImGuiTableSortSpecs (always stored in order starting from 0, tables sorted on a single criteria will always have a 0 here)")    // imgui.h:2264
        .def(py::init<>())    // imgui.h:2267
        ;


    auto pyClassImGuiTableSortSpecs =
        py::class_<ImGuiTableSortSpecs>    // imgui.h:2274
        ...
        .def_readonly("specs", &ImGuiTableSortSpecs::Specs, "Pointer to sort spec array.")    // imgui.h:2276
        .def_readwrite("specs_count", &ImGuiTableSortSpecs::SpecsCount, "Sort spec count. Most often 1. May be > 1 when ImGuiTableFlags_SortMulti is enabled. May be == 0 when ImGuiTableFlags_SortTristate is enabled.")    // imgui.h:2277
        .def_readwrite("specs_dirty", &ImGuiTableSortSpecs::SpecsDirty, "Set to True when specs have changed since last time! Use this to sort again, then clear the flag.")    // imgui.h:2278
        .def(py::init<>())    // imgui.h:2280
        ;


    auto pyClassImGuiOnceUponAFrame =
        py::class_<ImGuiOnceUponAFrame>    // imgui.h:2297
        ...
        .def(py::init<>())    // imgui.h:2299
        .def_readwrite("ref_frame", &ImGuiOnceUponAFrame::RefFrame, "")    // imgui.h:2300
        .def("__bool__",    // imgui.h:2301
            &ImGuiOnceUponAFrame::operator bool)
        ;


    auto pyClassImGuiTextFilter =
        py::class_<ImGuiTextFilter>    // imgui.h:2305
    ...
    { // inner classes & enums of TextFilter
        auto pyClassImGuiTextFilter_ClassImGuiTextRange =
            py::class_<ImGuiTextFilter::ImGuiTextRange>    // imgui.h:2315
                (pyClassImGuiTextFilter, "TextRange", "[Internal]")
            .def_readonly("b", &ImGuiTextFilter::ImGuiTextRange::b, "")    // imgui.h:2317
            .def_readonly("e", &ImGuiTextFilter::ImGuiTextRange::e, "")    // imgui.h:2318
            .def(py::init<>())    // imgui.h:2320
            .def(py::init<const char *, const char *>(),    // imgui.h:2321
                py::arg("_b"), py::arg("_e"))
            ;
    } // end of inner classes & enums of TextFilter

    pyClassImGuiTextFilter
        .def(py::init<const char *>(),    // imgui.h:2307
            py::arg("default_filter") = "")
        .def("draw",    // imgui.h:2308
            &ImGuiTextFilter::Draw,
            py::arg("label") = "Filter (inc,-exc)", py::arg("width") = 0.0f,
            "Helper calling InputText+Build")
        .def("pass_filter",    // imgui.h:2309
            &ImGuiTextFilter::PassFilter, py::arg("text"), py::arg("text_end") = py::none())
        .def("build",    // imgui.h:2310
            &ImGuiTextFilter::Build)
        .def_readwrite("count_grep", &ImGuiTextFilter::CountGrep, "")    // imgui.h:2327
        ;


    auto pyClassImGuiTextBuffer =
        py::class_<ImGuiTextBuffer>    // imgui.h:2332
            (m, "TextBuffer", " Helper: Growable text buffer for logging/accumulating text\n (this could be called 'ImGuiTextBuilder' / 'ImGuiStringBuilder')")
        .def(py::init<>())    // imgui.h:2337
        .def("append",    // imgui.h:2346
            &ImGuiTextBuffer::append, py::arg("str"), py::arg("str_end") = py::none())
        .def("appendf",    // imgui.h:2347
    ...
    auto pyClassImGuiStorage =
        py::class_<ImGuiStorage>    // imgui.h:2359
    ...
    { // inner classes & enums of Storage
        auto pyClassImGuiStorage_ClassImGuiStoragePair =
            py::class_<ImGuiStorage::ImGuiStoragePair>    // imgui.h:2362
                (pyClassImGuiStorage, "StoragePair", "[Internal]")
            .def_readwrite("key", &ImGuiStorage::ImGuiStoragePair::key, "")    // imgui.h:2364
            .def(py::init<ImGuiID, int>(),    // imgui.h:2366
                py::arg("_key"), py::arg("_val_i"))
            .def(py::init<ImGuiID, float>(),    // imgui.h:2367
                py::arg("_key"), py::arg("_val_f"))
            .def(py::init<ImGuiID, void *>(),    // imgui.h:2368
    ...
    auto pyClassImGuiListClipper =
        py::class_<ImGuiListClipper>    // imgui.h:2422
        ...
        .def_readwrite("display_start", &ImGuiListClipper::DisplayStart, "First item to display, updated by each call to Step()")    // imgui.h:2424
        .def_readwrite("display_end", &ImGuiListClipper::DisplayEnd, "End of items to display (exclusive)")    // imgui.h:2425
        .def_readwrite("items_count", &ImGuiListClipper::ItemsCount, "[Internal] Number of items")    // imgui.h:2426
        .def_readwrite("items_height", &ImGuiListClipper::ItemsHeight, "[Internal] Height of item after a first step and item submission can calculate it")    // imgui.h:2427
        .def_readwrite("start_pos_y", &ImGuiListClipper::StartPosY, "[Internal] Cursor position at the time of Begin() or after table frozen rows are all processed")    // imgui.h:2428
        .def_readwrite("temp_data", &ImGuiListClipper::TempData, "[Internal] Internal data")    // imgui.h:2429
        .def(py::init<>(),    // imgui.h:2433
        ...
        .def("force_display_range_by_indices",    // imgui.h:2440
    ...
    auto pyClassImColor =
        py::class_<ImColor>    // imgui.h:2473
        ...
        .def_readwrite("value", &ImColor::Value, "")    // imgui.h:2475
        .def(py::init<>())    // imgui.h:2477
        .def(py::init<float, float, float, float>(),    // imgui.h:2478
            py::arg("r"), py::arg("g"), py::arg("b"), py::arg("a") = 1.0f)
        .def(py::init<const ImVec4 &>(),    // imgui.h:2479
            py::arg("col"))
        .def(py::init<int, int, int, int>(),    // imgui.h:2480
            py::arg("r"), py::arg("g"), py::arg("b"), py::arg("a") = 255)
        .def(py::init<ImU32>(),    // imgui.h:2481
            py::arg("rgba"))
        ;


    auto pyClassImDrawCmd =
        py::class_<ImDrawCmd>    // imgui.h:2522
        ...
        .def_readwrite("clip_rect", &ImDrawCmd::ClipRect, "4*4  // Clipping rectangle (x1, y1, x2, y2). Subtract ImDrawData->DisplayPos to get clipping rectangle in \"viewport\" coordinates")    // imgui.h:2524
        .def_readwrite("texture_id", &ImDrawCmd::TextureId, "4-8  // User-provided texture ID. Set by user in ImfontAtlas::SetTexID() for fonts or passed to Image*() functions. Ignore if never using images or multiple fonts atlas.")    // imgui.h:2525
        .def_readwrite("vtx_offset", &ImDrawCmd::VtxOffset, "4    // Start offset in vertex buffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support meshes larger than 64K vertices with 16-bit indices.")    // imgui.h:2526
        .def_readwrite("idx_offset", &ImDrawCmd::IdxOffset, "4    // Start offset in index buffer.")    // imgui.h:2527
        .def_readwrite("elem_count", &ImDrawCmd::ElemCount, "4    // Number of indices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtx_buffer[] array, indices in idx_buffer[].")    // imgui.h:2528
        .def_readwrite("user_callback_data", &ImDrawCmd::UserCallbackData, "4-8  // The draw callback code can access this.")    // imgui.h:2530
        .def(py::init<>(),    // imgui.h:2532
            "Also ensure our padding fields are zeroed")
        ;


    auto pyClassImDrawCmdHeader =
        py::class_<ImDrawCmdHeader>    // imgui.h:2555
        ...
        .def_readwrite("clip_rect", &ImDrawCmdHeader::ClipRect, "")    // imgui.h:2557
        .def_readwrite("texture_id", &ImDrawCmdHeader::TextureId, "")    // imgui.h:2558
        .def_readwrite("vtx_offset", &ImDrawCmdHeader::VtxOffset, "")    // imgui.h:2559
        ;


    auto pyClassImDrawChannel =
        py::class_<ImDrawChannel>    // imgui.h:2563
    ...
    auto pyClassImDrawListSplitter =
        py::class_<ImDrawListSplitter>    // imgui.h:2572
        ...
        .def_readwrite("_current", &ImDrawListSplitter::_Current, "Current channel number (0)")    // imgui.h:2574
        .def_readwrite("_count", &ImDrawListSplitter::_Count, "Number of active channels (1+)")    // imgui.h:2575
        .def(py::init<>())    // imgui.h:2578
        .def("clear_free_memory",    // imgui.h:2581
            &ImDrawListSplitter::ClearFreeMemory)
        .def("split",    // imgui.h:2582
            &ImDrawListSplitter::Split, py::arg("draw_list"), py::arg("count"))
        .def("merge",    // imgui.h:2583
            &ImDrawListSplitter::Merge, py::arg("draw_list"))
        .def("set_current_channel",    // imgui.h:2584
            &ImDrawListSplitter::SetCurrentChannel, py::arg("draw_list"), py::arg("channel_idx"))
        ;


    py::enum_<ImDrawFlags_>(m, "ImDrawFlags_", py::arithmetic(), " Flags for ImDrawList functions\n (Legacy: bit 0 must always correspond to ImDrawFlags_Closed to be backward compatible with old API using a bool. Bits 1..3 must be unused)")    // imgui.h:2589
    ...
    auto pyClassImDrawList =
        py::class_<ImDrawList>    // imgui.h:2627
        ...
        .def_readwrite("flags", &ImDrawList::Flags, "Flags, you may poke into these to adjust anti-aliasing settings per-primitive.")    // imgui.h:2633
        .def_readwrite("_vtx_current_idx", &ImDrawList::_VtxCurrentIdx, "[Internal] generally == VtxBuffer.Size unless we are past 64K vertices, in which case this gets reset to 0.")    // imgui.h:2636
        .def_readwrite("_data", &ImDrawList::_Data, "Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui context)")    // imgui.h:2637
        .def_readonly("_owner_name", &ImDrawList::_OwnerName, "Pointer to owner window's name for debugging")    // imgui.h:2638
        .def_readwrite("_vtx_write_ptr", &ImDrawList::_VtxWritePtr, "[Internal] point within VtxBuffer.Data after each add command (to avoid using the ImVector<> operators too much)")    // imgui.h:2639
        .def_readwrite("_idx_write_ptr", &ImDrawList::_IdxWritePtr, "[Internal] point within IdxBuffer.Data after each add command (to avoid using the ImVector<> operators too much)")    // imgui.h:2640
        .def_readwrite("_cmd_header", &ImDrawList::_CmdHeader, "[Internal] template of active commands. Fields should match those of CmdBuffer.back().")    // imgui.h:2644
        .def_readwrite("_splitter", &ImDrawList::_Splitter, "[Internal] for channels api (note: prefer using your own persistent instance of ImDrawListSplitter!)")    // imgui.h:2645
        .def_readwrite("_fringe_scale", &ImDrawList::_FringeScale, "[Internal] anti-alias fringe is scaled by this value, this helps to keep things sharp while zooming at vertex buffer content")    // imgui.h:2646
        .def(py::init<ImDrawListSharedData *>(),    // imgui.h:2649
            py::arg("shared_data"),
            "If you want to create ImDrawList instances, pass them ImGui::GetDrawListSharedData() or create and use your own ImDrawListSharedData (so you can use ImDrawList without ImGui)")
        .def("push_clip_rect",    // imgui.h:2652
        ...
        .def("add_image_quad",    // imgui.h:2691
        ...
        .def("add_image_rounded",    // imgui.h:2692
        ...
        .def("clone_output",    // imgui.h:2710
            &ImDrawList::CloneOutput,
            "Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer.",
            pybind11::return_value_policy::reference)
        .def("prim_reserve",    // imgui.h:2725
            &ImDrawList::PrimReserve, py::arg("idx_count"), py::arg("vtx_count"))
        .def("prim_unreserve",    // imgui.h:2726
            &ImDrawList::PrimUnreserve, py::arg("idx_count"), py::arg("vtx_count"))
        .def("prim_rect",    // imgui.h:2727
            &ImDrawList::PrimRect,
            py::arg("a"), py::arg("b"), py::arg("col"),
            "Axis aligned rectangle (composed of two triangles)")
        .def("prim_rect_uv",    // imgui.h:2728
            &ImDrawList::PrimRectUV, py::arg("a"), py::arg("b"), py::arg("uv_a"), py::arg("uv_b"), py::arg("col"))
        .def("prim_quad_uv",    // imgui.h:2729
            &ImDrawList::PrimQuadUV, py::arg("a"), py::arg("b"), py::arg("c"), py::arg("d"), py::arg("uv_a"), py::arg("uv_b"), py::arg("uv_c"), py::arg("uv_d"), py::arg("col"))
        .def("_reset_for_new_frame",    // imgui.h:2740
            &ImDrawList::_ResetForNewFrame)
        .def("_clear_free_memory",    // imgui.h:2741
            &ImDrawList::_ClearFreeMemory)
        .def("_pop_unused_draw_cmd",    // imgui.h:2742
            &ImDrawList::_PopUnusedDrawCmd)
        .def("_try_merge_draw_cmds",    // imgui.h:2743
            &ImDrawList::_TryMergeDrawCmds)
        .def("_on_changed_clip_rect",    // imgui.h:2744
            &ImDrawList::_OnChangedClipRect)
        .def("_on_changed_texture_id",    // imgui.h:2745
            &ImDrawList::_OnChangedTextureID)
        .def("_on_changed_vtx_offset",    // imgui.h:2746
            &ImDrawList::_OnChangedVtxOffset)
        .def("_calc_circle_auto_segment_count",    // imgui.h:2747
            &ImDrawList::_CalcCircleAutoSegmentCount, py::arg("radius"))
        .def("_path_arc_to_fast_ex",    // imgui.h:2748
            &ImDrawList::_PathArcToFastEx, py::arg("center"), py::arg("radius"), py::arg("a_min_sample"), py::arg("a_max_sample"), py::arg("a_step"))
        .def("_path_arc_to_n",    // imgui.h:2749
            &ImDrawList::_PathArcToN, py::arg("center"), py::arg("radius"), py::arg("a_min"), py::arg("a_max"), py::arg("num_segments"))
        ;


    auto pyClassImDrawData =
        py::class_<ImDrawData>    // imgui.h:2755
            (m, "ImDrawData", " All draw data to render a Dear ImGui frame\n (NB: the style and the naming convention here is a little inconsistent, we currently preserve them for backward compatibility purpose,\n as this is one of the oldest structure exposed by the library! Basically, ImDrawList == CmdList)")
        .def_readwrite("valid", &ImDrawData::Valid, "Only valid after Render() is called and before the next NewFrame() is called.")    // imgui.h:2757
        .def_readwrite("cmd_lists_count", &ImDrawData::CmdListsCount, "Number of ImDrawList* to render")    // imgui.h:2758
        .def_readwrite("total_idx_count", &ImDrawData::TotalIdxCount, "For convenience, sum of all ImDrawList's IdxBuffer.Size")    // imgui.h:2759
        .def_readwrite("total_vtx_count", &ImDrawData::TotalVtxCount, "For convenience, sum of all ImDrawList's VtxBuffer.Size")    // imgui.h:2760
        .def_readwrite("display_pos", &ImDrawData::DisplayPos, "Top-left position of the viewport to render (== top-left of the orthogonal projection matrix to use) (== GetMainViewport()->Pos for the main viewport, == (0.0) in most single-viewport applications)")    // imgui.h:2762
        .def_readwrite("display_size", &ImDrawData::DisplaySize, "Size of the viewport to render (== GetMainViewport()->Size for the main viewport, == io.DisplaySize in most single-viewport applications)")    // imgui.h:2763
        .def_readwrite("framebuffer_scale", &ImDrawData::FramebufferScale, "Amount of pixels for each unit of DisplaySize. Based on io.DisplayFramebufferScale. Generally (1,1) on normal display, (2,2) on OSX with Retina display.")    // imgui.h:2764
        .def_readwrite("owner_viewport", &ImDrawData::OwnerViewport, "Viewport carrying the ImDrawData instance, might be of use to the renderer (generally not).")    // imgui.h:2765
        .def(py::init<>(),    // imgui.h:2768
            "Functions")
        .def("de_index_all_buffers",    // imgui.h:2770
        ...
        .def("scale_clip_rects",    // imgui.h:2771
    ...
    auto pyClassImFontConfig =
        py::class_<ImFontConfig>    // imgui.h:2778
            (m, "ImFontConfig", "")
        .def_readwrite("font_data", &ImFontConfig::FontData, "// TTF/OTF data")    // imgui.h:2780
        .def_readwrite("font_data_size", &ImFontConfig::FontDataSize, "// TTF/OTF data size")    // imgui.h:2781
        .def_readwrite("font_data_owned_by_atlas", &ImFontConfig::FontDataOwnedByAtlas, "True     // TTF/OTF data ownership taken by the container ImFontAtlas (will delete memory itself).")    // imgui.h:2782
        .def_readwrite("font_no", &ImFontConfig::FontNo, "0        // Index of font within TTF/OTF file")    // imgui.h:2783
        .def_readwrite("size_pixels", &ImFontConfig::SizePixels, "// Size in pixels for rasterizer (more or less maps to the resulting font height).")    // imgui.h:2784
        .def_readwrite("oversample_h", &ImFontConfig::OversampleH, "3        // Rasterize at higher quality for sub-pixel positioning. Note the difference between 2 and 3 is minimal so you can reduce this to 2 to save memory. Read https://github.com/nothings/stb/blob/master/tests/oversample/README.md for details.")    // imgui.h:2785
        .def_readwrite("oversample_v", &ImFontConfig::OversampleV, "1        // Rasterize at higher quality for sub-pixel positioning. This is not really useful as we don't use sub-pixel positions on the Y axis.")    // imgui.h:2786
        .def_readwrite("pixel_snap_h", &ImFontConfig::PixelSnapH, "False    // Align every glyph to pixel boundary. Useful e.g. if you are merging a non-pixel aligned font with the default font. If enabled, you can set OversampleH/V to 1.")    // imgui.h:2787
        .def_readwrite("glyph_extra_spacing", &ImFontConfig::GlyphExtraSpacing, "0, 0     // Extra spacing (in pixels) between glyphs. Only X axis is supported for now.")    // imgui.h:2788
        .def_readwrite("glyph_offset", &ImFontConfig::GlyphOffset, "0, 0     // Offset all glyphs from this font input.")    // imgui.h:2789
        .def_readwrite("glyph_min_advance_x", &ImFontConfig::GlyphMinAdvanceX, "0        // Minimum AdvanceX for glyphs, set Min to align font icons, set both Min/Max to enforce mono-space font")    // imgui.h:2791
        .def_readwrite("glyph_max_advance_x", &ImFontConfig::GlyphMaxAdvanceX, "FLT_MAX  // Maximum AdvanceX for glyphs")    // imgui.h:2792
        .def_readwrite("merge_mode", &ImFontConfig::MergeMode, "False    // Merge into previous ImFont, so you can combine multiple inputs font into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want to use GlyphOffset.y when merge font of different heights.")    // imgui.h:2793
        .def_readwrite("font_builder_flags", &ImFontConfig::FontBuilderFlags, "0        // Settings for custom font builder. THIS IS BUILDER IMPLEMENTATION DEPENDENT. Leave as zero if unsure.")    // imgui.h:2794
        .def_readwrite("rasterizer_multiply", &ImFontConfig::RasterizerMultiply, "1.0     // Brighten (>1.0) or darken (<1.0) font output. Brightening small fonts may be a good workaround to make them more readable.")    // imgui.h:2795
        .def_readwrite("ellipsis_char", &ImFontConfig::EllipsisChar, "-1       // Explicitly specify unicode codepoint of ellipsis character. When fonts are being merged first specified ellipsis will be used.")    // imgui.h:2796
        .def_readwrite("dst_font", &ImFontConfig::DstFont, "")    // imgui.h:2800
        .def(py::init<>())    // imgui.h:2802
        ;


    auto pyClassImFontGlyph =
        py::class_<ImFontGlyph>    // imgui.h:2807
        ...
        .def_readwrite("advance_x", &ImFontGlyph::AdvanceX, "Distance to next character (= data from font + ImFontConfig::GlyphExtraSpacing.x baked in)")    // imgui.h:2812
        .def_readwrite("x0", &ImFontGlyph::X0, "Glyph corners")    // imgui.h:2813
        .def_readwrite("y0", &ImFontGlyph::Y0, "Glyph corners")    // imgui.h:2813
        .def_readwrite("x1", &ImFontGlyph::X1, "Glyph corners")    // imgui.h:2813
        .def_readwrite("y1", &ImFontGlyph::Y1, "Glyph corners")    // imgui.h:2813
        .def_readwrite("u0", &ImFontGlyph::U0, "Texture coordinates")    // imgui.h:2814
        .def_readwrite("v0", &ImFontGlyph::V0, "Texture coordinates")    // imgui.h:2814
        .def_readwrite("u1", &ImFontGlyph::U1, "Texture coordinates")    // imgui.h:2814
        .def_readwrite("v1", &ImFontGlyph::V1, "Texture coordinates")    // imgui.h:2814
        ;


    auto pyClassImFontGlyphRangesBuilder =
        py::class_<ImFontGlyphRangesBuilder>    // imgui.h:2819
        ...
        .def(py::init<>())    // imgui.h:2823
        .def("add_text",    // imgui.h:2828
            &ImFontGlyphRangesBuilder::AddText,
            py::arg("text"), py::arg("text_end") = py::none(),
            "Add string (each character of the UTF-8 string are added)")
        .def("add_ranges",    // imgui.h:2829
            &ImFontGlyphRangesBuilder::AddRanges,
            py::arg("ranges"),
            "Add ranges, e.g. builder.AddRanges(ImFontAtlas::GetGlyphRangesDefault()) to force add all of ASCII/Latin+Ext")
        .def("build_ranges",    // imgui.h:2830
    ...
    auto pyClassImFontAtlasCustomRect =
        py::class_<ImFontAtlasCustomRect>    // imgui.h:2834
            (m, "ImFontAtlasCustomRect", "See ImFontAtlas::AddCustomRectXXX functions.")
        .def_readwrite("width", &ImFontAtlasCustomRect::Width, "Input    // Desired rectangle dimension")    // imgui.h:2836
        .def_readwrite("height", &ImFontAtlasCustomRect::Height, "Input    // Desired rectangle dimension")    // imgui.h:2836
        .def_readwrite("x", &ImFontAtlasCustomRect::X, "Output   // Packed position in Atlas")    // imgui.h:2837
        .def_readwrite("y", &ImFontAtlasCustomRect::Y, "Output   // Packed position in Atlas")    // imgui.h:2837
        .def_readwrite("glyph_id", &ImFontAtlasCustomRect::GlyphID, "Input    // For custom font glyphs only (ID < 0x110000)")    // imgui.h:2838
        .def_readwrite("glyph_advance_x", &ImFontAtlasCustomRect::GlyphAdvanceX, "Input    // For custom font glyphs only: glyph xadvance")    // imgui.h:2839
        .def_readwrite("glyph_offset", &ImFontAtlasCustomRect::GlyphOffset, "Input    // For custom font glyphs only: glyph display offset")    // imgui.h:2840
        .def_readwrite("font", &ImFontAtlasCustomRect::Font, "Input    // For custom font glyphs only: target font")    // imgui.h:2841
        .def(py::init<>())    // imgui.h:2842
        ;


    py::enum_<ImFontAtlasFlags_>(m, "ImFontAtlasFlags_", py::arithmetic(), "Flags for ImFontAtlas build")    // imgui.h:2847
    ...
    auto pyClassImFontAtlas =
        py::class_<ImFontAtlas>    // imgui.h:2872
        ...
        .def("add_font",    // imgui.h:2876
            &ImFontAtlas::AddFont,
            py::arg("font_cfg"),
            pybind11::return_value_policy::reference)
        .def("add_font_default",    // imgui.h:2877
            &ImFontAtlas::AddFontDefault,
            py::arg("font_cfg") = py::none(),
            pybind11::return_value_policy::reference)
        .def("add_font_from_memory_ttf",    // imgui.h:2879
            ...
            pybind11::return_value_policy::reference)
        .def("add_font_from_memory_compressed_ttf",    // imgui.h:2880
            ...
            pybind11::return_value_policy::reference)
        .def("add_font_from_memory_compressed_base85_ttf",    // imgui.h:2881
            ...
            pybind11::return_value_policy::reference)
        .def("clear_input_data",    // imgui.h:2882
        ...
        .def("clear_tex_data",    // imgui.h:2883
        ...
        .def("calc_custom_rect_uv",    // imgui.h:2931
            &ImFontAtlas::CalcCustomRectUV, py::arg("rect"), py::arg("out_uv_min"), py::arg("out_uv_max"))
        .def("get_mouse_cursor_tex_data",    // imgui.h:2932
        ...
        .def_readwrite("flags", &ImFontAtlas::Flags, "Build flags (see ImFontAtlasFlags_)")    // imgui.h:2938
        .def_readwrite("tex_id", &ImFontAtlas::TexID, "User data to refer to the texture once it has been uploaded to user's graphic systems. It is passed back to you during rendering via the ImDrawCmd structure.")    // imgui.h:2939
        .def_readwrite("tex_desired_width", &ImFontAtlas::TexDesiredWidth, "Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics API have texture size restrictions you may want to increase texture width to decrease height.")    // imgui.h:2940
        .def_readwrite("tex_glyph_padding", &ImFontAtlas::TexGlyphPadding, "Padding between glyphs within texture in pixels. Defaults to 1. If your rendering method doesn't rely on bilinear filtering you may set this to 0 (will also need to set AntiAliasedLinesUseTex = False).")    // imgui.h:2941
        .def_readwrite("locked", &ImFontAtlas::Locked, "Marked as Locked by ImGui::NewFrame() so attempt to modify the atlas will assert.")    // imgui.h:2942
        .def_readwrite("user_data", &ImFontAtlas::UserData, "Store your own atlas related user-data (if e.g. you have multiple font atlas).")    // imgui.h:2943
        .def_readwrite("tex_ready", &ImFontAtlas::TexReady, "Set when texture was built matching current font input")    // imgui.h:2947
        .def_readwrite("tex_pixels_use_colors", &ImFontAtlas::TexPixelsUseColors, "Tell whether our texture data is known to use colors (rather than just alpha channel), in order to help backend select a format.")    // imgui.h:2948
        .def_readwrite("tex_width", &ImFontAtlas::TexWidth, "Texture width calculated during Build().")    // imgui.h:2951
        .def_readwrite("tex_height", &ImFontAtlas::TexHeight, "Texture height calculated during Build().")    // imgui.h:2952
        .def_readwrite("tex_uv_scale", &ImFontAtlas::TexUvScale, "= (1.0/TexWidth, 1.0/TexHeight)")    // imgui.h:2953
        .def_readwrite("tex_uv_white_pixel", &ImFontAtlas::TexUvWhitePixel, "Texture coordinates to a white pixel")    // imgui.h:2954
        .def_readonly("font_builder_io", &ImFontAtlas::FontBuilderIO, "Opaque interface to a font builder (default to stb_truetype, can be changed to use FreeType by defining IMGUI_ENABLE_FREETYPE).")    // imgui.h:2961
        .def_readwrite("font_builder_flags", &ImFontAtlas::FontBuilderFlags, "Shared flags (for all fonts) for custom font builder. THIS IS BUILD IMPLEMENTATION DEPENDENT. Per-font override is also available in ImFontConfig.")    // imgui.h:2962
        .def_readwrite("pack_id_mouse_cursors", &ImFontAtlas::PackIdMouseCursors, "Custom texture rectangle ID for white pixel and mouse cursors")    // imgui.h:2965
        .def_readwrite("pack_id_lines", &ImFontAtlas::PackIdLines, "Custom texture rectangle ID for baked anti-aliased lines")    // imgui.h:2966
        ;


    auto pyClassImFont =
        py::class_<ImFont>    // imgui.h:2975
            (m, "ImFont", " Font runtime data and rendering\n ImFontAtlas automatically loads a default embedded font for you when you call GetTexDataAsAlpha8() or GetTexDataAsRGBA32().")
        .def_readwrite("fallback_advance_x", &ImFont::FallbackAdvanceX, "4     // out // = FallbackGlyph->AdvanceX")    // imgui.h:2979
        .def_readwrite("font_size", &ImFont::FontSize, "4     // in  //            // Height of characters/line, set during loading (don't change after loading)")    // imgui.h:2980
        .def_readonly("fallback_glyph", &ImFont::FallbackGlyph, "4-8   // out // = FindGlyph(FontFallbackChar)")    // imgui.h:2985
        .def_readwrite("container_atlas", &ImFont::ContainerAtlas, "4-8   // out //            // What we has been loaded into")    // imgui.h:2988
        .def_readonly("config_data", &ImFont::ConfigData, "4-8   // in  //            // Pointer within ContainerAtlas->ConfigData")    // imgui.h:2989
        .def_readwrite("config_data_count", &ImFont::ConfigDataCount, "2     // in  // ~ 1        // Number of ImFontConfig involved in creating this font. Bigger than 1 when merging multiple font sources into one ImFont.")    // imgui.h:2990
        .def_readwrite("fallback_char", &ImFont::FallbackChar, "2     // out // = FFFD/'?' // Character used if a glyph isn't found.")    // imgui.h:2991
        .def_readwrite("ellipsis_char", &ImFont::EllipsisChar, "2     // out // = '...'    // Character used for ellipsis rendering.")    // imgui.h:2992
        .def_readwrite("dot_char", &ImFont::DotChar, "2     // out // = '.'      // Character used for ellipsis rendering (if a single '...' character isn't found)")    // imgui.h:2993
        .def_readwrite("dirty_lookup_tables", &ImFont::DirtyLookupTables, "1     // out //")    // imgui.h:2994
        .def_readwrite("scale", &ImFont::Scale, "4     // in  // = 1.      // Base font scale, multiplied by the per-window font scale which you can adjust with SetWindowFontScale()")    // imgui.h:2995
        .def_readwrite("ascent", &ImFont::Ascent, "4+4   // out //            // Ascent: distance from top to bottom of e.g. 'A' [0..FontSize]")    // imgui.h:2996
        .def_readwrite("descent", &ImFont::Descent, "4+4   // out //            // Ascent: distance from top to bottom of e.g. 'A' [0..FontSize]")    // imgui.h:2996
        .def_readwrite("metrics_total_surface", &ImFont::MetricsTotalSurface, "4     // out //            // Total surface in pixels to get an idea of the font rasterization/texture cost (not exact, we approximate the cost of padding between glyphs)")    // imgui.h:2997
        .def(py::init<>(),    // imgui.h:3001
            "Methods")
        .def("find_glyph",    // imgui.h:3003
            &ImFont::FindGlyph,
            py::arg("c"),
            pybind11::return_value_policy::reference)
        .def("find_glyph_no_fallback",    // imgui.h:3004
            &ImFont::FindGlyphNoFallback,
            py::arg("c"),
            pybind11::return_value_policy::reference)
        .def("calc_word_wrap_position_a",    // imgui.h:3012
            &ImFont::CalcWordWrapPositionA,
            py::arg("scale"), py::arg("text"), py::arg("text_end"), py::arg("wrap_width"),
            pybind11::return_value_policy::reference)
        .def("render_char",    // imgui.h:3013
            &ImFont::RenderChar, py::arg("draw_list"), py::arg("size"), py::arg("pos"), py::arg("col"), py::arg("c"))
        .def("render_text",    // imgui.h:3014
        ...
        .def("build_lookup_table",    // imgui.h:3017
            &ImFont::BuildLookupTable)
        .def("clear_output_data",    // imgui.h:3018
            &ImFont::ClearOutputData)
        .def("grow_index",    // imgui.h:3019
            &ImFont::GrowIndex, py::arg("new_size"))
        .def("add_glyph",    // imgui.h:3020
        ...
        .def("add_remap_char",    // imgui.h:3021
            &ImFont::AddRemapChar,
            py::arg("dst"), py::arg("src"), py::arg("overwrite_dst") = true,
            "Makes 'dst' character/glyph points to 'src' character/glyph. Currently needs to be called AFTER fonts have been built.")
        .def("set_glyph_visible",    // imgui.h:3022
            &ImFont::SetGlyphVisible, py::arg("c"), py::arg("visible"))
        .def("is_glyph_range_unused",    // imgui.h:3023
            &ImFont::IsGlyphRangeUnused, py::arg("c_begin"), py::arg("c_last"))
        ;


    py::enum_<ImGuiViewportFlags_>(m, "ViewportFlags_", py::arithmetic(), "Flags stored in ImGuiViewport::Flags, giving indications to the platform backends.")    // imgui.h:3031
    ...
    auto pyClassImGuiViewport =
        py::class_<ImGuiViewport>    // imgui.h:3056
        ...
        .def_readwrite("id_", &ImGuiViewport::ID, "Unique identifier for the viewport")    // imgui.h:3058
        .def_readwrite("flags", &ImGuiViewport::Flags, "See ImGuiViewportFlags_")    // imgui.h:3059
        .def_readwrite("pos", &ImGuiViewport::Pos, "Main Area: Position of the viewport (Dear ImGui coordinates are the same as OS desktop/native coordinates)")    // imgui.h:3060
        .def_readwrite("size", &ImGuiViewport::Size, "Main Area: Size of the viewport.")    // imgui.h:3061
        .def_readwrite("work_pos", &ImGuiViewport::WorkPos, "Work Area: Position of the viewport minus task bars, menus bars, status bars (>= Pos)")    // imgui.h:3062
        .def_readwrite("work_size", &ImGuiViewport::WorkSize, "Work Area: Size of the viewport minus task bars, menu bars, status bars (<= Size)")    // imgui.h:3063
        .def_readwrite("dpi_scale", &ImGuiViewport::DpiScale, "1.0 = 96 DPI = No extra scale.")    // imgui.h:3064
        .def_readwrite("parent_viewport_id", &ImGuiViewport::ParentViewportId, "(Advanced) 0: no parent. Instruct the platform backend to setup a parent/child relationship between platform windows.")    // imgui.h:3065
        .def_readwrite("draw_data", &ImGuiViewport::DrawData, "The ImDrawData corresponding to this viewport. Valid after Render() and until the next call to NewFrame().")    // imgui.h:3066
        .def_readwrite("renderer_user_data", &ImGuiViewport::RendererUserData, "None* to hold custom data structure for the renderer (e.g. swap chain, framebuffers etc.). generally set by your Renderer_CreateWindow function.")    // imgui.h:3073
        .def_readwrite("platform_user_data", &ImGuiViewport::PlatformUserData, "None* to hold custom data structure for the OS / platform (e.g. windowing info, render context). generally set by your Platform_CreateWindow function.")    // imgui.h:3074
        .def_readwrite("platform_handle", &ImGuiViewport::PlatformHandle, "None* for FindViewportByPlatformHandle(). (e.g. suggested to use natural platform handle such as HWND, GLFWWindow*, SDL_Window*)")    // imgui.h:3075
        .def_readwrite("platform_handle_raw", &ImGuiViewport::PlatformHandleRaw, "None* to hold lower-level, platform-native window handle (under Win32 this is expected to be a HWND, unused for other platforms), when using an abstraction layer like GLFW or SDL (where PlatformHandle would be a SDL_Window*)")    // imgui.h:3076
        .def_readwrite("platform_window_created", &ImGuiViewport::PlatformWindowCreated, "Platform window has been created (Platform_CreateWindow() has been called). This is False during the first frame where a viewport is being created.")    // imgui.h:3077
        .def_readwrite("platform_request_move", &ImGuiViewport::PlatformRequestMove, "Platform window requested move (e.g. window was moved by the OS / host window manager, authoritative position will be OS window position)")    // imgui.h:3078
        .def_readwrite("platform_request_resize", &ImGuiViewport::PlatformRequestResize, "Platform window requested resize (e.g. window was resized by the OS / host window manager, authoritative size will be OS window size)")    // imgui.h:3079
        .def_readwrite("platform_request_close", &ImGuiViewport::PlatformRequestClose, "Platform window requested closure (e.g. window was moved by the OS / host window manager, e.g. pressing ALT-F4)")    // imgui.h:3080
        .def(py::init<>())    // imgui.h:3082
        ;


    auto pyClassImGuiPlatformIO =
        py::class_<ImGuiPlatformIO>    // imgui.h:3140
    ...
    auto pyClassImGuiPlatformMonitor =
        py::class_<ImGuiPlatformMonitor>    // imgui.h:3203
        ...
        .def_readwrite("main_pos", &ImGuiPlatformMonitor::MainPos, "Coordinates of the area displayed on this monitor (Min = upper left, Max = bottom right)")    // imgui.h:3205
        .def_readwrite("main_size", &ImGuiPlatformMonitor::MainSize, "Coordinates of the area displayed on this monitor (Min = upper left, Max = bottom right)")    // imgui.h:3205
        .def_readwrite("work_pos", &ImGuiPlatformMonitor::WorkPos, "Coordinates without task bars / side bars / menu bars. Used to avoid positioning popups/tooltips inside this region. If you don't have this info, please copy the value for MainPos/MainSize.")    // imgui.h:3206
        .def_readwrite("work_size", &ImGuiPlatformMonitor::WorkSize, "Coordinates without task bars / side bars / menu bars. Used to avoid positioning popups/tooltips inside this region. If you don't have this info, please copy the value for MainPos/MainSize.")    // imgui.h:3206
        .def_readwrite("dpi_scale", &ImGuiPlatformMonitor::DpiScale, "1.0 = 96 DPI")    // imgui.h:3207
        .def(py::init<>())    // imgui.h:3208
        ;


    auto pyClassImGuiPlatformImeData =
        py::class_<ImGuiPlatformImeData>    // imgui.h:3212
            (m, "PlatformImeData", "(Optional) Support for IME (Input Method Editor) via the io.SetPlatformImeDataFn() function.")
        .def_readwrite("want_visible", &ImGuiPlatformImeData::WantVisible, "A widget wants the IME to be visible")    // imgui.h:3214
        .def_readwrite("input_pos", &ImGuiPlatformImeData::InputPos, "Position of the input cursor")    // imgui.h:3215
        .def_readwrite("input_line_height", &ImGuiPlatformImeData::InputLineHeight, "Line height")    // imgui.h:3216
