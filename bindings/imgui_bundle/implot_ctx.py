...
- `implot.begin...()` and `implot.end...()`
  can be replaced by:
  >>> with implot_ctx.begin...() as plot:
  ...     if plot: ...
  Do note that the context manager returns a boolean indicating whether the plot is visible or not,
  and thus you should check if the plot evaluates to True, just like the `if plot:` in the example above.
  See the `implot_ctx.begin_plot()` function doc for more details & examples if required.
...
- `implot.push...()` and `implot.pop...()`
  ...
  See the `implot_ctx.push_style_color()` function doc for more details & examples if required.
"""
