...
) -> Tuple[float, float, float]:  # imgui.h:903
    pass
...
) -> Tuple[float, float, float]:  # imgui.h:904
    pass
...
class ImGuiTableFlags_(enum.Enum):  # imgui.h:1179
    ...
    scroll_x = (
        enum.auto()
    )  # (= 1 << 24)  # Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to specify the container size. Changes default sizing policy. Because this create a child window, ScrollY is currently generally recommended when using ScrollX.
...
class ImGuiKey_(enum.Enum):  # imgui.h:1408
    ...
    gamepad_back = (
        enum.auto()
    )  # (= 618)  # View (Xbox)          - (Switch)   Share (PS)         // --
    # ImGuiKey_GamepadFaceUp,             /* original C++ signature */
    gamepad_face_up = (
        enum.auto()
    )  # (= 619)  # Y (Xbox)             X (Switch)   Triangle (PS)      // -> ImGuiNavInput_Input
    # ImGuiKey_GamepadFaceDown,           /* original C++ signature */
    gamepad_face_down = (
        enum.auto()
    )  # (= 620)  # A (Xbox)             B (Switch)   Cross (PS)         // -> ImGuiNavInput_Activate
    # ImGuiKey_GamepadFaceLeft,           /* original C++ signature */
    gamepad_face_left = (
        enum.auto()
    )  # (= 621)  # X (Xbox)             Y (Switch)   Square (PS)        // -> ImGuiNavInput_Menu
    # ImGuiKey_GamepadFaceRight,          /* original C++ signature */
    gamepad_face_right = (
        enum.auto()
    )  # (= 622)  # B (Xbox)             A (Switch)   Circle (PS)        // -> ImGuiNavInput_Cancel
    # ImGuiKey_GamepadDpadUp,             /* original C++ signature */
    gamepad_dpad_up = (
        enum.auto()
    )  # (= 623)  # D-pad Up                                             // -> ImGuiNavInput_DpadUp
    # ImGuiKey_GamepadDpadDown,           /* original C++ signature */
    gamepad_dpad_down = (
        enum.auto()
    )  # (= 624)  # D-pad Down                                           // -> ImGuiNavInput_DpadDown
    # ImGuiKey_GamepadDpadLeft,           /* original C++ signature */
    gamepad_dpad_left = (
        enum.auto()
    )  # (= 625)  # D-pad Left                                           // -> ImGuiNavInput_DpadLeft
    # ImGuiKey_GamepadDpadRight,          /* original C++ signature */
    gamepad_dpad_right = (
        enum.auto()
    )  # (= 626)  # D-pad Right                                          // -> ImGuiNavInput_DpadRight
    # ImGuiKey_GamepadL1,                 /* original C++ signature */
    gamepad_l1 = (
        enum.auto()
    )  # (= 627)  # L Bumper (Xbox)      L (Switch)   L1 (PS)            // -> ImGuiNavInput_FocusPrev + ImGuiNavInput_TweakSlow
    # ImGuiKey_GamepadR1,                 /* original C++ signature */
    gamepad_r1 = (
        enum.auto()
    )  # (= 628)  # R Bumper (Xbox)      R (Switch)   R1 (PS)            // -> ImGuiNavInput_FocusNext + ImGuiNavInput_TweakFast
    # ImGuiKey_GamepadL2,                 /* original C++ signature */
    gamepad_l2 = (
        enum.auto()
    )  # (= 629)  # L Trigger (Xbox)     ZL (Switch)  L2 (PS) [Analog]
    # ImGuiKey_GamepadR2,                 /* original C++ signature */
    gamepad_r2 = (
        enum.auto()
    )  # (= 630)  # R Trigger (Xbox)     ZR (Switch)  R2 (PS) [Analog]
...
class ImGuiNavInput_(enum.Enum):  # imgui.h:1534
    ...
    activate = (
        enum.auto()
    )  # (= 0)  # Activate / Open / Toggle / Tweak value       // e.g. Cross  (PS4), A (Xbox), A (Switch), Space (Keyboard)
    # ImGuiNavInput_Cancel,            /* original C++ signature */
    cancel = (
        enum.auto()
    )  # (= 1)  # Cancel / Close / Exit                        // e.g. Circle (PS4), B (Xbox), B (Switch), Escape (Keyboard)
    # ImGuiNavInput_Input,             /* original C++ signature */
    input = (
        enum.auto()
    )  # (= 2)  # Text input / On-Screen keyboard              // e.g. Triang.(PS4), Y (Xbox), X (Switch), Return (Keyboard)
    # ImGuiNavInput_Menu,              /* original C++ signature */
    menu = (
        enum.auto()
    )  # (= 3)  # Tap: Toggle menu / Hold: Focus, Move, Resize // e.g. Square (PS4), X (Xbox), Y (Switch), Alt (Keyboard)
    # ImGuiNavInput_DpadLeft,          /* original C++ signature */
    dpad_left = (
        enum.auto()
    )  # (= 4)  # Move / Tweak / Resize window (w/ PadMenu)    // e.g. D-pad Left/Right/Up/Down (Gamepads), Arrow keys (Keyboard)
    ...
    l_stick_left = (
        enum.auto()
    )  # (= 8)  # Scroll / Move window (w/ PadMenu)            // e.g. Left Analog Stick Left/Right/Up/Down
    ...
    focus_prev = (
        enum.auto()
    )  # (= 12)  # Focus Next window (w/ PadMenu)               // e.g. L1 or L2 (PS4), LB or LT (Xbox), L or ZL (Switch)
    # ImGuiNavInput_FocusNext,         /* original C++ signature */
    focus_next = (
        enum.auto()
    )  # (= 13)  # Focus Prev window (w/ PadMenu)               // e.g. R1 or R2 (PS4), RB or RT (Xbox), R or ZL (Switch)
    # ImGuiNavInput_TweakSlow,         /* original C++ signature */
    tweak_slow = (
        enum.auto()
    )  # (= 14)  # Slower tweaks                                // e.g. L1 or L2 (PS4), LB or LT (Xbox), L or ZL (Switch)
    # ImGuiNavInput_TweakFast,         /* original C++ signature */
    tweak_fast = (
        enum.auto()
    )  # (= 15)  # Faster tweaks                                // e.g. R1 or R2 (PS4), RB or RT (Xbox), R or ZL (Switch)
