...
# IMGUI_BUNDLE_BUILD_PYTHON is a compile time define that enables to add small modifications for the python
# binding provided by ImGui Bundle
...
def save_ini_settings_to_memory(out_ini_size: Optional[int] = None) -> str:
...
class Style:
    ...
    cell_padding: ImVec2  # Padding within a table cell
...
class IO:
    ...
    hover_delay_normal: float  # = 0.30 sec       // Delay on hovering before IsItemHovered(ImGuiHoveredFlags_DelayNormal) returns True.
    # float       HoverDelayShort;    /* original C++ signature */
    hover_delay_short: float  # = 0.10 sec       // Delay on hovering before IsItemHovered(ImGuiHoveredFlags_DelayShort) returns True.
    # void*       UserData;    /* original C++ signature */
    user_data: Any  # = None           // Store your own data for retrieval by callbacks.
    ...
    def add_mouse_wheel_event(self, wh_x: float, wh_y: float) -> None:
    ...
    metrics_active_allocations: int  # Number of active allocations, updated by MemAlloc/MemFree based on current context. May be off if you have multiple imgui contexts.
    ...
    mouse_wheel: float  # Mouse wheel Vertical: 1 unit scrolls about 5 lines text.
    # float       MouseWheelH;    /* original C++ signature */
    mouse_wheel_h: float  # Mouse wheel Horizontal. Most users don't have a mouse with a horizontal wheel, may not be filled by all backends.
...
class ListClipper:
    ...
    def force_display_range_by_indices(self, item_min: int, item_max: int) -> None:
...
class ImFontConfig:
    ...
    oversample_h: int  # 3        // Rasterize at higher quality for sub-pixel positioning. Note the difference between 2 and 3 is minimal so you can reduce this to 2 to save memory. Read https://github.com/nothings/stb/blob/master/tests/oversample/README.md for details.
...
class ImFontGlyphRangesBuilder:
    ...
    def add_ranges(self, ranges: ImWchar) -> None:
...
class ImFont:
    ...
    dot_char: ImWchar  # 2     // out // = '.'      // Character used for ellipsis rendering (if a single '...' character isn't found)
