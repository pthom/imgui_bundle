...
class TreeNodeFlags_(enum.Enum):
    ...
    nav_left_jumps_to_parent = (
        enum.auto()
    )  # (= 1 << 17)  # Nav: left arrow moves back to parent. This is processed in TreePop() when there's an unfullfilled Left nav request remaining.
...
class BackendFlags_(enum.Enum):
    ...
    renderer_has_textures = (
        enum.auto()
    )  # (= 1 << 4)  # Backend Renderer supports ImTextureData requests to create/update/destroy textures. This enables incremental texture updates and texture reloads.
...
class ImTextureFormat(enum.Enum):
...
class ImTextureStatus(enum.Enum):
...
class ImFontConfig:
    ...
    # ImS8            FontNo;    /* original C++ signature */
    font_no: ImS8  # 0        // Index of font within TTF/OTF file
    ...
    font_loader: ImFontLoader  # Custom font backend for this source (other use one stored in ImFontAtlas) # (const)
...
class ImFontBaked:
    ...
    baked_id: ID  # 4     //
    # ImFont*                     ContainerFont;    /* original C++ signature */
    container_font: ImFont  # 4-8   // in  // Parent font
...
class ImFontFlags_(enum.Enum):
    ...
    none = enum.auto()  # (= 0)
    # ImFontFlags_DefaultToLegacySize     = 1 << 0,       /* original C++ signature */
    default_to_legacy_size = (
        enum.auto()
    )  # (= 1 << 0)  # Legacy compatibility: make PushFont() calls without explicit size use font->LegacySize instead of current font size.
