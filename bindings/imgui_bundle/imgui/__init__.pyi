...
# IMGUI_API bool          OpenPopup(const char* str_id, ImGuiPopupFlags popup_flags = 0);                         /* original C++ signature */
@overload
def open_popup(str_id: str, popup_flags: PopupFlags = 0) -> bool:
    """call to mark popup as open (don't call every frame!)."""
    pass

# IMGUI_API bool          OpenPopup(ImGuiID id, ImGuiPopupFlags popup_flags = 0);                                 /* original C++ signature */
@overload
def open_popup(id_: ID, popup_flags: PopupFlags = 0) -> bool:
    """id overload to facilitate calling from nested stacks"""
    pass

# IMGUI_API bool          OpenPopupOnItemClick(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 0);       /* original C++ signature */
def open_popup_on_item_click(str_id: Optional[str] = None, popup_flags: PopupFlags = 0) -> bool:
...
# IMGUI_API int           GetItemClickedCountWithSingleClickDelay(ImGuiMouseButton mouse_button = 0, float delay = -1.0f);     /* original C++ signature */
def get_item_clicked_count_with_single_click_delay(mouse_button: MouseButton = 0, delay: float = -1.0) -> int:
...
# IMGUI_API bool          SetItemKeyOwner(ImGuiKey key);                                          /* original C++ signature */
def set_item_key_owner(key: Key) -> bool:
...
# IMGUI_API bool          IsMouseReleasedWithDelay(ImGuiMouseButton button, float delay=-1.f);    /* original C++ signature */
def is_mouse_released_with_delay(button: MouseButton, delay: float = -1.0) -> bool:
...
class ItemFlags_(enum.IntFlag):
    ...
    # ---------------------------------------------------------------------------------
    # ImGuiItemFlags_LiveEditOnInputText      = 1 << 7,       /* original C++ signature */
    live_edit_on_input_text = (
        enum.auto()
    )  # (= 1 << 7)  # True     // InputText: apply keyboard edits to backing value while typing. Otherwise, edits are applied when validating, tabbing out or losing focus.
    # ImGuiItemFlags_LiveEditOnInputScalar    = 1 << 8,       /* original C++ signature */
    live_edit_on_input_scalar = (
        enum.auto()
    )  # (= 1 << 8)  # False    // DragXXX, SliderXXX, InputScalar: apply keyboard edits to backing value while typing. Otherwise, edits are applied when validating, tabbing out or losing focus.
    # ImGuiItemFlags_LiveEditOnInput          = ImGuiItemFlags_LiveEditOnInputText | ImGuiItemFlags_LiveEditOnInputScalar,    /* original C++ signature */
    # }
    live_edit_on_input = enum.auto()  # (= ItemFlags_LiveEditOnInputText | ItemFlags_LiveEditOnInputScalar)
...
class InputTextFlags_(enum.IntFlag):
    ...
    enter_returns_true = (
        enum.auto()
    )  # (= 1 << 6)  # Return 'True' when Enter is pressed (as opposed to every time the value was modified). Consider disabling LiveEdit! or using IsItemDeactivatedAfterEdit() instead!
...
class TabBarFlags_(enum.IntFlag):
    ...
    fitting_policy_mixed = (
        enum.auto()
    )  # (= 1 << 7)  # Shrink down tabs when they don't fit, until width is style.TabMinWidthShrink, then enable scrolling. Setting TabMinWidthShrink to FLT_MAX makes this behave like ImGuiTabBarFlags_FittingPolicyScroll.
...
class Col_(enum.IntFlag):
    ...
    # ImGuiCol_CheckboxSelectedBg,        /* original C++ signature */
    checkbox_selected_bg = enum.auto()  # (= 19)  # Checkbox background when Selected, otherwise use FrameBg
    # ImGuiCol_SliderGrab,    /* original C++ signature */
    slider_grab = enum.auto()  # (= 20)
    # ImGuiCol_SliderGrabActive,    /* original C++ signature */
    slider_grab_active = enum.auto()  # (= 21)
    # ImGuiCol_Button,    /* original C++ signature */
    button = enum.auto()  # (= 22)
    # ImGuiCol_ButtonHovered,    /* original C++ signature */
    button_hovered = enum.auto()  # (= 23)
    # ImGuiCol_ButtonActive,    /* original C++ signature */
    button_active = enum.auto()  # (= 24)
    # ImGuiCol_Header,                    /* original C++ signature */
    header = enum.auto()  # (= 25)  # Header* colors are used for CollapsingHeader, TreeNode, Selectable, MenuItem
    # ImGuiCol_HeaderHovered,    /* original C++ signature */
    header_hovered = enum.auto()  # (= 26)
    # ImGuiCol_HeaderActive,    /* original C++ signature */
    header_active = enum.auto()  # (= 27)
    # ImGuiCol_Separator,    /* original C++ signature */
    separator = enum.auto()  # (= 28)
    # ImGuiCol_SeparatorHovered,    /* original C++ signature */
    separator_hovered = enum.auto()  # (= 29)
    # ImGuiCol_SeparatorActive,    /* original C++ signature */
    separator_active = enum.auto()  # (= 30)
    # ImGuiCol_ResizeGrip,                /* original C++ signature */
    resize_grip = enum.auto()  # (= 31)  # Resize grip in lower-right and lower-left corners of windows.
    # ImGuiCol_ResizeGripHovered,    /* original C++ signature */
    resize_grip_hovered = enum.auto()  # (= 32)
    # ImGuiCol_ResizeGripActive,    /* original C++ signature */
    resize_grip_active = enum.auto()  # (= 33)
    # ImGuiCol_InputTextCursor,           /* original C++ signature */
    input_text_cursor = enum.auto()  # (= 34)  # InputText cursor/caret
    # ImGuiCol_TabHovered,                /* original C++ signature */
    tab_hovered = enum.auto()  # (= 35)  # Tab background, when hovered
    # ImGuiCol_Tab,                       /* original C++ signature */
    tab = enum.auto()  # (= 36)  # Tab background, when tab-bar is focused & tab is unselected
    # ImGuiCol_TabSelected,               /* original C++ signature */
    tab_selected = enum.auto()  # (= 37)  # Tab background, when tab-bar is focused & tab is selected
    # ImGuiCol_TabSelectedOverline,       /* original C++ signature */
    tab_selected_overline = enum.auto()  # (= 38)  # Tab horizontal overline, when tab-bar is focused & tab is selected
    # ImGuiCol_TabDimmed,                 /* original C++ signature */
    tab_dimmed = enum.auto()  # (= 39)  # Tab background, when tab-bar is unfocused & tab is unselected
    # ImGuiCol_TabDimmedSelected,         /* original C++ signature */
    tab_dimmed_selected = enum.auto()  # (= 40)  # Tab background, when tab-bar is unfocused & tab is selected
    # ImGuiCol_TabDimmedSelectedOverline,    /* original C++ signature */
    tab_dimmed_selected_overline = (
        enum.auto()
    )  # (= 41)  #..horizontal overline, when tab-bar is unfocused & tab is selected
    # ImGuiCol_DockingPreview,            /* original C++ signature */
    docking_preview = enum.auto()  # (= 42)  # Preview overlay color when about to docking something
    # ImGuiCol_DockingEmptyBg,            /* original C++ signature */
    docking_empty_bg = (
        enum.auto()
    )  # (= 43)  # Background color for empty node (e.g. CentralNode with no window docked into it)
    # ImGuiCol_PlotLines,    /* original C++ signature */
    plot_lines = enum.auto()  # (= 44)
    # ImGuiCol_PlotLinesHovered,    /* original C++ signature */
    plot_lines_hovered = enum.auto()  # (= 45)
    # ImGuiCol_PlotHistogram,    /* original C++ signature */
    plot_histogram = enum.auto()  # (= 46)
    # ImGuiCol_PlotHistogramHovered,    /* original C++ signature */
    plot_histogram_hovered = enum.auto()  # (= 47)
    # ImGuiCol_TableHeaderBg,             /* original C++ signature */
    table_header_bg = enum.auto()  # (= 48)  # Table header background
    # ImGuiCol_TableBorderStrong,         /* original C++ signature */
    table_border_strong = enum.auto()  # (= 49)  # Table outer and header borders (prefer using Alpha=1.0 here)
    # ImGuiCol_TableBorderLight,          /* original C++ signature */
    table_border_light = enum.auto()  # (= 50)  # Table inner borders (prefer using Alpha=1.0 here)
    # ImGuiCol_TableRowBg,                /* original C++ signature */
    table_row_bg = enum.auto()  # (= 51)  # Table row background (even rows)
    # ImGuiCol_TableRowBgAlt,             /* original C++ signature */
    table_row_bg_alt = enum.auto()  # (= 52)  # Table row background (odd rows)
    # ImGuiCol_TextLink,                  /* original C++ signature */
    text_link = enum.auto()  # (= 53)  # Hyperlink color
    # ImGuiCol_TextSelectedBg,            /* original C++ signature */
    text_selected_bg = enum.auto()  # (= 54)  # Selected text inside an InputText
    # ImGuiCol_TreeLines,                 /* original C++ signature */
    tree_lines = enum.auto()  # (= 55)  # Tree node hierarchy outlines when using ImGuiTreeNodeFlags_DrawLines
    # ImGuiCol_DragDropTarget,            /* original C++ signature */
    drag_drop_target = enum.auto()  # (= 56)  # Rectangle border highlighting a drop target
    # ImGuiCol_DragDropTargetBg,          /* original C++ signature */
    drag_drop_target_bg = enum.auto()  # (= 57)  # Rectangle background highlighting a drop target
    # ImGuiCol_UnsavedMarker,             /* original C++ signature */
    unsaved_marker = enum.auto()  # (= 58)  # Unsaved Document marker (in window title and tabs)
    # ImGuiCol_NavCursor,                 /* original C++ signature */
    nav_cursor = enum.auto()  # (= 59)  # Color of keyboard/gamepad navigation cursor/rectangle, when visible
    # ImGuiCol_NavWindowingHighlight,     /* original C++ signature */
    nav_windowing_highlight = enum.auto()  # (= 60)  # Highlight window when using Ctrl+Tab
    # ImGuiCol_NavWindowingDimBg,         /* original C++ signature */
    nav_windowing_dim_bg = (
        enum.auto()
    )  # (= 61)  # Darken/colorize entire screen behind the Ctrl+Tab window list, when active
    # ImGuiCol_ModalWindowDimBg,          /* original C++ signature */
    modal_window_dim_bg = (
        enum.auto()
    )  # (= 62)  # Darken/colorize entire screen behind a modal window, when one is active
...
class ColorEditFlags_(enum.IntFlag):
    ...
    # ImGuiColorEditFlags_PickerNoRotate  = 1 << 27,      /* original C++ signature */
    picker_no_rotate = (
        enum.auto()
    )  # (= 1 << 27)  # [Picker]     // ColorPicker: disable rotating Sat/Value triangle. Best set in io.ConfigColorEditFlags once.
    # ImGuiColorEditFlags_InputRGB        = 1 << 28,      /* original C++ signature */
    input_rgb = (
        enum.auto()
    )  # (= 1 << 28)  # [Input]      // ColorEdit, ColorPicker: input and output data in RGB format.
    # ImGuiColorEditFlags_InputHSV        = 1 << 29,      /* original C++ signature */
    input_hsv = (
        enum.auto()
    )  # (= 1 << 29)  # [Input]      // ColorEdit, ColorPicker: input and output data in HSV format.
...
class Style:
    ...
    tab_close_button_min_width_selected: float  # -1: always visible. 0.0: visible when hovered. >0.0: visible when hovered if minimum width. FLT_MAX: never shrink, will behave like ImGuiTabBarFlags_FittingPolicyScroll.
    ...
    # float       InputTextCursorSize;    /* original C++ signature */
    input_text_cursor_size: float  # Thickness of cursor/caret in InputText().
    # float       SeparatorSize;    /* original C++ signature */
    separator_size: float  # Thickness of border in Separator(). Must be >= 1.0.
    ...
    anti_aliased_lines: bool  # Enable anti-aliased lines/borders. Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).    bool        AntiAliasedLines;           // Enable anti-aliased lines/borders. Used at the beginning of the frame to set ImDrawFlags_AALines in all draw-lists.
    # bool        AntiAliasedLineEnds;    /* original C++ signature */
    anti_aliased_line_ends: bool  # Enable anti-aliased lines/borders ends. Nicer for thick lines but more expensive. Used at the beginning of the frame to set ImDrawFlags_AALineEnds in all draw-lists.
    # bool        AntiAliasedFill;    /* original C++ signature */
    anti_aliased_fill: bool  # Enable anti-aliased edges around filled shapes (rounded rectangles, circles, etc.). Used at the beginning of the frame to set ImDrawFlags_AAFill in all draw-lists.
    # float       CurveTessellationMaxError;    /* original C++ signature */
    curve_tessellation_max_error: float  # Maximum error (in pixels) when using PathBezierCurveTo() without a specific number of segments. Decrease for highly tessellated curves (higher quality, more polygons), increase to reduce quality.
...
class IO:
    ...
    config_input_text_enter_keep_active: bool  # = False          // [BETA] Pressing Enter will reactivate item and select all text (single-line only). Ctrl+Enter or Shift+Enter will deactivate normally.
    # ImGuiColorEditFlags ConfigColorEditFlags;    /* original C++ signature */
    config_color_edit_flags: ColorEditFlags  # = <defaults>     // Current settings for ColorEdit/ColorPicker widgets. Must have one bit of ImGuiColorEditFlags_DisplayMask_, one bit of ImGuiColorEditFlags_DataTypeMask_, one bit of ImGuiColorEditFlags_PickerMask_, one bit of ImGuiColorEditFlags_InputMask_. Defaults to ImGuiColorEditFlags_DefaultOptions_. May be further edited by users, unless you also set ImGuiColorEditFlags_NoOptions.
    ...
    # bool        ConfigIniSettingsSaveLastUsedDate;    /* original C++ signature */
    config_ini_settings_save_last_used_date: bool  # = True         // Enable loading/saving last used day (YYYYMMDD) in some .ini struct, making things easier to audit and allowing custom tools to cleanup old data.
    # int         ConfigIniSettingsAutoDiscardMonths;    /* original C++ signature */
    config_ini_settings_auto_discard_months: int  # = 0          // [BETA] Set number of months after which unused .ini entries are discarded on load. Require platform_io.Platform_SessionDate to be set. For systems supporting the feature, .ini entries without a LastUsed field will always be discarded! Please report if you are using this.
    ...
    mouse_double_click_time: (
        float  # = 0.30          // Time for consecutive clicks to account as a double-click, in seconds.
    )
    # float       MouseDoubleClickMaxDist;    /* original C++ signature */
    mouse_double_click_max_dist: float  # = 6.0           // Distance threshold to stay in to validate a double-click or multiple clicks, in pixels.
    # float       MouseSingleClickDelay;    /* original C++ signature */
    mouse_single_click_delay: float  # = 0.60          // Time for a delayed click when using GetItemClickedCountWithSingleClickDelay() or IsMouseReleasedWithDelay(), in seconds. Must be > io.MouseDoubleClickTime.
...
class InputTextCallbackData:
    ...
    cursor_pos: int  #                                      // Read-write   // [Completion,History,Always,CharFilter]
    # int                 SelectionStart;    /* original C++ signature */
    selection_start: int  #                                      // Read-write   // [Completion,History,Always,CharFilter] == to SelectionEnd when no selection
    # int                 SelectionEnd;    /* original C++ signature */
    selection_end: int  #                                      // Read-write   // [Completion,History,Always,CharFilter]
...
class ImDrawFlags_(enum.IntFlag):
    ...
    round_corners_top_left = (
        enum.auto()
    )  # (= 1 << 4)  # OK   --    // Round top-left corner only (when 'rounding > 0.0', we default to all corners).
    # ImDrawFlags_RoundCornersTopRight    = 1 << 5,       /* original C++ signature */
    round_corners_top_right = (
        enum.auto()
    )  # (= 1 << 5)  # OK   --    // Round top-right corner only (when 'rounding > 0.0', we default to all corners).
    # ImDrawFlags_RoundCornersBottomLeft  = 1 << 6,       /* original C++ signature */
    round_corners_bottom_left = (
        enum.auto()
    )  # (= 1 << 6)  # OK   --    // Round bottom-left corner only (when 'rounding > 0.0', we default to all corners).
    # ImDrawFlags_RoundCornersBottomRight = 1 << 7,       /* original C++ signature */
    round_corners_bottom_right = (
        enum.auto()
    )  # (= 1 << 7)  # OK   --    // Round bottom-right corner only (when 'rounding > 0.0', we default to all corners).
    # ImDrawFlags_RoundCornersNone        = 1 << 8,       /* original C++ signature */
    round_corners_none = (
        enum.auto()
    )  # (= 1 << 8)  # OK   --    // Disable rounding even when 'rounding > 0.0'. This value is NOT zero, it is NOT an implicit flag!
    ...
    round_corners_all = (
        enum.auto()
    )  # (= ImDrawFlags_.round_corners_top_left | ImDrawFlags_.round_corners_top_right | ImDrawFlags_.round_corners_bottom_left | ImDrawFlags_.round_corners_bottom_right)  # (Default!!)
    ...
    # ImDrawFlags_Closed                  = 1 << 9,       /* original C++ signature */
    closed = (
        enum.auto()
    )  # (= 1 << 9)  # OK   --     // PathStroke(), AddPolyline(): specify that shape should be closed.
    # ImDrawFlags_JoinMiter               = 1 << 10,      /* original C++ signature */
    join_miter = (
        enum.auto()
    )  # (= 1 << 10)  # OK   --     // PathStroke(), AddPolyline(): use miter joins/corners only. This assumes that the input polyline does not have corners sharper than 90 degrees. Slightly faster.
    # ImDrawFlags_CapSquare               = 1 << 11,      /* original C++ signature */
    cap_square = enum.auto()  # (= 1 << 11)  # OK   --     // PathStroke(), AddPolyline(): use square cap line ends.
    ...
    # ImDrawFlags_AAFill                  = 1 << 12,      /* original C++ signature */
    aa_fill = enum.auto()  # (= 1 << 12)  # OK*  OK(1)  // Enable anti-aliasing for Filled shapes.
    # ImDrawFlags_AALines                 = 1 << 13,      /* original C++ signature */
    aa_lines = enum.auto()  # (= 1 << 13)  # OK*  OK(1)  // Enable anti-aliasing for Strokes (lines, borders).
    # ImDrawFlags_AALineEnds              = 1 << 14,      /* original C++ signature */
    aa_line_ends = (
        enum.auto()
    )  # (= 1 << 14)  # OK   OK(0)  // Enable anti-aliasing for Strokes Ends. Requires AALines to also be enabled. Useful on thick strokes or for precise continuity of multiple lines. A little more costly.
    ...
    # ImDrawFlags_StrokeInside            = 1 << 17,      /* original C++ signature */
    stroke_inside = (
        enum.auto()
    )  # (= 1 << 17)  # OK   --     // Draw stroke inside of the shape outline (default for closed shapes and AddLineH, AddLineV functions)
    # ImDrawFlags_StrokeCenter            = 2 << 17,      /* original C++ signature */
    stroke_center = (
        enum.auto()
    )  # (= 2 << 17)  # OK   --     // Draw stroke at the center of the shape outline (default for paths, bezier, and AddLine functions)
    # ImDrawFlags_StrokeCenterBiased      = 3 << 17,      /* original C++ signature */
    stroke_center_biased = (
        enum.auto()
    )  # (= 3 << 17)  # OK   --     // Draw stroke at the center of the shape outline, so that half thickness rounded down will be outside, and the rest inside the shape outline. Useful for axis-aligned shapes: AddLineH, AddLineV, AddRect. Does not animate well!
    # ImDrawFlags_StrokeOutside           = 4 << 17,      /* original C++ signature */
    stroke_outside = enum.auto()  # (= 4 << 17)  # OK   --     // Draw stroke outside of the shape outline
    # ImDrawFlags_StrokeLegacy            = 7 << 17,      /* original C++ signature */
    stroke_legacy = (
        enum.auto()
    )  # (= 7 << 17)  # OK   OK(0)  // Use legacy positioning + enable JoinMiter + disable AALineEnds. Must be all bits set.

    # Other Options ------------------------------ // Prim/Scope?
    # ImDrawFlags_TextNoPixelSnap         = 1 << 20,      /* original C++ signature */
    text_no_pixel_snap = (
        enum.auto()
    )  # (= 1 << 20)  # OK   OK(0)  // Disable automatically snapping AddText() calls to pixel boundaries.
    # ImDrawFlags_UseTexForRoundCorners   = 1 << 21,      /* original C++ signature */
    use_tex_for_round_corners = (
        enum.auto()
    )  # (= 1 << 21)  # --   OK(1)  // Enable using textures instead of strokes to draw rounded corners/circles where possible (faster). Used by default unless 'ImFontAtlasFlags_NoBakedRoundCorners' is enabled in the font atlas.
    # ImDrawFlags_UseVtxOffset            = 1 << 22,      /* original C++ signature */
    use_vtx_offset = (
        enum.auto()
    )  # (= 1 << 22)  # --   OK(1)  // Can emit 'VtxOffset > 0' to allow large meshes with 16-bit ImDrawIdx. Used by default when 'ImGuiBackendFlags_RendererHasVtxOffset' is enabled by the backend.
    # ImDrawFlags_AllowTexForRoundCorners_= 1 << 23,      /* original C++ signature */
    allow_tex_for_round_corners_ = enum.auto()  # (= 1 << 23)  # --   OK(1)  // [Internal]

    # [Internal]
    # ImDrawFlags_RoundCornersMask_       = ImDrawFlags_RoundCornersAll | ImDrawFlags_RoundCornersNone,     /* original C++ signature */
    round_corners_mask_ = (
        enum.auto()
    )  # (= ImDrawFlags_.round_corners_all | ImDrawFlags_.round_corners_none)  # [Internal]
    # ImDrawFlags_AllowInPushScope_       = ImDrawFlags_AAFill | ImDrawFlags_AALines | ImDrawFlags_AALineEnds | ImDrawFlags_StrokeLegacy | ImDrawFlags_TextNoPixelSnap | ImDrawFlags_UseTexForRoundCorners | ImDrawFlags_UseVtxOffset | ImDrawFlags_RoundCornersMask_,     /* original C++ signature */
    allow_in_push_scope_ = (
        enum.auto()
    )  # (= ImDrawFlags_.aa_fill | ImDrawFlags_.aa_lines | ImDrawFlags_.aa_line_ends | ImDrawFlags_.stroke_legacy | ImDrawFlags_.text_no_pixel_snap | ImDrawFlags_.use_tex_for_round_corners | ImDrawFlags_.use_vtx_offset | ImDrawFlags_.round_corners_mask_)  # [Internal] Values allowed in PushDrawFlag() scope.
    # ImDrawFlags_AllowInFrameScope_      = ImDrawFlags_AllowInPushScope_ | ImDrawFlags_AllowTexForRoundCorners_,    /* original C++ signature */
    allow_in_frame_scope_ = (
        enum.auto()
    )  # (= ImDrawFlags_.allow_in_push_scope_ | ImDrawFlags_.allow_tex_for_round_corners_)
    # ImDrawFlags_StrokeMask_             = 0x07 << 17,                  /* original C++ signature */
    stroke_mask_ = enum.auto()  # (= 0x07 << 17)  # [Internal]
    # ImDrawFlags_InvalidMask_            = ~0x7FFFFFF0,                 /* original C++ signature */
    invalid_mask_ = enum.auto()  # (= ~0x7FFFFFF0)  # [Internal] == 0x8000000F. Reserved to detect misuses.
...
class ImDrawList:
    ...
    # ImDrawFlags             Flags;    /* original C++ signature */
    flags: ImDrawFlags  # Current flags for drawing primitives. You may poke into these to adjust anti-aliasing settings per-primitive. Alter with PushDrawFlag().
    ...
    # float                   _InvFringeScale;    /* original C++ signature */
    _inv_fringe_scale: float  # [internal] 1.0 / _FringeScale // FIXME: Consider renaming to _PixelDensity.
    # bool                    _FringeScaleIsInteger;    /* original C++ signature */
    _fringe_scale_is_integer: (
        bool  # [Internal] True if 1/_FringeScale is a whole number, used to select fast path for rendering
    )
    ...
    def pop_texture(self) -> None:
        pass
    # IMGUI_API void  PushDrawFlag(ImDrawFlags flags, bool enabled);     /* original C++ signature */
    def push_draw_flag(self, flags: ImDrawFlags, enabled: bool) -> None:
        """[BETA] Please notify me if you are using this."""
        pass
    # IMGUI_API void  PopDrawFlag();    /* original C++ signature */
    def pop_draw_flag(self) -> None:
        pass
    ...
    # IMGUI_API void  AddLine(const ImVec2& p1, const ImVec2& p2, ImU32 col, float thickness = 1.0f, ImDrawFlags flags = 0);    /* original C++ signature */
    def add_line(
        self, p1: ImVec2Like, p2: ImVec2Like, col: ImU32, thickness: float = 1.0, flags: ImDrawFlags = 0
    ) -> None:
        pass
    # IMGUI_API void  AddLineH(float min_x, float max_x, float y, ImU32 col, float thickness = 1.0f, ImDrawFlags flags = 0);    /* original C++ signature */
    def add_line_h(
        self, min_x: float, max_x: float, y: float, col: ImU32, thickness: float = 1.0, flags: ImDrawFlags = 0
    ) -> None:
        pass
    # IMGUI_API void  AddRect(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f, float thickness = 1.0f, ImDrawFlags flags = 0);       /* original C++ signature */
    def add_rect(
        self,
        p_min: ImVec2Like,
        p_max: ImVec2Like,
    ...
    def add_rect_filled_multi_color(
        ...
        pass
    # IMGUI_API void  AddQuad(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness = 1.0f, ImDrawFlags flags = 0);    /* original C++ signature */
    def add_quad(
        self,
        p1: ImVec2Like,
        p2: ImVec2Like,
    ...
    def add_quad_filled(self, p1: ImVec2Like, p2: ImVec2Like, p3: ImVec2Like, p4: ImVec2Like, col: ImU32) -> None:
        pass
    # IMGUI_API void  AddTriangle(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImU32 col, float thickness = 1.0f, ImDrawFlags flags = 0);    /* original C++ signature */
    def add_triangle(
        self, p1: ImVec2Like, p2: ImVec2Like, p3: ImVec2Like, col: ImU32, thickness: float = 1.0, flags: ImDrawFlags = 0
    ) -> None:
        pass
    ...
    def add_triangle_filled(self, p1: ImVec2Like, p2: ImVec2Like, p3: ImVec2Like, col: ImU32) -> None:
        pass
    # IMGUI_API void  AddCircle(const ImVec2& center, float radius, ImU32 col, int num_segments = 0, float thickness = 1.0f, ImDrawFlags flags = 0);    /* original C++ signature */
    def add_circle(
        self,
        center: ImVec2Like,
        radius: float,
    ...
    def add_circle_filled(self, center: ImVec2Like, radius: float, col: ImU32, num_segments: int = 0) -> None:
        pass
    # IMGUI_API void  AddNgon(const ImVec2& center, float radius, ImU32 col, int num_segments, float thickness = 1.0f, ImDrawFlags flags = 0);    /* original C++ signature */
    def add_ngon(
        self,
        center: ImVec2Like,
        radius: float,
    ...
    def add_ngon_filled(self, center: ImVec2Like, radius: float, col: ImU32, num_segments: int) -> None:
        pass
    # IMGUI_API void  AddEllipse(const ImVec2& center, const ImVec2& radius, ImU32 col, float rot = 0.0f, int num_segments = 0, float thickness = 1.0f, ImDrawFlags flags = 0);    /* original C++ signature */
    def add_ellipse(
        self,
        center: ImVec2Like,
        radius: ImVec2Like,
    ...
    def add_text(
        ...
        pass
    # IMGUI_API void  AddBezierCubic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int num_segments = 0, ImDrawFlags flags = 0);     /* original C++ signature */
    def add_bezier_cubic(
        ...
        pass
    # IMGUI_API void  AddBezierQuadratic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImU32 col, float thickness, int num_segments = 0, ImDrawFlags flags = 0);                   /* original C++ signature */
    ...
    def path_line_to_merge_duplicate(self, pos: ImVec2Like) -> None:
        """(private API)"""
        pass
    # inline    void  PathFillConvex(ImU32 col, ImDrawFlags flags = 0)            { AddConvexPolyFilled(_Path.Data, _Path.Size, col, flags); _Path.Size = 0; }    /* original C++ signature */
    def path_fill_convex(self, col: ImU32, flags: ImDrawFlags = 0) -> None:
        """(private API)"""
        pass
    ...
    def path_fill_concave(self, col: ImU32) -> None:
        """(private API)"""
        pass
    # inline    void  PathStroke(ImU32 col, float thickness = 1.0f, ImDrawFlags flags = 0) { _AddPolyline(_Path.Data, _Path.Size, col, thickness, flags, FLT_MAX); _Path.Size = 0; }    /* original C++ signature */
    ...
    def _reset_for_new_frame(self) -> None:
        pass
    # IMGUI_API void  _SetPixelDensity(float pixel_density);    /* original C++ signature */
    def _set_pixel_density(self, pixel_density: float) -> None:
        pass
    ...
    def _path_arc_to_n(self, center: ImVec2Like, radius: float, a_min: float, a_max: float, num_segments: int) -> None:
        pass
    # IMGUI_API void  _AddRectFilledBaked(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float r, const ImVec4& tex_uvs, ImDrawFlags flags);    /* original C++ signature */
    def _add_rect_filled_baked(
        self, p_min: ImVec2Like, p_max: ImVec2Like, col: ImU32, r: float, tex_uvs: ImVec4Like, flags: ImDrawFlags
    ) -> None:
        pass
    # IMGUI_API void  _AddRectBaked(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float r, float t, const ImVec4& tex_uvs, ImDrawFlags flags);    /* original C++ signature */
    def _add_rect_baked(
        self,
        p_min: ImVec2Like,
        p_max: ImVec2Like,
        ...
        pass
    # IMGUI_API void  _AddLine(const ImVec2& p1, const ImVec2& p2, ImU32 col, float thickness, ImDrawFlags flags);    /* original C++ signature */
    def _add_line(self, p1: ImVec2Like, p2: ImVec2Like, col: ImU32, thickness: float, flags: ImDrawFlags) -> None:
        pass
    # IMGUI_API void  _AddRectTinyRounding(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding, float thickness, ImDrawFlags flags);    /* original C++ signature */
    def _add_rect_tiny_rounding(
        self, p_min: ImVec2Like, p_max: ImVec2Like, col: ImU32, rounding: float, thickness: float, flags: ImDrawFlags
    ) -> None:
        pass
    # IMGUI_API void  _SelectLineTexture(float screen_thickness, ImVec2* out_uv0, ImVec2* out_uv1, float* out_fringe, ImDrawFlags flags);    /* original C++ signature */
    def _select_line_texture(
        self, screen_thickness: float, out_uv0: ImVec2Like, out_uv1: ImVec2Like, out_fringe: float, flags: ImDrawFlags
    ) -> float:
        pass
    # IMGUI_API float _CalculateCenterBiasedOffset(float thickness);    /* original C++ signature */
    def _calculate_center_biased_offset(self, thickness: float) -> float:
        pass
    # IMGUI_API void  _AddPolyline(const ImVec2* points, int num_points, ImU32 col, float thickness, ImDrawFlags flags, float max_inner_offset);    /* original C++ signature */
    def _add_polyline(
        self,
        points: ImVec2Like,
        num_points: int,
...
class ImDrawData:
    ...
    frame_count: int  # Frame counter of the emitter context. Mostly for debugging purpose.
...
class ImTextureData:
    ...
    # void*               QueueUserData;    /* original C++ signature */
    queue_user_data: Any  # r    -   // Convenience storage for a staged/multi-threaded rendering texture queue (e.g. imgui_threaded_rendering.h. See #8597). When != None, core assumes the texture is referenced by the queue.
...
class ImFontAtlasFlags_(enum.IntFlag):
    ...
    no_baked_lines = (
        enum.auto()
    )  # (= 1 << 2)  # Don't build anti-aliased line textures into the atlas (save a little texture memory). SINCE 1.93.0 THIS PREVENTS ANTI-ALIASED LINES FROM WORKING AND WILL DISABLE THEM.
    # ImFontAtlasFlags_NoBakedRoundCorners= 1 << 3,       /* original C++ signature */
    no_baked_round_corners = enum.auto()  # (= 1 << 3)  # Don't build round corners into the atlas.
...
class ImFontAtlas:
    ...
    tex_glyph_padding: int  # FIXME: Should be called "TexPackPadding". Padding between glyphs within texture in pixels. Defaults to 1. If your rendering method never relies on bilinear filtering you may set this to 0.
...
class Viewport:
    ...
    # void*               PlatformIconData;    /* original C++ signature */
    platform_icon_data: Any  # None* to hold custom data structure for the OS / platform to specify an icon. Currently unused for exposed to allow experiments.
...
class PlatformIO:
    ...
    # int         Platform_SessionDate;    /* original C++ signature */
    platform_session_date: (
        int  # Integer storing YYYYMMDD e.g. 20261231 corresponding to the beginning of application session.
    )
