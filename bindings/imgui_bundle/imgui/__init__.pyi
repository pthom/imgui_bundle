...
class InputTextFlags_(enum.IntFlag):
    ...
    word_wrap = enum.auto()  # (= 1 << 24)  # InputTextMultine(): word-wrap lines that are too long.
...
class Style:
    ...
    scrollbar_padding: float  # Padding of scrollbar grab within its frame (same for both axises).
...
class IO:
    ...
    # bool        ConfigViewportPlatformFocusSetsImGuiFocus;    /* original C++ signature */
    config_viewport_platform_focus_sets_focus: bool  # = True // When a platform window is focused (e.g. using Alt+Tab, clicking Platform Title Bar), apply corresponding focus on imgui windows (may clear focus/active id from imgui windows location in other platform windows). In principle this is better enabled but we provide an opt-out, because some Linux window managers tend to eagerly focus windows (e.g. on mouse hover, or even a simple window pos/size change).
