...
class LastItemData:
    ...
    clip_rect: ImRect  # Clip rectangle at the time of submitting item (ONLY VALID IF ImGuiItemStatusFlags_HasClipRect is set)
...
class InputFlags_(enum.Enum):
    ...
    lock_this_frame = (
        enum.auto()
    )  # (= 1 << 10)  # Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared at end of frame. This is useful to make input-owner-aware code steal keys from non-input-owner-aware code.
    # ImGuiInputFlags_LockUntilRelease    = 1 << 11,      /* original C++ signature */
    lock_until_release = (
        enum.auto()
    )  # (= 1 << 11)  # Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared when the key is released or at end of each frame if key is released. This is useful to make input-owner-aware code steal keys from non-input-owner-aware code.
    ...
    route_focused = (
        enum.auto()
    )  # (= 1 << 12)  # (Default) Register focused route: Accept inputs if window is in focus stack. Deep-most focused window takes inputs. ActiveId takes inputs over deep-most focused window.
    ...
    route_global = (
        enum.auto()
    )  # (= 1 << 14)  # Register route globally (medium priority: unless an active item registered the route, e.g. CTRL+A registered by InputText).
    # ImGuiInputFlags_RouteGlobalHigh     = 1 << 15,      /* original C++ signature */
    route_global_high = (
        enum.auto()
    )  # (= 1 << 15)  # Register route globally (highest priority: unlikely you need to use that: will interfere with every active items)
    ...
    route_extra_mask_ = enum.auto()  # (= InputFlags_RouteAlways | InputFlags_RouteUnlessBgFocused)
    ...
    # ImGuiInputFlags_SupportedByShortcut         = ImGuiInputFlags_RepeatMask_ | ImGuiInputFlags_RouteMask_ | ImGuiInputFlags_RouteExtraMask_,    /* original C++ signature */
    supported_by_shortcut = (
        enum.auto()
    )  # (= InputFlags_RepeatMask_ | InputFlags_RouteMask_ | InputFlags_RouteExtraMask_)
...
class Context:
    ...
    last_key_mods_change_time: float  # Record the last time key mods changed (affect repeat delay when using shortcut logic)
    # double                  LastKeyModsChangeFromNoneTime;    /* original C++ signature */
    last_key_mods_change_from_none_time: float  # Record the last time key mods changed away from being 0 (affect repeat delay when using shortcut logic)
...
def set_shortcut_routing(key_chord: KeyChord, owner_id: ID = 0, flags: InputFlags = 0) -> bool:
    pass
...
def render_nav_highlight(bb: ImRect, id_: ID, flags: NavHighlightFlags = NavHighlightFlags_TypeDefault) -> None:
