...
class GlassValidatorFontLoader:
    ...
    def refresh_fonts(self):
        ...
        with NoneContext():
            ...
            imgui.font_atlas_add_font_from_file_ttf(font_atlas,str(karla_path), size_18, glyph_ranges_as_int_list=karla_range)
            # load mdi => font won't render
            imgui.font_atlas_add_font_from_file_ttf(font_atlas,str(mdi_path),   size_18, font_cfg=mdi_config,  glyph_ranges_as_int_list=mdi_range)
            # load noto range: ?
            imgui.font_atlas_add_font_from_file_ttf(font_atlas,str(noto_path),  size_18, font_cfg=noto_config, glyph_ranges_as_int_list=noto_range)
            # load fontawesome => OK
            imgui.font_atlas_add_font_from_file_ttf(font_atlas,str(fontawesome_path),   size_18, font_cfg=mdi_config,  glyph_ranges_as_int_list=fontawesome_range)

            # Big font + more glyphs
            self.big_font = \
                imgui.font_atlas_add_font_from_file_ttf(imgui.get_io().fonts,str(karla_path), size_28,                       glyph_ranges_as_int_list=karla_range)
            imgui.font_atlas_add_font_from_file_ttf(imgui.get_io().fonts,str(noto_path),  size_28, font_cfg=noto_config, glyph_ranges_as_int_list=noto_range)
            imgui.font_atlas_add_font_from_file_ttf(imgui.get_io().fonts,str(mdi_path),   size_28, font_cfg=mdi_config,  glyph_ranges_as_int_list=mdi_range)
            # added font awesome
            imgui.font_atlas_add_font_from_file_ttf(font_atlas,str(fontawesome_path),   size_28, font_cfg=mdi_config,  glyph_ranges_as_int_list=fontawesome_range)

            # MsgBox type icons
            # This line alone leads to IM_ASSERT( font && font->IsLoaded() )   ---   imgui.cpp:7795
            # self.icon_font = \
            #     imgui.font_atlas_add_font_from_file_ttf(imgui.get_io().fonts,str(mdi_path),   size_69,                       glyph_ranges_as_int_list=msgbox_range)
            ...
            # self.icon_font = \
            #     imgui.font_atlas_add_font_from_file_ttf(imgui.get_io().fonts,str(karla_path), size_69, glyph_ranges_as_int_list=karla_range)
            self.icon_font = \
                imgui.font_atlas_add_font_from_file_ttf(imgui.get_io().fonts,str(mdi_path), size_69, glyph_ranges_as_int_list=msgbox_range)
            # # # added font awesome
            # self.icon_font = imgui.font_atlas_add_font_from_file_ttf(font_atlas,str(fontawesome_path),   size_69, font_cfg=mdi_config,  glyph_ranges_as_int_list=fontawesome_range)
