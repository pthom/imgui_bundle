...
if HAS_IMMVISION:
    ...
    def _immvision_init():
        ...
        # Create a synthetic image with interesting details for zooming
        w, h = 800, 600
        x = np.linspace(0, 4 * np.pi, w)
        y = np.linspace(0, 3 * np.pi, h)
        xv, yv = np.meshgrid(x, y)
        img = 0.5 + 0.5 * np.sin(xv) * np.cos(yv) + 0.5 * np.sin(3 * xv) * np.cos(2 * yv)
        img = (img * 255).astype(np.uint8)
        _immvision_image = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
...
def _notebook_slide_gui(content_size: ImVec2):
    imgui.dummy(ImVec2(0, hello_imgui.em_size(0.5)))
