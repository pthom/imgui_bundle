...
This document tries to be a complement to https://github.com/ocornut/imgui/blob/master/docs/FAQ.md#q-how-should-i-handle-dpi-in-my-application[the related ImGui FAQ article]


NOTE: This article deals with standard ImGui applications. `HelloImGui` and `Dear ImGui Bundle` provides a https://github.com/pthom/hello_imgui/blob/master/src/hello_imgui/dpi_aware.h[dedicated additional API ].
...
NOTE: the function `ImGui_ImplWin32_EnableDpiAwareness` inside https://github.com/ocornut/imgui/blob/master/backends/imgui_impl_win32.cpp[imgui_impl_win32.cpp]  is possibly broken. HelloImGui provides a corrected version:  https://github.com/pthom/hello_imgui/blob/master/src/hello_imgui/internal/backend_impls/backend_window_helper/win32_dpi_awareness.h[win32_dpi_awareness.h] and https://github.com/pthom/hello_imgui/blob/master/src/hello_imgui/internal/backend_impls/backend_window_helper/win32_dpi_awareness.cpp[win32_dpi_awareness.cpp]
...
You need to query the application monitor current Dpi scale. To achieve this, you can call https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getdpiforwindow[DpiForWindow] on windows.
