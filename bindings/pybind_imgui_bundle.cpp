...
void py_init_module_imgui_bundle_inner(py::module& m)
{
    ...
    m.def("run",
        py::overload_cast<HelloImGui::RunnerParams &, bool, bool, const std::optional<NodeEditorConfig> &, bool, const std::optional<ImGuiMd::MarkdownOptions> &, float>(ImGuiBundle::Run),
        py::arg("runner_params"), py::arg("with_implot") = false, py::arg("with_node_editor") = false, py::arg("with_node_editor_config") = py::none(), py::arg("with_markdown") = false, py::arg("with_markdown_options") = py::none(), py::arg("fps_idle") = 4.f,
    ...
    m.def("run",
        py::overload_cast<const HelloImGui::VoidFunction &, const std::optional<ImVec2> &, const std::optional<std::string> &, bool, bool, const std::optional<NodeEditorConfig> &, bool, const std::optional<ImGuiMd::MarkdownOptions> &, float>(ImGuiBundle::Run),
        py::arg("gui_function"), py::arg("window_size") = py::none(), py::arg("window_title") = py::none(), py::arg("with_implot") = false, py::arg("with_node_editor") = false, py::arg("with_node_editor_config") = py::none(), py::arg("with_markdown") = false, py::arg("with_markdown_options") = py::none(), py::arg("fps_idle") = 4.f,
        "Helper to run a hello_imgui app for imgui_bundle:\n\n- if `with_markdown` or `with_markdown_options` is specified, then  the markdown context will be initialized\n    (i.e. required fonts will be loaded)\n- if `with_implot` is True, then a context for implot will be created/destroyed automatically\n- if `with_node_editor` or with_node_editor_config` is specified, then a context for imgui_node_editor\n    will be created automatically.\n - `fpsIdle` enables to set the app FPS when it is idle (set it to 0 for maximum FPS).");
