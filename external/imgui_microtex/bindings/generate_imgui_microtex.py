...
LEVEL1_HEADER = THIS_DIR + "/../../imgui_rich_md/imgui_rich_md/imgui_rich_md/backends/latex/rich_md_latex.h"


def write_amalgamation() -> str:
    """The Python module binds the backend (Level 1, in imgui_rich_md) and the bundle's Level 2 (textures):
    litgen reads one header, so both are concatenated into a generated file."""
    level1 = open(LEVEL1_HEADER).read()
    level2 = open(CPP_HEADERS_DIR + "/imgui_microtex.h").read()
    level2 = level2.replace('#include "imgui_rich_md/backends/latex/rich_md_latex.h"\n', "")
    amalgamation = THIS_DIR + "/imgui_microtex_amalgamation.h"
    with open(amalgamation, "w") as f:
        f.write("// Generated by generate_imgui_microtex.py: the backend's header (imgui_rich_md) followed by the bundle's Level 2\n")
        f.write(level1 + "\n" + level2)
    return amalgamation
...
def main() -> None:
    ...
    input_cpp_header = write_amalgamation()
    ...
    options.custom_bindings.add_custom_bindings_to_class(
        qualified_class="RichMd::Latex::RenderedFormula",
        stub_code='''
