...
{
    using namespace RichMd::Latex;

    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  AUTOGENERATED CODE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // <litgen_pydef> // Autogenerated code below! Do not edit!
    ////////////////////    <generated_from:imgui_microtex_amalgamation.h>    ////////////////////
    auto pyEnumTexStyle =
        nb::enum_<RichMd::Latex::TexStyle>(m, "TexStyle", nb::is_arithmetic(), " Selects the layout style used when rendering a formula. This maps directly\n to MicroTeX's TexStyle and corresponds to the four TeX styles defined by\n Knuth (D, T, S, SS). Pick Display for centered \"display math\" ($$...$$)\n and Text for inline math ($...$).\n\n The style affects symbol size, big-operator appearance, and the spacing\n around \\frac (numerator shift-up and denominator shift-down): Display gives\n generous spacing; Text is compact.")
            .value("display", RichMd::Latex::TexStyle::Display, " Largest size. Big operators (\\sum, \\int, ...) use their large variants\n with limits placed above and below. \\frac uses generous vertical\n spacing. This is what LaTeX uses inside $$...$$ and \\[...\\].")
            .value("text", RichMd::Latex::TexStyle::Text, " Default inline size. Big operators use their small variants with\n limits attached as sub/superscripts. \\frac uses compact spacing.\n This is what LaTeX uses inside $...$ and \\(...\\).")
            .value("script", RichMd::Latex::TexStyle::Script, " Smaller size used by LaTeX inside sub/superscripts. Rarely useful at\n the top level; MicroTeX switches to it automatically where needed.")
            .value("script_script", RichMd::Latex::TexStyle::ScriptScript, "Smallest size, used inside scripts-of-scripts. Same caveat as Script.");
    ...
    auto pyClassRenderedFormula =
        nb::class_<RichMd::Latex::RenderedFormula>
            (m, "RenderedFormula", "")
        .def("__init__", [](RichMd::Latex::RenderedFormula * self, int Width = 0, int Height = 0, int Depth = 0, int BaselineY = 0)
        {
            new (self) RichMd::Latex::RenderedFormula();  // placement new
            auto r_ctor_ = self;
            r_ctor_->Width = Width;
    ...
    m.def("render",
        [](const std::string & latex, float fontSize, const std::optional<const ImU32> & color = std::nullopt, RichMd::Latex::TexStyle style = RichMd::Latex::TexStyle::Text) -> RichMd::Latex::RenderedFormula
        {
            auto Render_adapt_mutable_param_with_default_value = [](const std::string & latex, float fontSize, const std::optional<const ImU32> & color = std::nullopt, RichMd::Latex::TexStyle style = RichMd::Latex::TexStyle::Text) -> RichMd::Latex::RenderedFormula
            {
                ...
                auto lambda_result = RichMd::Latex::Render(latex, fontSize, color_or_default, style);
                return lambda_result;
            };

            return Render_adapt_mutable_param_with_default_value(latex, fontSize, color, style);
        },
        nb::arg("latex"), nb::arg("font_size"), nb::arg("color").none() = nb::none(), nb::arg("style") = RichMd::Latex::TexStyle::Text,
        "Python bindings defaults:\n    If color is None, then its default value will be: IM_COL32_BLACK");

    m.def("render",
        nb::overload_cast<const std::string &, float, const ImVec4 &, RichMd::Latex::TexStyle>(RichMd::Latex::Render), nb::arg("latex"), nb::arg("font_size"), nb::arg("color"), nb::arg("style") = RichMd::Latex::TexStyle::Text);


    auto pyClassFormulaTexture =
        nb::class_<RichMd::Latex::FormulaTexture>
        ...
        .def("__init__", [](RichMd::Latex::FormulaTexture * self, const std::optional<const std::shared_ptr<HelloImGui::TextureGpu>> & Texture = std::nullopt, int Width = 0, int Height = 0, int Depth = 0, int BaselineY = 0, int LastUsedFrame = 0)
        {
            new (self) RichMd::Latex::FormulaTexture();  // placement new
            auto r_ctor_ = self;
        ...
        .def_rw("texture", &RichMd::Latex::FormulaTexture::Texture, "")
        .def_rw("width", &RichMd::Latex::FormulaTexture::Width, "")
        .def_rw("height", &RichMd::Latex::FormulaTexture::Height, "")
        .def_rw("depth", &RichMd::Latex::FormulaTexture::Depth, "")
        .def_rw("baseline_y", &RichMd::Latex::FormulaTexture::BaselineY, " BaselineY: pixel y-offset from the TOP of the image to the formula's\n typographic baseline. See RenderedFormula::BaselineY for details.")
        .def_rw("last_used_frame", &RichMd::Latex::FormulaTexture::LastUsedFrame, " LastUsedFrame: ImGui::GetFrameCount() at the most recent cache hit\n or insertion. Used by the optional frame-generation eviction (see\n SetEvictionFrames). Not interesting to direct API consumers.")
        .def("texture_id",
            &RichMd::Latex::FormulaTexture::TextureId, "Convenience: returns the GPU texture id, or 0 if no texture is held.")
        ;


    m.def("render_to_texture",
        [](const std::string & latex, float fontSize, const std::optional<const ImU32> & color = std::nullopt, RichMd::Latex::TexStyle style = RichMd::Latex::TexStyle::Text) -> RichMd::Latex::FormulaTexture
        {
            auto RenderToTexture_adapt_mutable_param_with_default_value = [](const std::string & latex, float fontSize, const std::optional<const ImU32> & color = std::nullopt, RichMd::Latex::TexStyle style = RichMd::Latex::TexStyle::Text) -> RichMd::Latex::FormulaTexture
            {
                ...
                auto lambda_result = RichMd::Latex::RenderToTexture(latex, fontSize, color_or_default, style);
                return lambda_result;
            };

            return RenderToTexture_adapt_mutable_param_with_default_value(latex, fontSize, color, style);
        },
        nb::arg("latex"), nb::arg("font_size"), nb::arg("color").none() = nb::none(), nb::arg("style") = RichMd::Latex::TexStyle::Text,
        "Python bindings defaults:\n    If color is None, then its default value will be: IM_COL32_BLACK");

    m.def("render_to_texture",
        nb::overload_cast<const std::string &, float, const ImVec4 &, RichMd::Latex::TexStyle>(RichMd::Latex::RenderToTexture), nb::arg("latex"), nb::arg("font_size"), nb::arg("color"), nb::arg("style") = RichMd::Latex::TexStyle::Text);

    m.def("to_texture",
        RichMd::Latex::ToTexture,
        nb::arg("formula"),
        "Convert a previously rendered formula to an ImGui texture (not cached).");

    m.def("clear_texture_cache",
        RichMd::Latex::ClearTextureCache, "Clear the texture cache.");

    m.def("set_eviction_frames",
        RichMd::Latex::SetEvictionFrames,
        nb::arg("n"),
    ...
    m.def("get_cache_size",
        RichMd::Latex::GetCacheSize, " Returns the current cache size (number of formula entries). Useful for\n diagnostics, monitoring, and tests.");
    ////////////////////    </generated_from:imgui_microtex_amalgamation.h>    ////////////////////
