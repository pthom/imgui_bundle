...
#include "casters/immvision_nb_type_casters.h"
#include <cassert>
...
void py_init_module_immvision(nb::module_& m)
{
    ...
    auto pyClassSize2d =
        nb::class_<ImmVision::Size2d>
            (m, "Size2d", "2D double-precision size. Used for drawing operations (ellipse, rectangle_size).")
        .def_rw("width", &ImmVision::Size2d::width, "")
        .def_rw("height", &ImmVision::Size2d::height, "")
        .def(nb::init<>())
        .def(nb::init<double, double>(),
            nb::arg("w"), nb::arg("h"))
        .def(nb::init<const ImmVision::Size &>(),
            nb::arg("s"))
        ;


    auto pyClassColor4d =
        nb::class_<ImmVision::Color4d>
            (m, "Color4d", "4-channel double color value (e.g. RGBA or BGRA).")
        .def_prop_ro("v",
            [](ImmVision::Color4d &self) -> nb::ndarray<double, nb::numpy, nb::shape<4>, nb::c_contig>
            {
                return self.v;
            },
            "")
    ...
    auto pyClassRect =
        nb::class_<ImmVision::Rect>
            (m, "Rect", "Integer rectangle (x, y, width, height).")
        .def_rw("x", &ImmVision::Rect::x, "")
        .def_rw("y", &ImmVision::Rect::y, "")
        .def_rw("width", &ImmVision::Rect::width, "")
        .def_rw("height", &ImmVision::Rect::height, "")
        .def(nb::init<>())
        .def(nb::init<int, int, int, int>(),
            nb::arg("x_"), nb::arg("y_"), nb::arg("w"), nb::arg("h"))
        .def(nb::init<ImmVision::Point, ImmVision::Size>(),
            nb::arg("pt"), nb::arg("sz"))
        .def(nb::init<ImmVision::Point, ImmVision::Point>(),
    ...
    auto pyClassImageParams =
        ...
        .def("__init__", [](ImmVision::ImageParams * self, bool RefreshImage = false, const std::optional<const Size> & ImageDisplaySize = std::nullopt, const std::optional<const Matrix33d> & ZoomPanMatrix = std::nullopt, std::string ZoomKey = "", ImmVision::ImageInterpolationMode InterpolationMode = ImmVision::ImageInterpolationMode::Adaptive, const std::optional<const ImmVision::ColormapSettingsData> & ColormapSettings = std::nullopt, std::string ColormapKey = "", bool PanWithMouse = true, bool ZoomWithMouseWheel = true, bool CanResize = true, bool ResizeKeepAspectRatio = true, int SelectedChannel = -1, bool ShowSchoolPaperBackground = true, bool ShowAlphaChannelCheckerboard = true, bool ShowGrid = true, bool DrawValuesOnZoomedPixels = true, bool ShowImageInfo = true, bool ShowPixelInfo = true, bool ShowZoomButtons = true, bool ShowOptionsPanel = false, bool ShowOptionsInTooltip = false, bool ShowOptionsButton = true, const std::optional<const std::vector<Point>> & WatchedPixels = std::nullopt, bool AddWatchedPixelOnDoubleClick = true, bool HighlightWatchedPixels = true, const std::optional<const ImmVision::MouseInformation> & MouseInfo = std::nullopt, const std::optional<const ImVec2> & ImageScreenTopLeft = std::nullopt)
        {
            ...
            if (WatchedPixels.has_value())
                ...
                r_ctor_->WatchedPixels = std::vector<Point>();
        ...
        .def_rw("image_screen_top_left", &ImmVision::ImageParams::ImageScreenTopLeft, " Screen position (ImGui screen coordinates) of the top left corner of the image display area,\n filled after displaying an image (see ImageToScreen)")
        .def("image_to_screen",
            &ImmVision::ImageParams::ImageToScreen,
            nb::arg("image_point"),
    ...
    m.def("image_display",
        ...
        {
            auto ImageDisplay_adapt_mutable_param_with_default_value = [](const std::string & label_id, const ImageBuffer & image, const std::optional<const Size> & imageDisplaySize = std::nullopt, bool refreshImage = false, bool showOptionsButton = false) -> Point2d
            {

                const Size& imageDisplaySize_or_default = [&]() -> const Size {
                    if (imageDisplaySize.has_value())
                ...
                auto lambda_result = ImmVision::ImageDisplay(label_id, image, imageDisplaySize_or_default, refreshImage, showOptionsButton);
    ...
    m.def("inspector_add_image",
        [](const ImageBuffer & image, const std::string & legend, const std::string & zoomKey = "", const std::string & colormapKey = "", const std::optional<const Point2d> & zoomCenter = std::nullopt, double zoomRatio = -1.)
        {
            auto Inspector_AddImage_adapt_mutable_param_with_default_value = [](const ImageBuffer & image, const std::string & legend, const std::string & zoomKey = "", const std::string & colormapKey = "", const std::optional<const Point2d> & zoomCenter = std::nullopt, double zoomRatio = -1.)
            {

                const Point2d& zoomCenter_or_default = [&]() -> const Point2d {
    ...
    auto pyClassGlTexture =
        nb::class_<ImmVision::GlTexture>
            (m, "GlTexture", "GlTexture contains an OpenGL texture which can be created or updated from an ImageBuffer (C++), or numpy array (Python)")
        .def(nb::init<>(),
            "Create an empty texture")
        .def(nb::init<const ImageBuffer &, bool>(),
    ...
    // =========================================================================
    // _test_helpers: private submodule for testing type casters without OpenGL
    // =========================================================================
    {
        auto th = m.def_submodule("_test_helpers", "Private test helpers for type caster tests");

        // Round-trip: numpy -> ImageBuffer -> numpy (tests both from_python and from_cpp)
        th.def("roundtrip", [](const ImageBuffer& img) -> ImageBuffer { return img; },
            nb::arg("img"));

        // Return data pointer as int (to verify zero-copy)
        th.def("data_pointer", [](const ImageBuffer& img) -> uintptr_t {
            return reinterpret_cast<uintptr_t>(img.data);
        }, nb::arg("img"));

        // Return image metadata: (width, height, channels, depth_as_int)
        th.def("image_info", [](const ImageBuffer& img) -> std::tuple<int, int, int, int> {
            return {img.width, img.height, img.channels, static_cast<int>(img.depth)};
        }, nb::arg("img"));

        // Create an owned ImageBuffer on C++ side (tests from_cpp with self-owned data)
        th.def("create_test_image", [](int w, int h, int channels, int depth_int) -> ImageBuffer {
            return ImageBuffer::Zeros(w, h, channels, static_cast<ImageDepth>(depth_int));
        }, nb::arg("w"), nb::arg("h"), nb::arg("channels"), nb::arg("depth_int"));

        // Struct that stores an ImageBuffer (tests lifetime / _ref_keeper)
        struct TestHolder {
            ImageBuffer stored;
        };
        nb::class_<TestHolder>(th, "TestHolder")
            .def(nb::init<>())
            .def("store", [](TestHolder& self, const ImageBuffer& img) { self.stored = img; },
                nb::arg("img"))
        ...
        th.def("roundtrip_point", [](const Point& p) -> Point { return p; }, nb::arg("p"));
        th.def("roundtrip_point2d", [](const Point2d& p) -> Point2d { return p; }, nb::arg("p"));
        th.def("roundtrip_size", [](const Size& s) -> Size { return s; }, nb::arg("s"));
        th.def("roundtrip_matrix33d", [](const Matrix33d& m) -> Matrix33d { return m; }, nb::arg("m"));
    }
}
